...
SCRIPT = ROOT / "scripts" / "write_avbd_demo3d_stack_ratio_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo3d_stack_ratio_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo3d_stack_ratio",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo3d_stack_ratio.png"
...
def _write_benchmark_json(
    tmp_path: Path,
    *,
    rigid_bodies: float = 5.0,
    cpu_time: float = 90.0,
) -> Path:
    benchmark = {
        "benchmarks": [
            {
                "collision_shapes": 5.0,
                "cpu_time": cpu_time,
                "iterations": 10,
                "name": "BM_AvbdDemo3dStackRatioStep",
                "real_time": 100.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 0.0,
                "run_name": "BM_AvbdDemo3dStackRatioStep",
                "run_type": "iteration",
...
def test_avbd_demo3d_stack_ratio_packet_records_evidence(tmp_path: Path) -> None:
    ...
    assert packet["scene"] == "avbd_demo3d_stack_ratio"
    assert packet["source_demo_reference_row"]["scene_name"] == "Stack Ratio"
    assert packet["visual_capture"]["frames"]["count"] == 2
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo3dStackRatioStep"
    ...
    assert packet["reference_timing"]["invariants"]["dynamic_bodies"] == 4
    assert packet["reference_timing"]["invariants"]["static_bodies"] == 1
    assert packet["reference_timing"]["invariants"]["stack_ratio_box_count"] == 4
...
def test_avbd_demo3d_stack_ratio_packet_rejects_wrong_scene(tmp_path: Path) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit, match="capture scene must be avbd_demo3d_stack_ratio"
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_demo3d_stack_ratio_packet_rejects_wrong_counts(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path, rigid_bodies=4.0)
    reference_json = _write_reference_timing_json(tmp_path)

    with pytest.raises(SystemExit, match="expected rigid_bodies=5"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo3d_stack_ratio_packet_rejects_wrong_reference(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path, stack_ratio_box_count=3)

    with pytest.raises(SystemExit, match="expected stack_ratio_box_count=4"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
