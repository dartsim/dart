...
SCRIPT = ROOT / "scripts" / "write_avbd_demo2d_hanging_rope_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo2d_hanging_rope_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo2d_hanging_rope",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo2d_hanging_rope.png"
...
def _write_benchmark_json(
    tmp_path: Path,
    *,
    rigid_bodies: float = 50.0,
    cpu_time: float = 120.0,
) -> Path:
    benchmark = {
        "benchmarks": [
            {
                "collision_shapes": 50.0,
                "cpu_time": cpu_time,
                "iterations": 10,
                "linear_point_joints": 49.0,
                "name": "BM_AvbdDemo2dHangingRopeStep",
                "real_time": 125.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 49.0,
                "run_name": "BM_AvbdDemo2dHangingRopeStep",
                "run_type": "iteration",
...
def test_avbd_demo2d_hanging_rope_packet_records_evidence(tmp_path: Path) -> None:
    ...
    assert packet["scene"] == "avbd_demo2d_hanging_rope"
    assert packet["source_demo_reference_row"]["scene_name"] == "Hanging Rope"
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo2dHangingRopeStep"
    ...
    assert packet["reference_timing"]["scene_builder"] == "sceneHangingRope"
    assert packet["reference_timing"]["invariants"]["dynamic_bodies"] == 49
    assert packet["reference_timing"]["invariants"]["static_bodies"] == 1
    assert packet["reference_timing"]["invariants"]["linear_point_joints"] == 49
    assert packet["reference_timing"]["invariants"]["rope_links"] == 50
    assert packet["reference_timing"]["invariants"]["heavy_block_size"] == 10.0
...
def test_avbd_demo2d_hanging_rope_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_demo2d_hanging_rope",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_demo2d_hanging_rope_packet_rejects_wrong_counts(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path, rigid_bodies=49.0)
    reference_json = _write_reference_timing_json(tmp_path)

    with pytest.raises(SystemExit, match="expected rigid_bodies=50"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_hanging_rope_packet_rejects_wrong_reference(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path, linear_point_joints=48)

    with pytest.raises(SystemExit, match="expected linear_point_joints=49"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_hanging_rope_packet_uses_benchmark_median(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    ...
    median_row.update(
        {
            ...
            "cpu_time": 110.0,
            "name": "BM_AvbdDemo2dHangingRopeStep_median",
            "run_type": "aggregate",
        }
    )
    ...
    stddev_row.update(
        {
            ...
            "cpu_time": 5.0,
            "linear_point_joints": 0.0,
            "name": "BM_AvbdDemo2dHangingRopeStep_stddev",
            "rigid_bodies": 0.0,
