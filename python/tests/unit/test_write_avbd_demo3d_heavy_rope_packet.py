...
SCRIPT = ROOT / "scripts" / "write_avbd_demo3d_heavy_rope_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo3d_heavy_rope_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo3d_heavy_rope",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo3d_heavy_rope.png"
        ...
        "benchmarks": [
            {
                ...
                "name": "BM_AvbdDemo3dHeavyRopeStep",
                "real_time": 100.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 19.0,
                "run_name": "BM_AvbdDemo3dHeavyRopeStep",
                "run_type": "iteration",
...
def test_avbd_demo3d_heavy_rope_packet_records_evidence(tmp_path: Path) -> None:
    ...
    assert packet["scene"] == "avbd_demo3d_heavy_rope"
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo3dHeavyRopeStep"
    ...
    assert packet["reference_timing"]["invariants"]["heavy_block_size"] == 5.0
...
def test_avbd_demo3d_heavy_rope_packet_rejects_wrong_scene(tmp_path: Path) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo3d_rope")
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path)

    with pytest.raises(
        SystemExit, match="capture scene must be avbd_demo3d_heavy_rope"
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_demo3d_heavy_rope_packet_rejects_wrong_counts(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_demo3d_heavy_rope_packet_rejects_wrong_reference(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
