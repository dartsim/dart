...
SCRIPT = ROOT / "scripts" / "write_avbd_articulated_breakable_motor_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_articulated_breakable_motor_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_articulated_motor_breakable_joint",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_articulated_motor_breakable_joint.png"
...
def _write_benchmark_json(
    ...
    *,
    ...
    benchmark_name: str = "BM_AvbdArticulatedBreakableMotorStep/1",
) -> Path:
    benchmark = {
        "benchmarks": [
            {
...
def test_avbd_articulated_breakable_motor_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["scene"] == "avbd_articulated_motor_breakable_joint"
    ...
    assert packet["visual_scene_invariants"] == {
        ...
        "target_speed_rad_per_s": 0.8,
        "time_step": 0.005,
    }
    assert packet["visual_capture"]["frames"]["count"] == 2
    assert packet["benchmark"]["benchmark"] == "BM_AvbdArticulatedBreakableMotorStep/1"
    ...
    assert "prismatic and world-anchored motor break/reset packets" in packet[
        "remaining_gates"
    ]


def test_avbd_articulated_breakable_motor_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_articulated_motor_breakable_joint",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_articulated_breakable_motor_packet_rejects_wrong_benchmark_arg(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(
        tmp_path,
        benchmark_name="BM_AvbdArticulatedBreakableMotorStep/8",
    )

    with pytest.raises(
        SystemExit,
        match="missing BM_AvbdArticulatedBreakableMotorStep/1",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_articulated_breakable_motor_packet_rejects_wrong_motor_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_articulated_breakable_motor_packet_rejects_wrong_breakable_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
