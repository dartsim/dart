...
def _cache_reports_experimental_disabled() -> bool:
    ...
    if override and override.lower() in {"0", "false", "no", "off"}:
        return True
    value = _read_cache_flag("DART_BUILD_SIMULATION_EXPERIMENTAL")
...
def _build_frictionless_sphere_scene(sx):
    ...
    ground.set_collision_shape(
        sx.CollisionShape.box(np.array([5.0, 5.0, 0.5]))
    )
    ground.friction = 0.0
...
def test_world_exposes_differentiable_construction_flags():
    ...
    assert (
        default_world.contact_solver_method
...
def test_contact_gradient_mode_reflects_through_world():
    ...
    default_world = sx.World()
    assert (
        default_world.contact_gradient_mode == sx.ContactGradientMode.ANALYTIC
    )
...
def test_get_step_derivatives_and_vjp_match():
    ...
    np.testing.assert_allclose(
        np.asarray(result.state), state_jac.T @ grad, atol=1e-12
    )
    np.testing.assert_allclose(
...
def test_add_differentiable_parameter_center_of_mass_throws():
    ...
    with pytest.raises(Exception):
        world.add_differentiable_parameter(
...
def test_diff_timestep_autograd_matches_jacobians():
    ...
    state = torch.tensor(
        world.state_vector, dtype=torch.float64, requires_grad=True
    )
