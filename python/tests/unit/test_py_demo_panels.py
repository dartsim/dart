...
def test_lcp_physics_exposes_solver_manifest_and_benchmark_metadata() -> None:
    ...
    assert "SymmetricPsor" not in profile_by_surface["Boxed"]["current_leaders"]
    ...
    assert "Jacobi" not in profile_by_surface["FrictionIndex"][
        "current_leaders"
    ]
    ...
    assert "ShockPropagation is above 2x" in profile_by_surface["FrictionIndex"][
        "current_laggards"
    ]
    assert "Apgd" not in profile_by_surface["FrictionIndex"][
        "current_laggards"
    ]
    ...
    assert "Jacobi" in profile_by_surface["FrictionIndex"][
        "current_laggards"
    ]
