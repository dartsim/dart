...
def test_lcp_physics_exposes_solver_manifest_and_benchmark_metadata() -> None:
    ...
    assert "No Standard solver average is above 1.5x" in profile_by_surface[
        "Standard"
    ]["current_laggards"]
    ...
    assert "Dantzig" not in profile_by_surface["Boxed"]["current_leaders"]
    ...
    assert "BGS and ShockPropagation are above 2x" in profile_by_surface[
        "Boxed"
    ][
        "current_laggards"
    ]
