...
SCRIPT = ROOT / "scripts" / "write_avbd_articulated_high_ratio_chain_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_articulated_high_ratio_chain_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_articulated_high_ratio_chain",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_articulated_high_ratio_chain.png"
...
def test_avbd_articulated_high_ratio_chain_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["benchmark"]["benchmark"] == "BM_AvbdArticulatedHighRatioChainStep"
    assert packet["benchmark"]["cpu_time_per_step_ns"] == 2400.0
...
def test_avbd_articulated_high_ratio_chain_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_articulated_high_ratio_chain",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_articulated_high_ratio_chain_packet_rejects_wrong_counters(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path, links=50.0)

    with pytest.raises(SystemExit, match="expected links=5"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
