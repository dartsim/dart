...
def test_cylindrical_joint_constraint():
    ...
    bd1 = chain.getBodyNode("link 6")
    bd2 = chain.getBodyNode("link 10")
    offset1 = np.array([0.0, 0.025, 0.0])
    joint_pos = bd1.getTransform().multiply(offset1)
    offset2 = bd2.getTransform().inverse().multiply(joint_pos)
    axis = np.array([0.0, 1.0, 0.0])

    constraint = dart.constraint.CylindricalJointConstraint(
        bd1, bd2, joint_pos, axis, axis
    )
    assert (
        constraint.getType()
        == dart.constraint.CylindricalJointConstraint.getStaticType()
    )
    ...
    pos1 = bd1.getTransform().multiply(offset1)
    pos2 = bd2.getTransform().multiply(offset2)
    axis1 = bd1.getTransform().rotation().dot(axis)
    axis2 = bd2.getTransform().rotation().dot(axis)
    axis1 /= np.linalg.norm(axis1)
    axis2 /= np.linalg.norm(axis2)

    radial_error = pos1 - pos2
    radial_error -= np.dot(radial_error, axis1) * axis1
    assert np.linalg.norm(radial_error) < 5e-2
...
def test_revolute_joint_constraint():
    ...
    assert (
        constraint.getType() == dart.constraint.RevoluteJointConstraint.getStaticType()
    )
