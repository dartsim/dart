...
def test_cylindrical_joint_constraint():
    world = dart.io.SkelParser.read_world("dart://sample/skel/chain.skel")
    world.set_gravity([0, -9.81, 0])
    world.set_time_step(1.0 / 2000)
    ...
    chain = world.get_skeleton(0)
    init_pose = np.zeros(chain.get_num_dofs())
    init_pose[[20, 23, 26, 29]] = np.pi * 0.35
    chain.set_positions(init_pose)
    ...
    constraint = dart.CylindricalJointConstraint(
        bd1, bd2, joint_pos, axis, axis
    )
    assert (
        constraint.get_type()
        == dart.CylindricalJointConstraint.get_static_type()
    )
    ...
    final_radial_error = _radial_distance(
        bd1.get_transform().multiply(offset1),
        bd2.get_transform().multiply(offset2),
        axis_array,
    )
    assert final_radial_error < 5e-2

    axis1 = bd1.get_transform().rotation().dot(axis_array)
    axis2 = bd2.get_transform().rotation().dot(axis_array)
    axis1 /= np.linalg.norm(axis1)
    axis2 /= np.linalg.norm(axis2)
    assert abs(np.dot(axis1, axis2)) > 0.2


def _radial_distance(point1, point2, axis):
    axis = axis / np.linalg.norm(axis)
    delta = point1 - point2
    return np.linalg.norm(delta - np.dot(delta, axis) * axis)
