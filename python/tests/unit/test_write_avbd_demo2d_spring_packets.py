...
def test_avbd_demo2d_spring_packet_records_evidence(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_packet.py")
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo2d_spring")
    benchmark_json = _write_benchmark_json(
        tmp_path,
        name="BM_AvbdDemo2dSpringStep",
        rigid_bodies=2.0,
    ...
    assert packet["scene"] == "avbd_demo2d_spring"
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo2dSpringStep"
    ...
    assert packet["reference_timing"]["scene_builder"] == "sceneSpring"
    assert packet["reference_timing"]["invariants"]["dynamic_bodies"] == 1
    assert packet["reference_timing"]["invariants"]["distance_springs"] == 1
    assert packet["reference_timing"]["invariants"]["spring_rest_length"] == 4.0
...
def test_avbd_demo2d_spring_ratio_packet_records_evidence(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_ratio_packet.py")
    capture_manifest = _write_capture_manifest(
        tmp_path,
        scene="avbd_demo2d_spring_ratio",
    )
    benchmark_json = _write_benchmark_json(
        tmp_path,
        name="BM_AvbdDemo2dSpringRatioStep",
        rigid_bodies=8.0,
    ...
    packet = json.loads(output.read_text())
    assert packet["scene"] == "avbd_demo2d_spring_ratio"
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo2dSpringRatioStep"
    assert packet["dart_benchmark"]["invariants"] == {
        "collision_shapes": 8,
        "distance_springs": 7,
        "ignored_collision_pairs": 7,
        "rigid_bodies": 8,
        "rigid_body_joints": 0,
        "source_scene_index": 10,
        "time_step": 1.0 / 60.0,
    }
    assert packet["reference_timing"]["scene_builder"] == "sceneSpringsRatio"
...
def test_avbd_demo2d_spring_packet_rejects_wrong_scene(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_packet.py")
    ...
    with pytest.raises(SystemExit, match="capture scene must be avbd_demo2d_spring"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_spring_packet_rejects_wrong_counts(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_packet.py")
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo2d_spring")
    benchmark_json = _write_benchmark_json(
        tmp_path,
...
def test_avbd_demo2d_spring_packet_rejects_wrong_reference(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_packet.py")
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo2d_spring")
    benchmark_json = _write_benchmark_json(
        tmp_path,
    ...
    with pytest.raises(SystemExit, match="reference timing expected scene_index=9"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_spring_packet_uses_benchmark_median(tmp_path: Path) -> None:
    module = _load_packet_module("write_avbd_demo2d_spring_packet.py")
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo2d_spring")
    benchmark_json = _write_benchmark_json(
        tmp_path,
    ...
    benchmark["benchmarks"].append(
        {
            **benchmark["benchmarks"][0],
            ...
            "cpu_time": 80.0,
            "iterations": 3,
            "name": "BM_AvbdDemo2dSpringStep_median",
            "run_type": "aggregate",
        }
    )
