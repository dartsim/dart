...
SCRIPT = (
    ROOT
    / "scripts"
    / "write_avbd_articulated_fixed_pair_breakable_joint_packet.py"
)


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_articulated_fixed_pair_breakable_joint_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_articulated_fixed_pair_breakable_joint",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_articulated_fixed_pair_breakable_joint.png"
...
def test_avbd_articulated_fixed_pair_breakable_joint_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["scene"] == "avbd_articulated_fixed_pair_breakable_joint"
    ...
    assert "spherical break/reset packets" in packet["remaining_gates"]


def test_avbd_articulated_fixed_pair_breakable_joint_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_articulated_fixed_pair_breakable_joint",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_articulated_fixed_pair_breakable_joint_packet_rejects_wrong_arg(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_articulated_fixed_pair_breakable_joint_packet_rejects_wrong_joint_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
