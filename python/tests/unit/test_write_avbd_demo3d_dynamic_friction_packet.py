...
SCRIPT = ROOT / "scripts" / "write_avbd_demo3d_dynamic_friction_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo3d_dynamic_friction_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo3d_dynamic_friction",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo3d_dynamic_friction.png"
        ...
        "benchmarks": [
            {
                ...
                "cpu_time": cpu_time,
                "iterations": 10,
                "name": "BM_AvbdDemo3dDynamicFrictionStep",
                "real_time": 100.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 0.0,
                "run_name": "BM_AvbdDemo3dDynamicFrictionStep",
                "run_type": "iteration",
...
def test_avbd_demo3d_dynamic_friction_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["scene"] == "avbd_demo3d_dynamic_friction"
...
def test_avbd_demo3d_dynamic_friction_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_demo3d_dynamic_friction",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_demo3d_dynamic_friction_packet_rejects_wrong_counts(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_demo3d_dynamic_friction_packet_rejects_wrong_reference(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path, max_friction=4.0)

    with pytest.raises(SystemExit, match="expected max_friction=5"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
