...
SCRIPT = (
    ROOT
    / "scripts"
    / "write_avbd_articulated_world_prismatic_breakable_motor_packet.py"
)


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_articulated_world_prismatic_breakable_motor_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_articulated_prismatic_motor_breakable_joint",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_articulated_prismatic_motor_breakable_joint.png"
...
def _write_benchmark_json(
    ...
    *,
    ...
    benchmark_name: str = "BM_AvbdArticulatedWorldPrismaticBreakableMotorStep/1",
) -> Path:
    benchmark = {
        "benchmarks": [
            {
...
def test_avbd_world_prismatic_breakable_motor_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["scene"] == "avbd_articulated_prismatic_motor_breakable_joint"
    assert packet["target"] == {
        "broad_motor_lifecycle_complete": False,
        "row_family": (
            "public world-anchored articulated prismatic velocity motor break/reset"
        ),
        "scope": (
    ...
    assert packet["benchmark"]["cpu_time_per_step_ns"] == 5900.0
    ...
    assert "rigid/articulated non-motor breakable joint packets" in packet[
        "remaining_gates"
    ]
    assert "world-anchored revolute motor break/reset packet" not in packet[
        "remaining_gates"
    ]


def test_avbd_world_prismatic_breakable_motor_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
...
def test_avbd_world_prismatic_breakable_motor_packet_rejects_wrong_arg(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(
        tmp_path,
        benchmark_name="BM_AvbdArticulatedWorldPrismaticBreakableMotorStep/8",
    )

    with pytest.raises(
        SystemExit,
        match="missing BM_AvbdArticulatedWorldPrismaticBreakableMotorStep/1",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_world_prismatic_breakable_motor_packet_rejects_wrong_motor_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_world_prismatic_breakable_motor_packet_rejects_wrong_breakable_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
