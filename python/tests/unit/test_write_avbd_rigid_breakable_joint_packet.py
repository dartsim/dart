...
SCRIPT = ROOT / "scripts" / "write_avbd_rigid_breakable_joint_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_rigid_breakable_joint_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_rigid_breakable_joint",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_rigid_breakable_joint.png"
...
def _write_benchmark_json(
    ...
    *,
    ...
    benchmark_name: str = "BM_AvbdRigidBreakableJointStep/1",
) -> Path:
    benchmark = {
        "benchmarks": [
            {
...
def test_avbd_rigid_breakable_joint_packet_records_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert packet["scene"] == "avbd_rigid_breakable_joint"
    ...
    assert packet["visual_scene_invariants"] == {
        ...
        "initial_linear_velocity_m_per_s": [0.65, 0.0, -0.35],
        "joint_anchor_scope": "rigid_body_pair",
        "payload_prestain_m": [0.18, 0.0, -0.08],
        "reset_break_force_n": 1.0e12,
        "rigid_bodies": 3,
        "time_step": 0.004,
    }
    assert packet["visual_capture"]["frames"]["count"] == 2
    assert packet["benchmark"]["benchmark"] == "BM_AvbdRigidBreakableJointStep/1"
    ...
    assert "articulated fixed break/reset packet" in packet["remaining_gates"]


def test_avbd_rigid_breakable_joint_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(
        SystemExit,
        match="capture scene must be avbd_rigid_breakable_joint",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_rigid_breakable_joint_packet_rejects_wrong_arg(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(
        tmp_path,
        benchmark_name="BM_AvbdRigidBreakableJointStep/8",
    )

    with pytest.raises(
        SystemExit,
        match="missing BM_AvbdRigidBreakableJointStep/1",
    ):
        module.main(
            [
                "--capture-manifest",
...
def test_avbd_rigid_breakable_joint_packet_rejects_wrong_joint_count(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
