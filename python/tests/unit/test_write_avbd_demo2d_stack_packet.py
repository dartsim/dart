...
SCRIPT = ROOT / "scripts" / "write_avbd_demo2d_stack_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo2d_stack_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo2d_stack",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo2d_stack.png"
        ...
        "benchmarks": [
            {
                ...
                "cpu_time": cpu_time,
                "iterations": 10,
                "name": "BM_AvbdDemo2dStackStep",
                "real_time": 100.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 0.0,
                "run_name": "BM_AvbdDemo2dStackStep",
                "run_type": "iteration",
                "source_scene_index": 11.0,
                "time_unit": "ns",
            }
        ],
        "context": {
...
def test_avbd_demo2d_stack_packet_records_evidence(tmp_path: Path) -> None:
    ...
    assert packet["scene"] == "avbd_demo2d_stack"
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo2dStackStep"
    ...
    assert packet["reference_timing"]["invariants"]["dynamic_bodies"] == 20
    assert packet["reference_timing"]["invariants"]["static_bodies"] == 1
    assert packet["reference_timing"]["invariants"]["stack_box_count"] == 20
...
def test_avbd_demo2d_stack_packet_rejects_wrong_scene(tmp_path: Path) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path, scene="avbd_demo2d_motor")
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path)

    with pytest.raises(SystemExit, match="capture scene must be avbd_demo2d_stack"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_stack_packet_rejects_wrong_counts(tmp_path: Path) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
...
def test_avbd_demo2d_stack_packet_rejects_wrong_reference(tmp_path: Path) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path, stack_box_count=19)

    with pytest.raises(SystemExit, match="expected stack_box_count=20"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_stack_packet_uses_benchmark_median(tmp_path: Path) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    ...
    median_row.update(
        {
            ...
            "cpu_time": 80.0,
            "name": "BM_AvbdDemo2dStackStep_median",
            "run_type": "aggregate",
        }
    )
    ...
    stddev_row.update(
        {
            ...
            "cpu_time": 5.0,
            "name": "BM_AvbdDemo2dStackStep_stddev",
            "rigid_bodies": 0.0,
            "rigid_body_joints": 0.0,
            "run_type": "aggregate",
