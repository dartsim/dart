...
SCRIPT = ROOT / "scripts" / "write_avbd_demo2d_fracture_packet.py"


def _load_packet_module():
    spec = importlib.util.spec_from_file_location(
        "write_avbd_demo2d_fracture_packet",
        SCRIPT,
    )
...
def _write_capture_manifest(
    tmp_path: Path,
    *,
    scene: str = "avbd_demo2d_fracture",
) -> Path:
    capture = tmp_path / "capture"
    frames = capture / "png_frames"
    frames.mkdir(parents=True)
    screenshot = capture / "avbd_demo2d_fracture.png"
        ...
        "benchmarks": [
            {
                ...
                "collision_shapes": 29.0,
                "cpu_time": cpu_time,
                "iterations": 10,
                "joint_connected_collision_pairs": 10.0,
                "name": "BM_AvbdDemo2dFractureStep",
                "real_time": 125.0,
                "rigid_bodies": rigid_bodies,
                "rigid_body_joints": 10.0,
                "run_name": "BM_AvbdDemo2dFractureStep",
                "run_type": "iteration",
                "source_scene_index": 18.0,
                "time_unit": "ns",
            }
        ],
        "context": {
...
def test_avbd_demo2d_fracture_packet_records_evidence(tmp_path: Path) -> None:
    ...
    assert packet["scene"] == "avbd_demo2d_fracture"
    assert packet["source_demo_reference_row"]["scene_name"] == "Fracture"
    assert packet["source_demo_reference_row"]["source_shapes"]["chain"] == {
        "count": 11,
        "density": 1.0,
    ...
    assert packet["source_demo_reference_row"]["source_shapes"]["support"] == {
        "count": 2,
        "density": 1.0,
        "friction": 0.5,
    ...
    assert packet["source_demo_reference_row"]["source_shapes"]["falling_block"] == {
        "count": 15,
        "density": 1.0,
    ...
    assert packet["dart_benchmark"]["benchmark"] == "BM_AvbdDemo2dFractureStep"
    ...
    assert packet["reference_timing"]["scene_builder"] == "sceneFracture"
    assert packet["reference_timing"]["invariants"]["dynamic_bodies"] == 28
    assert packet["reference_timing"]["invariants"]["static_bodies"] == 1
    assert packet["reference_timing"]["invariants"]["breakable_joints"] == 10
    assert (
        packet["reference_timing"]["invariants"]["joint_connected_collision_pairs"]
        == 10
    )
    assert packet["reference_timing"]["invariants"]["chain_links"] == 11
    assert packet["reference_timing"]["invariants"]["falling_blocks"] == 15
    assert packet["reference_timing"]["invariants"]["support_blocks"] == 2
    assert packet["reference_timing"]["invariants"]["break_force"] == 500.0
...
def test_avbd_demo2d_fracture_packet_rejects_wrong_scene(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    ...
    with pytest.raises(SystemExit, match="capture scene must be avbd_demo2d_fracture"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_fracture_packet_rejects_wrong_counts(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path, rigid_bodies=28.0)
    reference_json = _write_reference_timing_json(tmp_path)

    with pytest.raises(SystemExit, match="expected rigid_bodies=29"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_fracture_packet_rejects_wrong_reference(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    benchmark_json = _write_benchmark_json(tmp_path)
    reference_json = _write_reference_timing_json(tmp_path, breakable_joints=9)

    with pytest.raises(SystemExit, match="expected breakable_joints=10"):
        module.main(
            [
                "--capture-manifest",
                str(capture_manifest),
...
def test_avbd_demo2d_fracture_packet_uses_benchmark_median(
    tmp_path: Path,
) -> None:
    module = _load_packet_module()
    capture_manifest = _write_capture_manifest(tmp_path)
    ...
    median_row.update(
        {
            ...
            "cpu_time": 110.0,
            "name": "BM_AvbdDemo2dFractureStep_median",
            "run_type": "aggregate",
        }
    )
    ...
    stddev_row.update(
        {
            ...
            "cpu_time": 5.0,
            "name": "BM_AvbdDemo2dFractureStep_stddev",
            "rigid_bodies": 0.0,
