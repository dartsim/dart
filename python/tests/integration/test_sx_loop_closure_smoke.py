"""Headless smokes for the sx loop-closure showcase and the bridge drag damping.

1. Build the ``sx_loop_closure`` scene, step the underlying sx::World ~100 times
...
2. Exercise the SxRenderBridge force-drag velocity-damping path once: a moving
   sx RigidBody under an active drag gets ``F_spring - kd*v`` applied (not just
   the spring term), so the dragged force opposes the body's motion.
"""
...
def test_sx_loop_closure_holds_under_vi() -> None:
    _sx()  # skip cleanly if experimental is unavailable
    from examples.demos.scenes.sx_loop_closure import build
    ...
    # Step the sx physics ~100 times (the scene's pre_step advances the sx
    # world and re-syncs the render mirror).
    max_residual = 0.0
    for _ in range(100):
        setup.pre_step()
...
def test_bridge_force_drag_applies_velocity_damping() -> None:
    ...
    bridge = SxRenderBridge(world, name="drag_damping_smoke")
    ...
    expected_damped = spring - SxRenderBridge._DRAG_DAMPING_KD * np.array(
        [1.0, 0.0, 0.0]
    )
