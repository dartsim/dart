...
void defGridVisual(nb::module_& m)
{
  ...
  auto grid = nb::class_<GridVisual, dart::gui::osg::ViewerAttachment>(
                  m, "GridVisual")
                  .def(
                      nb::new_([]() { return makeOsgShared<GridVisual>(); }))
                  .def("setNumCells", &GridVisual::setNumCells)
                  .def("setMinorLineStepSize", &GridVisual::setMinorLineStepSize)
                  .def("setNumMinorLinesPerMajorLine",
...
void defPolyhedronVisual(nb::module_& m)
{
  ...
  nb::class_<PolyhedronVisual, dart::gui::osg::ViewerAttachment>(m, "PolyhedronVisual")
      .def(nb::new_([]() { return makeOsgShared<PolyhedronVisual>(); }))
      .def(
          "setVertices",
          [](PolyhedronVisual& self, const std::vector<Eigen::Vector3d>& vertices) {
            self.setVertices(vertices);
          })
      .def(
          "setVerticesMatrix",
          [](PolyhedronVisual& self, const Eigen::Ref<const Eigen::MatrixXd>& vertices) {
            self.setVertices(vertices);
          },
          nb::arg("vertices"))
