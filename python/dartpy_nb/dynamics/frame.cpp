...
void defFrame(nb::module_& m)
{
  ...
  nb::class_<Frame, std::shared_ptr<Frame>>(m, "Frame")
      .def("getRelativeTransform", &Frame::getRelativeTransform, nb::rv_policy::reference_internal)
      .def("getWorldTransform", &Frame::getWorldTransform, nb::rv_policy::reference_internal)
      .def("getTransform", [](const Frame& self) { return self.getTransform(); })
      .def("getTransform",
      ...
      .def("getTransform",
          [](const Frame& self, const Frame* withRespectTo, const Frame* inCoordinatesOf) {
            const Frame* wrt = withRespectTo ? withRespectTo : Frame::World();
            const Frame* coords = inCoordinatesOf ? inCoordinatesOf : Frame::World();
