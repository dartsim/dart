...
def main():
    ...
    frame = dart.gui.InteractiveFrame(
        dart.Frame.World(), "interactive frame", tf, 2
    )
    world.addSimpleFrame(frame)

    tf.set_translation([-4, 4, 0])
    draggable = dart.SimpleFrame(frame, "draggable", tf)
    draggable.setShape(dart.BoxShape([1, 1, 1]))
    draggable.getVisualAspect(True).setColor([0.9, 0, 0])
    world.addSimpleFrame(draggable)

    tf.set_translation([8, 0, 0])
    x_marker = dart.SimpleFrame(dart.Frame.World(), "X", tf)
    x_shape = dart.BoxShape([0.2, 0.2, 0.2])
    ...
    y_marker = dart.SimpleFrame(dart.Frame.World(), "Y", tf)
    y_shape = dart.BoxShape([0.2, 0.2, 0.2])
    ...
    z_marker = dart.SimpleFrame(dart.Frame.World(), "Z", tf)
    z_shape = dart.BoxShape([0.2, 0.2, 0.2])
