"""AVBD port of the avbd-demo3d Stack Ratio source scene."""
...
_BOX_COUNT = 4
_GROUND_SIZE = np.array([100.0, 100.0, _GROUND_THICKNESS])
_SOURCE_ROW: dict[str, Any] = {
    ...
    "source_shapes": {
        ...
        "boxes": {
            ...
            "initial_size": 1.0,
            "size_multiplier": 2.0,
            "sizes": ((1.0, 1.0, 1.0), (2.0, 2.0, 2.0), (4.0, 4.0, 4.0), (8.0, 8.0, 8.0)),
...
def _source_box_specs() -> list[tuple[np.ndarray, np.ndarray]]:
    specs: list[tuple[np.ndarray, np.ndarray]] = []
    top_z = _GROUND_THICKNESS * 0.5
    size = 1.0
    for _ in range(_BOX_COUNT):
        half = size * 0.5
        center_z = top_z + half
        specs.append((np.array([size, size, size]), np.array([0.0, 0.0, center_z])))
        top_z = center_z + half
        size *= 2.0
    return specs
...
def build() -> SceneSetup:
    ...
    ground = _add_source_box(
        world,
        "avbd_demo3d_stack_ratio_ground",
        size=_GROUND_SIZE,
        density=0.0,
        friction=0.5,
        position=np.zeros(3),
    ...
    for index, (size, position) in enumerate(_source_box_specs()):
        boxes.append(
            _add_source_box(
                world,
                f"avbd_demo3d_stack_ratio_box_{index}",
                size=size,
                density=1.0,
                friction=0.5,
                position=position,
    ...
    bridge = WorldRenderBridge(world, name="avbd_demo3d_stack_ratio_render")
    bridge.add_rigid_body_visual(
        ground,
        dart.BoxShape(_GROUND_SIZE),
        (0.40, 0.41, 0.43),
        name="avbd_demo3d_stack_ratio_ground_visual",
    )
    for index, (box, size) in enumerate(zip(boxes, box_sizes, strict=True)):
        ...
        bridge.add_rigid_body_visual(
            box,
            dart.BoxShape(size),
            (0.70 - 0.28 * blend, 0.46 + 0.22 * blend, 0.33 + 0.30 * blend),
            name=f"avbd_demo3d_stack_ratio_box_{index}_visual",
        )
    bridge.sync()
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("source corpus: avbd-demo3d Stack Ratio")
        builder.text("source scene: sceneStackRatio, index 10 of 14")
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Demo3D Stack Ratio", build_panel)],
        info={
            "sx_world": world,
            "ground": ground,
            "boxes": boxes,
            "box_sizes": box_sizes,
            "source_demo_row": "avbd-demo3d stack ratio",
...
SCENE = PythonDemoScene(
    id="avbd_demo3d_stack_ratio",
    title="AVBD Demo3D Stack Ratio (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A source-demo Stack Ratio row port with four size-ratio boxes.",
    build=build,
)
