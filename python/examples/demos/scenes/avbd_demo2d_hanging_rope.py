"""AVBD port of the avbd-demo2d Hanging Rope source scene."""
...
_THICKNESS = 0.2
_REGULAR_SIZE_2D = np.array([0.5, 1.0])
_REGULAR_SIZE = np.array([0.5, 1.0, _THICKNESS])
_HEAVY_SIZE = 10.0
...
_SOURCE_ROW: dict[str, Any] = {
    ...
    "source_constraints": {
        "linear_point_joints": {
            ...
            "heavy_child_anchor": (0.0, _HEAVY_SIZE * 0.5),
...
def _source_link_size(index: int) -> np.ndarray:
    return _HEAVY_BLOCK_SIZE if index == _ROPE_LINKS - 1 else _REGULAR_SIZE


def _source_link_size_2d(index: int) -> np.ndarray:
    return _HEAVY_BLOCK_SIZE_2D if index == _ROPE_LINKS - 1 else _REGULAR_SIZE_2D
...
def build() -> SceneSetup:
    ...
    for index in range(_ROPE_LINKS):
        ...
        links.append(
            _add_source_box(
                world,
                f"avbd_demo2d_hanging_rope_link_{index:02d}",
                size=size,
                size_2d=size_2d,
    ...
    for index, (link, size) in enumerate(zip(links, link_sizes)):
        ...
        bridge.add_rigid_body_visual(
            ...
            dart.BoxShape(size),
            color,
            name=f"avbd_demo2d_hanging_rope_link_{index:02d}_visual",
        )
    bridge.sync()
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("source corpus: avbd-demo2d Hanging Rope")
        builder.text("source scene: sceneHangingRope, index 8 of 19")
        builder.text(f"rigid bodies: {world.num_rigid_bodies}")
        builder.text(f"linear point joints: {len(joints)}")
        builder.text(f"heavy block dy: {tail_y - initial_tail_y:.3f} m")
        builder.text(f"max endpoint error: {endpoint_error:.4f} m")
        builder.text(f"world time: {world.time:.3f} s")
        builder.plot_lines("Heavy block y", list(tail_y_history))
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Demo2D Hanging Rope", build_panel)],
        info={
            "sx_world": world,
            "links": links,
            "link_sizes": link_sizes,
            "joints": joints,
            "source_demo_row": "avbd-demo2d hanging rope",
...
SCENE = PythonDemoScene(
    id="avbd_demo2d_hanging_rope",
    title="AVBD Demo2D Hanging Rope (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A source-demo Hanging Rope row port with a large vertical endpoint block.",
    build=build,
)
