...
gravity on the experimental World with the **variational integrator** selected
...
def build() -> SceneSetup:
    ...
    bridge = SxRenderBridge(world, name="sx_loop_closure_render")
    palette = [
        (0.90, 0.30, 0.24),
        (0.95, 0.61, 0.16),
        (0.30, 0.69, 0.31),
    ...
    # render frame (not bridged to any sx body) so the bridge's per-step sync
