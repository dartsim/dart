"""AVBD same-multibody articulated breakable prismatic-motor scene."""
...
def build() -> SceneSetup:
    ...
    base = slider.add_link("avbd_articulated_prismatic_pair_breakable_base")
    carriage = slider.add_link(
        "avbd_articulated_prismatic_pair_breakable_carriage",
        parent=base,
        joint=sx.JointSpec(
            name="avbd_articulated_prismatic_pair_breakable_floating_carriage",
    ...
    motor_joint = world.add_joint(
        ...
        carriage,
                sx.JointSpec(
            name="avbd_articulated_prismatic_pair_breakable_axis",
    ...
    bridge = WorldRenderBridge(
        world, name="avbd_articulated_prismatic_pair_breakable_motor_render"
    )
    bridge.add_link_visual(
        base,
        dart.BoxShape(_full(_BASE_HALF)),
        (0.26, 0.29, 0.34),
        name="avbd_articulated_prismatic_pair_breakable_base_visual",
    )
    bridge.add_link_visual(
        carriage,
        dart.BoxShape(_full(_CARRIAGE_HALF)),
        (0.25, 0.66, 0.92),
        name="avbd_articulated_prismatic_pair_breakable_carriage_visual",
    )

    rail = dart.SimpleFrame(
        dart.gui.world_render_frame(),
        "avbd_articulated_prismatic_pair_breakable_rail_visual",
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("solver: AVBD same-multibody prismatic motor break/reset")
    ...
    return SceneSetup(
        ...
        panels=[
            ScenePanel(
                "AVBD Articulated Pair Prismatic Breakable Motor", build_panel
            )
        ],
        info={
            "sx_world": world,
...
SCENE = PythonDemoScene(
    id="avbd_articulated_prismatic_pair_motor_breakable_joint",
    title="AVBD Articulated Pair Prismatic Breakable Motor (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A public same-multibody articulated prismatic velocity motor "
    "breaks, releases, and re-engages through the AVBD variational bridge.",
    build=build,
)
