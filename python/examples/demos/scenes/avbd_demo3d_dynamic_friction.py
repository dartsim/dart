"""AVBD port of the avbd-demo3d Dynamic Friction source scene."""
...
def _source_box_position(index: int) -> np.ndarray:
    return np.array([0.0, -30.0 + 2.0 * float(index), 0.75])
...
def build() -> SceneSetup:
    ...
    ground = _add_source_box(
        world,
        "avbd_demo3d_dynamic_friction_ground",
        size=_GROUND_SIZE,
        density=0.0,
        friction=_GROUND_FRICTION,
        position=np.zeros(3),
    ...
    for index in range(_BOX_COUNT):
        boxes.append(
            _add_source_box(
                world,
                f"avbd_demo3d_dynamic_friction_box_{index}",
                size=_BOX_SIZE,
                density=1.0,
    ...
    bridge = WorldRenderBridge(world, name="avbd_demo3d_dynamic_friction_render")
    bridge.add_rigid_body_visual(
        ground,
        dart.BoxShape(_GROUND_SIZE),
        (0.42, 0.43, 0.45),
        name="avbd_demo3d_dynamic_friction_ground_visual",
    )
    for index, box in enumerate(boxes):
        ...
        bridge.add_rigid_body_visual(
            ...
            color,
            name=f"avbd_demo3d_dynamic_friction_box_{index}_visual",
        )
    bridge.sync()
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("source corpus: avbd-demo3d Dynamic Friction")
        builder.text("source scene: sceneDynamicFriction, index 2 of 14")
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Demo3D Dynamic Friction", build_panel)],
        info={
            "sx_world": world,
            "ground": ground,
            "boxes": boxes,
            "source_demo_row": "avbd-demo3d dynamic friction",
...
SCENE = PythonDemoScene(
    id="avbd_demo3d_dynamic_friction",
    title="AVBD Demo3D Dynamic Friction (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A source-demo Dynamic Friction row port with sliding rigid boxes "
