...
OUTCOME_ORACLE: dict[str, Any] = {
    ...
    "displacement_threshold": _DISPLACEMENT_THRESHOLD,
    "impact_band_radius": _IMPACT_BAND_RADIUS,
    "outside_radius": _OUTSIDE_RADIUS,
    "minimum_displaced_bricks_per_impact_band": 4,
    "minimum_outside_retained_fraction": 0.80,
    "minimum_total_retained_fraction": 0.70,
...
def compute_outcome_metrics(
    *,
        ...
        impact_band_displaced_counts.append(
            int(
                np.count_nonzero(
                    ...
                    & (displacements > _DISPLACEMENT_THRESHOLD)
                )
            )
        )
    ...
    retained_mask = displacements < _DISPLACEMENT_THRESHOLD
...
def build_solver_variant(
    *,
    ...
    capture_focus = tuple(
        f"{brick_rows[row][column].name}_visual"
        for row, column in (
            (0, 0),
            (0, _WALL_COLUMNS - 1),
            (_WALL_ROWS - 1, 0),
            (_WALL_ROWS - 1, _WALL_COLUMNS - 1),
        )
    )
