"""AVBD articulated spherical breakable-joint scene for experimental World."""
...
def _world_anchor(link: sx.Link, child_anchor: np.ndarray) -> np.ndarray:
    return np.asarray(link.translation, dtype=float).reshape(3) + (
        np.asarray(link.rotation, dtype=float) @ child_anchor
    )
...
def build() -> SceneSetup:
    ...
    base = arm.add_link("avbd_articulated_spherical_breakable_base")
    payload = arm.add_link(
        "avbd_articulated_spherical_breakable_payload",
        parent=base,
        joint=sx.JointSpec(
            name="avbd_articulated_spherical_breakable_floating_payload",
    ...
    bridge = WorldRenderBridge(
        world, name="avbd_articulated_spherical_breakable_joint_render"
    )
    bridge.add_link_visual(
        base,
        dart.BoxShape(_full(_BASE_HALF)),
        (0.26, 0.29, 0.34),
        name="avbd_articulated_spherical_breakable_base_visual",
    )
    bridge.add_link_visual(
        payload,
        dart.BoxShape(_full(_PAYLOAD_HALF)),
        (0.88, 0.48, 0.18),
        name="avbd_articulated_spherical_breakable_payload_visual",
    )

    connector = dart.SimpleFrame(
        dart.gui.world_render_frame(),
        "avbd_articulated_spherical_breakable_connector_visual",
        _connector_transform(world_anchor, _world_anchor(payload, _CHILD_ANCHOR)),
    )
    connector.set_shape(
        dart.BoxShape(
            np.array(
                [
                    max(float(np.linalg.norm(_CAPTURED_POSITION - world_anchor)), 0.08),
                    0.035,
                    0.035,
                ]
            )
    ...
    def payload_anchor() -> np.ndarray:
        return _world_anchor(payload, _CHILD_ANCHOR)

    def sync_connector() -> None:
        connector.set_transform(_connector_transform(world_anchor, payload_anchor()))
    ...
    def build_panel(builder: object, context: object) -> None:
        anchor_residual = float(np.linalg.norm(payload_anchor() - world_anchor))
        ...
        builder.text("solver: AVBD world-anchored articulated spherical point joint")
    ...
    return SceneSetup(
        ...
        panels=[
            ScenePanel("AVBD Articulated Spherical Breakable Joint", build_panel)
        ],
        info={
            "sx_world": world,
...
SCENE = PythonDemoScene(
    id="avbd_articulated_spherical_breakable_joint",
    title="AVBD Articulated Spherical Breakable Joint (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A public world-anchored articulated spherical point joint breaks, "
