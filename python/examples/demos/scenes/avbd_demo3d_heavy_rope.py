"""AVBD port of the avbd-demo3d Heavy Rope source scene."""
...
_HEAVY_SIZE = 5.0
_HEAVY_BLOCK_SIZE = np.array([_HEAVY_SIZE, _HEAVY_SIZE, _HEAVY_SIZE])
_FRICTION = 0.5
...
_SOURCE_ROW: dict[str, Any] = {
    ...
    "source_shapes": {
        ...
        "heavy_block": {
            "count": 1,
            "size": tuple(_HEAVY_BLOCK_SIZE),
            "density": 1.0,
            "friction": _FRICTION,
            "position": (19.0 + _HEAVY_SIZE * 0.5, 0.0, 10.0),
        },
    },
    "source_constraints": {
        "linear_point_joints": {
            ...
            "regular_child_anchor": tuple(_REGULAR_CHILD_ANCHOR),
            "heavy_child_anchor": tuple(_HEAVY_CHILD_ANCHOR),
...
def build() -> SceneSetup:
    ...
    for index in range(_ROPE_LINKS):
        ...
        links.append(
            _add_source_box(
                world,
                f"avbd_demo3d_heavy_rope_link_{index:02d}",
                size=size,
    ...
    for index, (parent, child) in enumerate(zip(links, links[1:])):
        joints.append(
            world.add_joint(
                ...
                child,
                                sx.JointSpec(
                    name=f"avbd_demo3d_heavy_rope_point_joint_{index:02d}",
    ...
    bridge = WorldRenderBridge(world, name="avbd_demo3d_heavy_rope_render")
    bridge.add_rigid_body_visual(
        ground,
        dart.BoxShape(_GROUND_SIZE),
        (0.40, 0.41, 0.43),
        name="avbd_demo3d_heavy_rope_ground_visual",
    )
    for index, (link, size) in enumerate(zip(links, link_sizes)):
        if index == 0:
            ...
            color = (0.23, 0.58, 0.78)
        bridge.add_rigid_body_visual(
            link,
            dart.BoxShape(size),
            color,
            name=f"avbd_demo3d_heavy_rope_link_{index:02d}_visual",
        )
    bridge.sync()
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("source corpus: avbd-demo3d Heavy Rope")
        builder.text("source scene: sceneHeavyRope, index 6 of 14")
        builder.text(f"rigid bodies: {world.num_rigid_bodies}")
        builder.text(f"linear point joints: {len(joints)}")
        builder.text(f"heavy block dz: {tail_z - initial_tail_z:.3f} m")
        builder.text(f"max endpoint error: {endpoint_error:.4f} m")
        builder.text(f"world time: {world.time:.3f} s")
        builder.plot_lines("Heavy block z", list(tail_z_history))
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Demo3D Heavy Rope", build_panel)],
        info={
            "sx_world": world,
            "ground": ground,
            "links": links,
            ...
            "joints": joints,
            "source_demo_row": "avbd-demo3d heavy rope",
...
SCENE = PythonDemoScene(
    id="avbd_demo3d_heavy_rope",
    title="AVBD Demo3D Heavy Rope (sx)",
