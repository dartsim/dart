"""AVBD articulated breakable-motor scene for experimental World."""
...
def build() -> SceneSetup:
    ...
    base = arm.add_link("avbd_articulated_breakable_motor_base")
    rotor = arm.add_link(
        "avbd_articulated_breakable_motor_rotor",
        parent=base,
        joint=sx.JointSpec(
            name="avbd_articulated_breakable_motor_floating_rotor",
    ...
    motor_joint = world.add_joint(
        ...
        rotor,
                sx.JointSpec(
            name="avbd_articulated_breakable_motor_hinge",
    ...
    bridge = WorldRenderBridge(
        world, name="avbd_articulated_motor_breakable_joint_render"
    )
    bridge.add_link_visual(
        base,
        dart.BoxShape(_full(_HUB_HALF)),
        (0.26, 0.29, 0.34),
        name="avbd_articulated_breakable_motor_base_visual",
    )
    bridge.add_link_visual(
        rotor,
        dart.BoxShape(_full(_ROTOR_HALF)),
        (0.25, 0.66, 0.92),
        name="avbd_articulated_breakable_motor_rotor_visual",
    )

    axis = dart.SimpleFrame(
        dart.gui.world_render_frame(),
        "avbd_articulated_breakable_motor_axis_visual",
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("solver: AVBD articulated revolute motor break/reset")
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Articulated Breakable Motor", build_panel)],
        info={
            "sx_world": world,
            "base": base,
            "rotor": rotor,
...
SCENE = PythonDemoScene(
    id="avbd_articulated_motor_breakable_joint",
    title="AVBD Articulated Breakable Motor (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A public articulated revolute velocity motor breaks, releases, "
    "and re-engages through the AVBD variational bridge.",
    build=build,
)
