"""AVBD port of the avbd-demo2d Stack source scene."""
...
_BOX_SIZE_2D = np.array([1.0, 1.0])
_BOX_SIZE = np.array([1.0, 1.0, _THICKNESS])
...
def _source_box_position(index: int) -> np.ndarray:
    return np.array([0.0, float(index) * 2.0 + 1.0, 0.0])
...
def build() -> SceneSetup:
    ...
    for index in range(_BOX_COUNT):
        boxes.append(
            _add_source_box(
                world,
                f"avbd_demo2d_stack_box_{index}",
                size=_BOX_SIZE,
    ...
    bridge = WorldRenderBridge(world, name="avbd_demo2d_stack_render")
    bridge.add_rigid_body_visual(
        ground,
        dart.BoxShape(_GROUND_SIZE),
        (0.40, 0.41, 0.43),
        name="avbd_demo2d_stack_ground_visual",
    )
    for index, box in enumerate(boxes):
        ...
        bridge.add_rigid_body_visual(
            ...
            name=f"avbd_demo2d_stack_box_{index}_visual",
        )
    bridge.sync()
    ...
    def build_panel(builder: object, context: object) -> None:
        ...
        builder.text("source corpus: avbd-demo2d Stack")
        builder.text("source scene: sceneStack, index 11 of 19")
    ...
    return SceneSetup(
        ...
        panels=[ScenePanel("AVBD Demo2D Stack", build_panel)],
        info={
            "sx_world": world,
            "ground": ground,
            "boxes": boxes,
            "source_demo_row": "avbd-demo2d stack",
...
SCENE = PythonDemoScene(
    id="avbd_demo2d_stack",
    title="AVBD Demo2D Stack (sx)",
    category="AVBD Rigid Constraints (sx)",
    summary="A source-demo Stack row port with twenty 2D boxes over ground.",
    build=build,
)
