...
- Simulation
  ...
  - Added DART 7 `dart::simulation::World` boxed-LCP snapshot coverage for one
    ...
    family), plus 8-sphere rows for `Pgs`, `Jacobi`, `BlockedJacobi`,
    `RedBlackGaussSeidel`, and `NNCG`, and 8-/9-/10-sphere rows for `Pgs`,
    `Jacobi`, `BlockedJacobi`, `RedBlackGaussSeidel`, `NNCG`, and the narrower
    passing 10-solver subset, so
