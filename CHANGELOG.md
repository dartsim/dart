...
skeleton, options)` and dartpy `build_multibody_from_skeleton` /
    ...
    capsule, and cylinder collision shapes are carried onto the links with their
    shape-node offsets preserved (gated by `loadCollisionShapes`). Other
    collision shape types and rotated parent-side offsets on ball/free/planar
    joints are not yet translated (the former is skipped; the latter raises a
    descriptive error).
...
qdot)` that reaches the target exactly even under inertial coupling. The
  ...
  - Added experimental rigid-body collision queries: attach a `CollisionShape`
    (sphere, box, capsule, or cylinder) to a rigid body and call
