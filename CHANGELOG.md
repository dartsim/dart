...
- Simulation
  ...
  - Improved exactly torque-free single-body `FreeJoint` stepping for asymmetric inertia and center-of-mass offsets, preserving kinetic energy and world angular momentum over long free-body simulations while keeping explicit acceleration integration for forced or manually accelerated cases.
  - Added `World::stepSubsteps()` and dartpy bindings to run repeated smaller internal timesteps while preserving the existing `World::step()` command-reset semantics.
  - Added `World::stepUnconstrainedRungeKutta4()` and dartpy bindings for higher-order rigid articulated dynamics in worlds that do not require contact or constraint impulses.
