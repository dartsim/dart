...
- Added a conservative URDF writer for `Skeleton` round-trips that fit URDF's
  ...
  single-DoF mimic metadata, zero-offset coupler mimic transmissions, inertial
  data, local visual/collision poses, primitive geometry, absolute and package
  URI meshes, visual colors, and explicit diagnostics for unsupported or lossy
  DART constructs including soft bodies and invalid mesh resources.
