...
- Collision and Geometry
  - Decoupled FCL, Bullet, and ODE collision backends from `libdart.so` into explicit reference shared libraries (`dart-collision-reference-fcl`, `dart-collision-reference-bullet`, `dart-collision-reference-ode`) while promoting the native DART backend to the default detector (`"dart"` factory key). The legacy `collision-fcl`, `collision-bullet`, and `collision-ode` package components plus installed/source-tree detector headers are native-backed compatibility facades; old engines remain available only through reference targets and `reference/` source paths for tests and benchmarks during the migration.
  - Added collision benchmark regression checks that parse Google Benchmark JSON and compare native collision timings against the best enabled FCL, Bullet, or ODE reference result across narrowphase, distance, raycast, mixed primitive, mesh-heavy, raycast-batch, and public DART adapter scenarios.
  - Made the FCL collision backend optional behind `DART_BUILD_COLLISION_FCL`; core `dart` and `dartpy` now configure and build with FCL, Bullet, and ODE collision backends disabled.
