...
- Collision and Geometry
  ...
  - Scoped `DART_BUILD_COLLISION_FCL`, `DART_BUILD_COLLISION_BULLET`, and
    `DART_BUILD_COLLISION_ODE` to optional `collision-reference-*`
    components; core `dart`, `dartpy`, gz-physics runtime integration, and
    native-backed `collision-fcl`/`collision-bullet`/`collision-ode`
    compatibility facades build with all three disabled.
