...
- Examples and Tutorials
  ...
  - Reoriented the matching Y-up C++ `pixi run demos` scenes (`rigid_chain`, `rigid_cubes`, `mixed_chain`, `add_delete_skels`, `rigid_loop`, `soft_bodies`, `vehicle`, `hybrid_dynamics`, `human_joint_limits`, `rigid_shapes`) to Z-up via a shared `reorientWorldToZUp` helper, keeping the C++ golden-set parity smoke green. `rigid_shapes` also had a Y-up camera and Y-axis spawn heights, now both Z-up. The remaining Y-up scenes whose balance controllers hardcode the up axis (`biped_stand`, `joint_constraints`) and the `lcp_physics` benchmark are left as a follow-up.
