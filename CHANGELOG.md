...
_DART 7 is the clean-break release from the DART 6 API line. This section is
written for release readers: it groups related work by user impact and links to
representative evidence instead of preserving every implementation step. DART 6
compatibility remains on the active DART 6 LTS branch._

#### Release Highlights

- DART 7 raises the baseline to C++23, CMake 4.2.3+, Python 3.14 for dartpy,
  and the current supported compiler set.
- The promoted simulation direction is the ECS-backed
  `dart::simulation::World` pipeline, with Model/State/Control data separated
  so rigid, articulated, deformable, differentiable, and solver-family work can
  share one public facade.
- Native DART collision is now the default direction. FCL, Bullet, and ODE are
  retained as compatibility or benchmark/reference paths instead of normal
  runtime dependencies.
- Filament with GLFW3 and Dear ImGui is the maintained renderer stack, with
  `dart::gui`, `dart-demos`, and Python `py-demos` replacing the old
  OpenSceneGraph/Raylib era.
- dartpy moved to nanobind, a flatter namespace, Python 3.14 wheels, and
  Python-first demos/tutorials.
- The repository now has AI-native operating docs, generated workflow adapters,
  living plans, and verification gates for agent-assisted development.
...
- Existing DART 6 consumers should expect source updates for removed deprecated
  ...
  changes, the nanobind dartpy surface, native collision defaults, and the new
  GUI/demo stack.
- Legacy examples and DART 6 whole-World loading behavior should be checked
  against `release-6.*` branches. DART 7 examples are being rebuilt around
  `dart::simulation::World`, `dart::gui`, `dart-demos`, and `py-demos`.
- Changelog updates for the rest of the DART 7 cycle must follow
  [docs/onboarding/changelog.md](docs/onboarding/changelog.md): one
  reader-visible outcome per bullet, no implementation diary, and links to PRs
  or owner docs for detail.

#### Requirements and Supported Platforms

- Required C++ standard: C++23.
- Minimum source-build CMake: 4.2.3.
- Minimum dartpy Python: 3.14.
- Minimum compilers: GCC 11.0+ on Linux, Clang 12.0+ on macOS, and MSVC 19.50+
  on Windows.
- Tested release platforms are still TBD and will be finalized during release
  packaging.

#### Breaking Changes

- Raised language and build floors to C++23, CMake 4.2.3+, Python 3.14 for
  dartpy, and MSVC 19.50+ on Windows. ([#2068](https://github.com/dartsim/dart/pull/2068),
  [#2995](https://github.com/dartsim/dart/pull/2995),
  [#3005](https://github.com/dartsim/dart/pull/3005),
  [#3010](https://github.com/dartsim/dart/pull/3010))
- Removed the DART 6-deprecated API surface across common, math, dynamics,
  collision, constraint, parser, and utility APIs; use the non-deprecated DART 6
  replacements before moving code to DART 7. ([#2119](https://github.com/dartsim/dart/pull/2119),
  [#2122](https://github.com/dartsim/dart/pull/2122),
  [#2132](https://github.com/dartsim/dart/pull/2132),
  [#2140](https://github.com/dartsim/dart/pull/2140),
  [#2143](https://github.com/dartsim/dart/pull/2143))
- Renamed installed C++ headers to snake_case and standardized component
  umbrella headers around `All.hpp` plus `dart/all.hpp`; PascalCase wrappers are
...
- Removed GLUT, OpenSceneGraph, Raylib, legacy GUI examples/tutorials, and legacy
  ...
  [#2209](https://github.com/dartsim/dart/pull/2209),
  [#2257](https://github.com/dartsim/dart/pull/2257))
- Moved FCL, Bullet, and ODE out of the normal runtime dependency path. Native
  DART collision is the default detector; old engine names are compatibility or
  reference-test surfaces during migration.
- Removed the legacy integration module, optional pagmo optimizer APIs, old
  config/build-mode macros, `dart/common/filesystem.hpp`, and C3D/VSK utilities.
  ([#2201](https://github.com/dartsim/dart/pull/2201),
  [#2204](https://github.com/dartsim/dart/pull/2204),
  [#2275](https://github.com/dartsim/dart/pull/2275),
  [#2363](https://github.com/dartsim/dart/pull/2363))
- Removed the VBD-specific worker-thread option; deformable parallelism now
  comes from the `ComputeExecutor` passed to `World::step()`.
  ([#3029](https://github.com/dartsim/dart/pull/3029))

#### Simulation and Solvers

- Introduced the ECS-backed experimental `World` as the DART 7 simulation
  facade, with separated Model, State, and Control data for clean solver and
  binding boundaries. ([#3026](https://github.com/dartsim/dart/pull/3026),
  [#3027](https://github.com/dartsim/dart/pull/3027))
...
- Promoted the native DART collision backend to the default detector, with FCL,
  Bullet, and ODE retained as compatibility facades or explicit reference
  targets.
- Added native narrow-phase, broad-phase, distance, raycast, continuous collision
  detection, convex/sphere/capsule casts, and spline casts needed by DART 7
  rigid/deformable solvers.
- Improved native mesh, primitive, voxel-grid, heightmap, SDF, and GLTF material
  handling, including sparse voxel storage without requiring OctoMap.
...
- Reworked dartpy packaging around `pyproject.toml`, Pixi wheel builds, Windows
  ...
  [#2266](https://github.com/dartsim/dart/pull/2266),
  [#2268](https://github.com/dartsim/dart/pull/2268))
...
- Consolidated GUI examples into `dart-demos` and Python demos into `py-demos`,
  with scene selection, headless smoke/capture modes, and clearer migration
  placeholders for DART 6 examples that still need World-native ports.
...
- Upgraded Gazebo integration to gz-physics9 and restored DART 7 compatibility,
  including mimic/coupler constraints and downstream crash/regression fixes.
  ([#2093](https://github.com/dartsim/dart/pull/2093),
  [#2315](https://github.com/dartsim/dart/pull/2315),
  [#2320](https://github.com/dartsim/dart/pull/2320))

#### Core Libraries and APIs

- Added core math/runtime foundations for DART 7, including SIMD abstractions,
...
- Upgraded the AI-native contributor infrastructure for current Codex and
  GPT-5.6 with task-shaped model/reasoning routing, a reusable controlled
  model-upgrade audit, raw prompt/context/model-pin diagnostics, leaner
  autonomous prompts, manifest-owned skills, bounded project agents and hooks,
  deterministic branch-aware drift/scenario gates, and a text-first plus
  claim-tied visual verification route, while preserving Claude Code and
  OpenCode adapters.
