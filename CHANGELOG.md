...
- Tooling and Docs
  ...
  - Added a DART-owned performance dashboard pipeline that normalizes Google
    Benchmark JSON, renders Pages-ready benchmark history, links a Read the
    Docs dashboard guide, and keeps optional Bencher reporting behind
    maintainer-provided configuration.
...
- Simulation
  ...
  - Added experimental C++ `World::hasMultiBody()` and dartpy
    `world.has_multi_body()` so world-owned multibody lookup has the same
