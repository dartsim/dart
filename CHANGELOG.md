...
- Collision and Geometry
  - Decoupled FCL, Bullet, and ODE collision backends from `libdart.so` into separate shared libraries (`dart-collision-fcl`, `dart-collision-bullet`, `dart-collision-ode`) while promoting the native DART backend to the default detector (`"dart"` factory key). The legacy backends remain available for tests, benchmarks, and Gazebo compatibility during the migration.
