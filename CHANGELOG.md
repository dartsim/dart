...
skeleton, options)` and dartpy `build_multibody_from_skeleton` /
    ...
    capsule, cylinder, plane, triangular `MeshShape`, and `ConvexMeshShape`
    collision shapes are carried onto the links with their shape-node offsets
    preserved (gated by `loadCollisionShapes`). Other collision shape types and
    rotated parent-side offsets on
