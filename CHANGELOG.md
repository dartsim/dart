...
- Simulation
  ...
  - Fixed boxed semi-smooth Newton LCP solves with friction-index moving
    ...
    1x-/4x-/8x-/16x-coupled friction-index
    6-/8-/12-/16-/24-/32-/48-/64-/96-contact packets, plus an
    ...
    friction-index 16-, 24-, 32-, 48-, 64-, and 96-contact packets.
  - Added an opt-in CPU worker-thread update path for the DART 7 projected
    ...
    serial-vs-worker benchmark rows up to 2048 banded rows. The local benchmark
  ...
  - Added focused DART 7 PGS/PSOR, symmetric PSOR, and Red-Black Gauss-Seidel
    relaxation sweep benchmark rows for standard, boxed, and friction-index
    fixtures at relaxation 0.5, 1.0, and 1.3, verified in default,
  ...
  - Added manifest-generated serial and DART 7 `ParallelExecutor` batch LCP
    ...
    and CUDA-enabled build trees, plus 80 production active-set transition rows
    for 24-contact/72-row, 32-contact/96-row, 48-contact/144-row, and
    64-contact/192-row, and 96-contact/288-row coupled friction-index packets verified across default,
    SIMD-enabled, and CUDA-enabled build trees, plus 454 production active-set transition batch
    ...
    8-/12-/16-/24-/32-/48-/64-/96-contact active-set packets verified across default,
    SIMD-enabled, and CUDA-enabled build trees, plus
    larger mildly ill-conditioned generated correctness coverage and 584
    benchmark rows for standard 32-row,
    boxed 16-row, friction-index 8-contact, and 1x-/4x-/8x-/16x-coupled
    6-/8-/12-/16-/24-/32-/48-/64-/96-contact packets, with Boxed
    ...
    1168 larger mildly ill-conditioned batch rows for batch-size-4 serial and
    ...
    trees, plus 42 extreme exact rank-deficient singular-degenerate rows for
    standard 128-row, boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-/96-contact
    packets verified across default, SIMD-enabled, and CUDA-enabled build
    trees, plus 54 exact rank-deficient singular-degenerate friction-index
    batch rows for batch-size-4 serial and DART 7 `ParallelExecutor` runs over
    coupled friction-index 6-/8-/12-/16-/24-/32-/48-/64-/96-contact packets verified
  ...
  - Added DART 7 experimental CUDA LCP execution paths for fixed-iteration
    ...
    matching CPU serial and DART 7 `ParallelExecutor` Jacobi/PGS batch rows at
    those 24-/48-/96-row and 8-/16-/32-contact direct CUDA packet sizes,
  ...
  - Added DART 7 generated correctness and single/batch benchmark coverage for
    1x-/4x-/8x-/16x-coupled mildly ill-conditioned friction-index LCP packets
    through 96 contacts across the scoped iterative solver set plus Boxed
    Semi-Smooth Newton. Boxed Semi-Smooth Newton reports tuned line-search
    settings on the 16x rows.
    Backend build-state counters distinguish default, SIMD-enabled, and
    CUDA-enabled CPU solver runs.
