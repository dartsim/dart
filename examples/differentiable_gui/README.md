...
`dart::simulation::diff::rollout` plus
...
This example only builds when the GUI, the DART 7 World stack, and the opt-in
differentiable build are all available (`DART_BUILD_GUI` and `DART_BUILD_DIFF`).

Run the interactive demo:

```bash
DART_BUILD_DIFF_OVERRIDE=ON pixi run ex differentiable_gui
```
...
DART_BUILD_DIFF_OVERRIDE=ON pixi run ex differentiable_gui -- \
  ...
  --screenshot /tmp/differentiable_gui.ppm
```
