...
- Controls: press 1-4 or click a target handle to select it; left-drag a gizmo
  axis arrow to move it or a rotation ring to orient it; Ctrl-left-drag a
  selected target handle to move it; keyboard nudges also move the selected
  handle; WASD moves the root; Q/E yaw; F/Z adjusts height; space toggles
  simulation.
...
- The transform gizmos currently support X/Y/Z axis-arrow dragging and rotation
  ring dragging. Plane handles remain follow-up public API work.
