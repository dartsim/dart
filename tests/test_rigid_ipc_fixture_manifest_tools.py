...
def test_rigid_ipc_manifest_marks_rolling_cone_row_implemented(tmp_path):
    module = _load_script("generate_rigid_ipc_fixture_manifest")
    data_path = tmp_path / "fixtures" / "3D" / "friction" / "rolling" / "cone.json"
    data_path.parent.mkdir(parents=True)
    data_path.write_text('{"rigid_body_problem":{"coefficient_friction":0.5}}')
    ...
    assert row["status"] == "implemented"
    assert "RollingConeFixtureRowAdvancesWithContact" in row["dart_artifact"]
    assert "rolling-cone row" in row["expected_invariant"]
...
def test_rigid_ipc_manifest_marks_card_tent_row_implemented(tmp_path):
    module = _load_script("generate_rigid_ipc_fixture_manifest")
    data_path = (
        tmp_path / "fixtures" / "3D" / "friction" / "card-house" / "card-tent.json"
    )
    data_path.parent.mkdir(parents=True)
    data_path.write_text('{"rigid_body_problem":{"coefficient_friction":0.5}}')
    ...
    assert row["status"] == "implemented"
    assert "CardTentFixtureRowStaysUprightWithFriction" in row["dart_artifact"]
    assert "card-tent row" in row["expected_invariant"]
    assert "Larger card-house rows remain planned" in row["notes_or_gap"]
...
def test_rigid_ipc_manifest_marks_erleben_spike_in_crack_row_implemented(
    tmp_path,
):
    module = _load_script("generate_rigid_ipc_fixture_manifest")
    data_path = (
        tmp_path / "fixtures" / "3D" / "unit-tests" / "erleben" / "spike-in-crack.json"
    )
    data_path.parent.mkdir(parents=True)
    data_path.write_text('{"rigid_body_problem":{"rigid_bodies":[]}}')
    ...
    assert row["status"] == "implemented"
    assert "ErlebenSpikeInCrackFixtureRowStaysSeparated" in row["dart_artifact"]
    assert "Erleben spike-in-crack row" in row["expected_invariant"]
    assert "hole row remains planned" in row["notes_or_gap"]


def test_rigid_ipc_manifest_marks_erleben_wedge_in_crack_row_implemented(
    tmp_path,
):
    module = _load_script("generate_rigid_ipc_fixture_manifest")
    data_path = (
        tmp_path / "fixtures" / "3D" / "unit-tests" / "erleben" / "wedge-in-crack.json"
    )
    data_path.parent.mkdir(parents=True)
    data_path.write_text('{"rigid_body_problem":{"rigid_bodies":[]}}')
    ...
    assert row["status"] == "implemented"
    assert "ErlebenWedgeInCrackFixtureRowStaysSeparated" in row["dart_artifact"]
    assert "Erleben wedge-in-crack row" in row["expected_invariant"]
    assert "hole row remains planned" in row["notes_or_gap"]


def test_rigid_ipc_manifest_marks_erleben_spike_in_hole_row_implemented(
    tmp_path,
):
    module = _load_script("generate_rigid_ipc_fixture_manifest")
    data_path = (
        tmp_path / "fixtures" / "3D" / "unit-tests" / "erleben" / "spike-in-hole.json"
    )
    data_path.parent.mkdir(parents=True)
    data_path.write_text('{"rigid_body_problem":{"rigid_bodies":[]}}')
    ...
    assert row["status"] == "implemented"
    assert "ErlebenSpikeInHoleFixtureRowStaysSeparated" in row["dart_artifact"]
    assert "Erleben spike-in-hole row" in row["expected_invariant"]
    assert "retires the audited Erleben" in row["notes_or_gap"]
