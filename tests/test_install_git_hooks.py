...
Covers the fast staged-file wiring added with `pixi run install-hooks`: idempotent
...
def test_foreign_hook_is_preserved_and_chained(tmp_path):
    ...
    # the DART gate is reached (so no dry-run flag is needed here).
    run = subprocess.run(
        [str(hook)],
        cwd=repo,
        env=env,
...
def test_installed_hook_uses_network_free_fallback_in_older_worktree(tmp_path):
    repo, env = _init_repo(tmp_path)
    assert _install(repo, env).returncode == 0
    readme = repo / "README.md"
    readme.write_text("trailing space \n")
    ...
    assert "full agent gate unavailable" in run.stderr
    assert "trailing whitespace" in run.stdout


def test_installed_hook_falls_back_for_python_without_tomllib(tmp_path):
    repo, env = _init_repo(tmp_path)
    assert _install(repo, env).returncode == 0
    _write_gate(repo, "raise SystemExit('must not run')\n")
    readme = repo / "README.md"
...
def test_guard_validates_target_linked_worktree_index(tmp_path):
    repo, env = _init_repo(tmp_path)
    subprocess.run(
        [
            "git",
    ...
    (linked / "README.md").write_text("trailing space \n")
    subprocess.run(["git", "-C", str(linked), "add", "README.md"], check=True, env=env)
    env.update({"CLAUDE_PROJECT_DIR": str(repo)})
    ...
    assert returncode == 2
    assert "staged diff check FAILED" in stderr
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "command -V git commit",
        "time --version git commit -m x",
        "nice --help git commit -m x",
        "nohup --help git commit -m x",
...
def test_guard_routes_git_c_commit_in_linked_worktree(tmp_path):
    repo, env = _init_repo(tmp_path)
    subprocess.run(
        [
            "git",
