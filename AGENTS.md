...
| Architecture                             | `docs/readthedocs/architecture.md` (DART 7 map: framework, step data flow, compute graph, library context), `docs/onboarding/architecture.md` (classic core), `docs/onboarding/README.md`     |
...
- **Commit/PR text**: Do not prefix commit messages or PR titles with agent tags like `[codex]`; use plain descriptive titles. Do not add AI attribution anywhere in commit messages, PR descriptions, or comments: no `Co-Authored-By` AI trailers, no generated-by footers, and no links to agent sessions, even when a tool's default template or a runtime instruction supplies one. See `docs/onboarding/contributing.md`.
...
- **PR pushes**: Before every push to a PR branch, first merge the latest base branch (usually `main`) into it — merge, never rebase a published PR branch. The local merge is routine; the push still needs approval. See `docs/onboarding/ai-reviews.md`.
