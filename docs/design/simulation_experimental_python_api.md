# Simulation Experimental Python API

## Status

Proposal. This document owns durable API-shape rationale for
`dartpy.simulation_experimental`, the Python binding surface for
...
The experimental Python API should make the new simulation stack usable from
...
> The experimental Python API is a small, Pythonic public facade over DART's
...
Compute APIs belong in `dartpy.simulation_experimental.compute` only after the
public wrapper set is deliberate:

```text
dartpy.simulation_experimental.compute
  SequentialExecutor
  ParallelExecutor
  ExecutionProfile
  StageMetadata
...
The current C++ tests allow duplicate multibody names. Dict-style world lookup
...
| Global initialization                | Centralizes backend configuration.                                                                | Avoid global mutable setup for experimental simulation; configure a `World` or future executor object directly.                                |
...
`dartpy.simulation_experimental`. The module can grow while its contracts are
