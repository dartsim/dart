...
The demo diagnostics are compile-time opt-in and default to
`DART_BUILD_DEMOS_MEMORY_DIAGNOSTICS=OFF`; that is the literal zero-runtime-cost
mode because the app contains no panel/session/model object code or calls. In an
enabled build, collection remains runtime opt-in. Runtime-disabled collection
must return before OS probes, allocator walks, ECS/object traversal, snapshot
allocation, address sorting, formatting, or rendering-model construction, but
the enabled build still constructs and renders the panel controls. Therefore
only collection, not the entire diagnostics UI, is zero-work in that mode.
The richer region and ECS vectors are also constructed only by an explicit
World diagnostics query; the always-resident tracker retains fixed counters
only. DART 7's pre-existing frame-scratch counter updates remain part of the
base World step path and are not work introduced by the demo option.
...
| Disabled overhead              | A default `DART_BUILD_DEMOS_MEMORY_DIAGNOSTICS=OFF` `dart-demos` executable contains no demo panel/session/model object code, scene calls, process probes, or platform-specific links. Test builds may compile the isolated library without linking it into the app. In an enabled build, runtime-disabled collection performs no OS probe, World/ECS/object walk, snapshot formatting, or diagnostic history allocation. Existing no-allocation-after-bake gates remain unchanged. |
