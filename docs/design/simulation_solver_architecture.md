...
is assumed. Each supported method needs direct contract evidence; broader
selection and coupled physics follow evidence-led intake. PLAN-040 owns the
sequence in which those capabilities are admitted.
...
| ----------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Free rigid default      | Semi-implicit dynamics and SI contact; boxed-LCP is another contact solve within that family | General asymmetric rotation, friction isotropy and full CUDA contact remain incomplete.                       |
| Articulated integration | Semi-implicit and opt-in variational paths, with rigid/articulated shared constraints        | Qualify each admitted joint/actuator/contact envelope; do not infer O(n) execution from an unused operator.   |
...
add a general search planner, learned tuner or combinatorial optimizer without
a demonstrated scene-selection need.
...
| Physical scope                      | Recommendation                                                                                                | Tradeoffs and evidence required                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Free rigid baseline                 | Semi-implicit translation, correct manifold-aware rotation, hard unilateral SI with circular Coulomb friction | Small implementation and interpretable analytic tests; sequential contact updates are a useful CPU baseline. Finite iterations/order can affect stacking; GPU correctness need not imply single-body speedup. [Solver2D](https://box2d.org/posts/2024/02/solver2d/) explains iterative contact variants; general rotation follows [Euler rigid dynamics](https://www.cs.cmu.edu/afs/cs.cmu.edu/user/baraff/www/pbm/rigid1.pdf). |
| Many-body scaling                   | Optimize collision/islands and measured batching before replacing the physical method                         | Parallel independent islands preserve internal update order. Coloring or Jacobi can expose more GPU work but changes convergence and sometimes the variant; compare at matched physical residuals. Retain an independent small-problem reference.                                                                                                                                                                               |
| Articulated and alternative methods | Qualify existing articulated methods and compare compatible SI/boxed-LCP, VBD/AVBD and barrier variants       | Reuse current implementation investment. [VBD](https://ankachan.github.io/Projects/VertexBlockDescent/index.html) and [AVBD](https://graphics.cs.utah.edu/research/projects/avbd/) offer local-block parallelism and research value, but nonlinear convergence, constraint/friction coverage and paper fidelity need their own evidence.                                                                                        |
| Rigid-solid coupling                | Fixed-corotational tetrahedral solid; compare a shared coupled solve with a partitioned rigid-solid prototype | Clear material/deformation references and a concrete interface test. [IPC](https://ipc-sim.github.io/) offers a strong contact framework but robust geometry, CCD, derivatives and global solves raise complexity; no unconditional first-default choice.                                                                                                                                                                       |
| Cloth and rods                      | Evaluate XPBD for a bounded cloth/rod baseline alongside energy-based methods                                 | [XPBD](https://matthias-research.github.io/pages/publications/XPBD.pdf) is simple and compliance-aware with parallel opportunities, but accuracy/constitutive behavior and convergence must be measured. It does not automatically supply all shell/rod physics.                                                                                                                                                                |
| Fluids and granular models          | Choose fluid/granular formulation from a concrete use case and coupling experiment                            | MPM, SPH and grid formulations have different material, boundary, conservation and hardware tradeoffs. Particle storage alone is not a fluid solver. Defer this choice until a bounded problem and reference exist.                                                                                                                                                                                                             |

Perfect restitution does not imply exact energy conservation for a discretized
trajectory. Validation should separate flight, impact and positional-correction
error and measure timestep convergence. Friction and inelastic contact should dissipate
...
Capability/version identity, complete state ownership, numerical/error
contracts, graph completion and checkpoint semantics apply to each admitted
method. Independent states must stay isolated, public workflows should be
simple, and unsupported combinations should be rejected explicitly. A tiny
nonstepping variable-block test can challenge layout extensibility without
implementing another physics domain.

[PLAN-040](../plans/040-dart7-release-hardening.md) owns milestone assignments,
method sequencing and deferred scope. Public solver plugins/ABI, arbitrary
automatic composition, universal gradients and all-domain checkpoints require
separate architectural decisions. Audit these contracts continuously as
representations, methods and execution modes are added.
