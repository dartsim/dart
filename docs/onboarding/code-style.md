...
- [CMake Style](#cmake-style-summary)
...
- **Core headers (`dart/`)**: Use **snake_case** for headers (e.g., `body_node.hpp`, `collision_group.hpp`). PascalCase compatibility headers are generated by CMake (`dart_generate_case_compat_headers()` and `dart_install_compat_headers()` in `cmake/dart_defs.cmake`) for legacy includes and emit deprecation warnings; case-only wrappers are skipped on case-insensitive filesystems.
...
- **DART 7 simulation modules (`dart/simulation/`, `tests/unit/simulation/`, `tests/benchmark/simulation/`, and dependents)**: Use all **snake_case** file names (e.g., `rigid_body.hpp`, `free_joint.cpp`, `test_multibody.py`). Build-system files such as `CMakeLists.txt` keep their canonical capitalization.
...
- **Header guards**: `#pragma once` (macro guards only in vendored code and generated files)
...
- Use `#pragma once` instead of macro include guards; it cannot collide and
  ...
  headers under tests) and vendored upstream code (`dart/math/lcp/pivoting/dantzig/`,
  `tests/baseline/odelcpsolver/`, `ikfast.h`) are exempt.
...
`.gersemirc` in the repository root is the executable reference;
`pixi run lint-cmake` applies it and `pixi run check-lint-cmake` verifies it.
