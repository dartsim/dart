...
- **File naming**: PascalCase in `dart/` and `python/dartpy/`; snake*case everywhere in `dart/simulation/experimental/` and its dependents (including `tests/unit/simulation/experimental/` and `tests/benchmark/simulation/experimental/`); legacy `tests/` use `test*` + PascalCase while next-gen tests stay fully snake_case
