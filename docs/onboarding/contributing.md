...
- Keep legacy files in `dart/` and `python/dartpy/` using PascalCase names, but use snake_case for anything new in `dart/simulation/next/` and its related test/example directories
