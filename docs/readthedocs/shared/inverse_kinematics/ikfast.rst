...
* The ``filament_gui`` example's ``wam-ikfast`` scene demonstrates how to load
  the generated solver at runtime, feed solutions back into a skeleton, and
  visualize the result with the maintained Filament renderer.
