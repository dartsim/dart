"""Tabbed views for Sphinx, with HTML builder (patched for Sphinx 8)."""
...
def _matches_script(path: Path, candidate) -> bool:
    """Return True if candidate script file matches the given path."""
    name = getattr(candidate, "filename", None)
    if name is None:
        name = str(candidate)
    return name == path.as_posix()
...
def update_context(app, pagename, templatename, context, doctree):
    ...
    if not visitor.found_tabs_directive and not include_assets_in_all_pages:
        ...
        if "script_files" in context:
            # Copy to avoid mutating original list during iteration.
            filtered_scripts = []
            ...
            context["script_files"] = filtered_scripts
