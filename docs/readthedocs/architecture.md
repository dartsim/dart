...
| Baked rigid candidate/contact capacity boundary             | ✅ available    | `dart::simulation::RigidCollisionCapacityOptions` sets independent construction-time limits with deterministic bounded AABB collection and atomic built-in-step overflow. Automatic limits use the complete shape-pair envelope as the rejection threshold and reserve storage up to a fixed budget; deformable surface candidates remain solver-owned. Tested by [`test_world.cpp`](https://github.com/dartsim/dart/blob/main/tests/unit/simulation/world/test_world.cpp). |
