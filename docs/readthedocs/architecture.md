...
| Baked rigid candidate/contact capacity boundary             | ✅ available    | Independent construction-time limits, deterministic bounded AABB collection, and atomic built-in-step overflow. Automatic limits use the complete shape-pair envelope as the rejection threshold and reserve storage up to a fixed budget; deformable surface candidates remain solver-owned. |
