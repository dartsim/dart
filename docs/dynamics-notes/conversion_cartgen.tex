...
that exactly satisfies the over-constrained system, we can arbitrarily
pick three distinctive points to form the constraints and the solution
plane will be the same. Therefore, if we know $\vc{V}$ can be achieved
in the generalized coordinates, we can pick a subset of rows from $J$
to form a $J'$ such that the rank of $J'$ is $n$, and compute
$\dot{\vc{q}} = J'^+ \vc{V}'$, where $\vc{V}'$ are the velocity
components corresponding to the rows in $J'$. The solution $\dot{\vc{q}}$ to this
