...
\frac{d}{dt}\frac{\partial T}{\partial \dot{\vc{q}}} - \frac{\partial
  T}{\partial \vc{q}} = M\ddot{\vc{q}} + \dot{M} \dot{\vc{q}} - \frac{1}{2}\dot{\vc{q}}^T \frac{\partial M}{\partial \vc{q}} \dot{\vc{q}} \equiv M
...
\begin{eqnarray}
\label{eq:vellin}
\vc{v} = \dot{\vc{c}}(\vc{q}) = \frac{\partial \vc{c}}{\partial \vc{q}} \dot{\vc{q}} \equiv J_v \dot{\vc{q}}
\end{eqnarray}
...
\item $R^0_k$ is the chain of rotational transformations from the
  ...
  \sumit{Also introduce what $R_k$ means.}
\end{itemize}

\sumit{why not index the ``set'' of dofs with the link index. would make indexing lot less confusing}
...
\sumit{here itself introduce the notation $\hat{J}$ instead of \vc{u}. Could be used later and things may get more convenient}.
...
its local frame. \sumit{probably change this since $\vc{c}$ was used as global point before}.
...
\begin{equation}
 \dot{\vc{q}} = (J^TJ)^{-1}J^T \vc{V} \equiv J^+\vc{V}
 \end{equation}
...
\sumit{if dofs indexed using links, then defining local angular velocity might be easier and cleaner.}\\
\sumit{Also, can define $D$ here and then use in the forces as well.}
...
the angular part of Jacobian, $\hat{J}_{\omega k}$, is usually more sparse
that $J_{\omega k}$. Also note that the computation of the Jacobian $\hat{J}_{\omega k}$ involves only the local joint rotation as opposed to the entire chain of rotations.
...
applied on link $k$. \sumit{can make the linear jacobian at the COM and add torque component in the $\bm{\tau}$}.
...
\bm{\tau}_k)$. \eqnref{force_conversion} is then modified to:
\begin{equation}
\vc{Q} = \left (
\begin{array}{cc}
J_{v}^T & J_\omega^T
