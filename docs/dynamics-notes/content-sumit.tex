...
exponential map, $\hat{J}_{\omega k}$ does not have a simple symbolic form. \sumit{Give explanation of why not?}.
...
\eqnref{torques_cart2gen} gives the relation to convert the given cartesian forces \vc{f} and joint torques $\hat{\bm{\tau}}$ to the generalized forces $\vc{Q}$. We now describe the process to convert the given generalized forces $\vc{Q}$ to cartesian forces and torques. In general, the transposed Jacobian in \eqnref{force_conversion} can be inverted using pseudo-inverse to get the Cartesian forces and torques. However, the full Jacobian may be large and expensive to invert. Also note that the relation represents and under-constrained system when solving for $\vc{f}$ and $\bm{\tau}^{'}$. This is because the size of the unknowns is $6m$ and the number of constraints are $n$ with $n\le 6m$. Based on the information about the form of $\vc{Q}$, we can solve for the Cartesian forces and torques in different ways. We describe the solutions to the following cases:
\begin{enumerate}
...
\item \textbf{Known points of applied forces. }

\item \textbf{No linear forces. }

\end{enumerate}

\sumit{talk about inverting to get body force/torque from gen coordinates}.
 Therefore, by taking the pseudo-inverse, we get a particular \emph{least-squared} solution for the Cartesian forces and torques.
