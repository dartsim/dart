...
Next, we compute the linear and angular acceleration for the link
...
% If the total time derivative of the velocity in the local frame is 
% taken with respect to the inertial frame you will get the same result
% The trick is that you also have to take the derivative of the 
% implicit local coordinates as well as the partial velocities.
% e.g. v_point_local = vx*i + vy*j + vz*k
%     d(v_point_local)dt_global = d(vx)dt*i + vx*d(i)dt + d(vy)dt*j + vy*d(j)dt + ...
\begin{eqnarray}
