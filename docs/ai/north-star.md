...
model-format support, and an easy public API backed by a maintainable C++20
core.
...
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Release direction        | `main` targets DART 7 as a transition release with a Python-first API; `release-6.16` remains the DART 6 maintenance line; DART 8 is the planned clean break for removing deprecated DART 6 and DART 7 compatibility APIs. | `README.md`, `docs/onboarding/release-roadmap.md`, `CHANGELOG.md`                                                          |
...
| Public API modernization | DART 7 uses C++20, snake_case public headers with generated PascalCase compatibility wrappers, explicit API-boundary policy, and boundary inventory checks.                                                                | `docs/onboarding/code-style.md`, `docs/onboarding/api-boundaries.md`, `docs/onboarding/api-boundary-inventory.md`          |
| Python-first API         | `dartpy` is nanobind-based, flattens most user symbols onto the top-level module, keeps legacy submodules during DART 7, and treats Python bindings as a public API filter.                                                | `docs/onboarding/python-bindings.md`, `python/AGENTS.md`, `pyproject.toml`                                                 |
...
| Algorithm extensibility  | DART has modular foundations for math, LCP, collision, constraints, simulation, SIMD, benchmarks, and tests; algorithm-family contracts and baseline comparisons are not yet tracked as first-class research surfaces.     | `dart/`, `tests/benchmark/`, `docs/onboarding/architecture.md`                                                             |
| Scalable computation     | The codebase includes SIMD and allocator work, CI covers major host platforms, and build options already separate experimental modules; GPU acceleration and multi-core algorithm policy still need a concrete roadmap.    | `dart/simd/`, `docs/design/hierarchical_allocator.md`, `.github/workflows/`, `CMakeLists.txt`                              |
...
| AI-native substrate      | Root instructions, AI-infra principles, workflow maps, session rules, verification policy, generated Codex/OpenCode adapters, and sync checks are in place.                                                                | `AGENTS.md`, `docs/ai/`, `.claude/commands/`, `.claude/skills/`, `scripts/sync_ai_commands.py`                             |
| Active large tasks       | World split has design/migration notes but lacks the required `README.md` and `RESUME.md` files for an active dev task. GUI rendering is now tracked in durable onboarding and plan docs instead of a completed dev task.  | `docs/dev_tasks/world_split/`, `docs/dev_tasks/README.md`, `docs/onboarding/gui-rendering.md`                              |
...
| No autonomous task-selection workflow exists. Current workflows start from a user-selected task such as `dart-fix-issue`, `dart-fix-ci`, or `dart-docs-update`. | "Keep agents running" requires a repeatable way to select the next highest-value task from roadmap, dev tasks, CI, and issues.                              | Add a future `dart-next` workflow that reads this file, active dev tasks, existing CI and pull-request state, and issue labels, then proposes one bounded next task with evidence and gates.                           |
| Current project status is not machine-checkable.                                                                                                                | A written status can drift unless checks verify required links, active task shape, and workflow coverage.                                                   | Extend AI/documentation checks to validate this file is linked from AI entrypoints and that every active `docs/dev_tasks/<task>/` has the required `README.md` and `RESUME.md`.                                        |
| Active dev-task hygiene is uneven.                                                                                                                              | Long-running work loses context when a task folder omits required status and resume files.                                                                  | Normalize `docs/dev_tasks/world_split/` with the required files or move durable content into onboarding docs if the task is complete. Continue deleting completed task folders in the same PR that completes the work. |
| Release roadmap is product-focused, not execution-focused.                                                                                                      | It states DART 7/8 direction but does not rank the technical work needed to get there.                                                                      | Track current execution order in `docs/plans/dashboard.md` and keep detailed design in focused onboarding or dev-task docs.                                                                                            |
...
| Algorithm extension points are not yet explicit enough.                                                                                                         | Researchers need to implement new algorithms and compare them against built-in baselines without rewriting foundations.                                     | Identify algorithm families that need stable extension contracts, benchmark harnesses, and baseline datasets; keep implementation details internal while exposing clear research-facing APIs.                          |
