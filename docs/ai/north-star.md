...
<!-- docs-policy: evidence-last-verified=2026-07-05 -->
...
| AI-native substrate      | Root instructions, AI-infra principles and terminology, workflow maps, session rules, verification policy, `dart-next` task selection, generated Codex/OpenCode adapters, sync/runtime/scenario checks, fast cross-tool commit guards, explicit full validation, and enforced dashboard entry budgets with a plan archive are in place. | `AGENTS.md`, `docs/ai/`, `.claude/commands/`, `.claude/skills/`, `.agents/skills/`, `.codex/`, `scripts/sync_ai_commands.py`, `scripts/install_git_hooks.py`, `docs/plans/archive.md` |
