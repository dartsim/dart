...
| Active large tasks       | Filament GUI migration has a detailed dev task and north-star migration plan. The world split implementation path has moved out of working docs; its Python binding and DART 7/8 transition rules now live in onboarding docs. | `docs/dev_tasks/filament_gui/`, `docs/onboarding/python-bindings.md`, `docs/dev_tasks/README.md`                           |
...
| `dart-next` now provides a local task-selection and routing workflow, but it is not a fully automated remote management loop. | "Keep agents running" requires repeatable selection of the next highest-value task while preserving maintainer control over pushes, PRs, CI re-triggers, and review-thread mutations. | Harden `dart-next` through use: keep selection evidence tied to the dashboard, active dev tasks, CI/PR state, and issue context; route execution through specialized workflows; require explicit approval for external mutations. |
...
- `docs/onboarding/` owns durable developer design explanations after work lands.

## Autonomous Agent Loop

1. Read `AGENTS.md`, then this file, then the task-specific docs named by
   `AGENTS.md`.
2. If no task is specified, use `$dart-next` in Codex or `/dart-next` in
   Claude/OpenCode to choose one bounded task from concrete evidence: active
   dev tasks, plan dashboard, release roadmap, issues, PR/CI state, or
...
- `dart-next` or an equivalent workflow can select, execute, verify, and hand
  off one bounded next task without hidden maintainer context;
