...
| Generated adapter entrypoint | A tool-specific file generated from an editable DART source. | `.codex/skills/`, `.opencode/command/` |
...
`.claude/skills/` as the editable domain-skill source. Generated `.codex/` and
`.opencode/` files are entrypoints, not sources of truth. Change the editable
