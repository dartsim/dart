...
- a bounded half-space friction-tangent row primitive:
  ...
  coverage; dynamic/rigid contact manifold persistence is still missing; and
...
- a private 6-DOF rigid-body block foundation:
  ...
  rows for that driver. A private World-contact snapshot helper can translate
...
| DART-owned reproductions of 2D/3D demos and paper/video scenes                                     | First narrow AVBD rigid-constraint py-demo started (`avbd_rigid_fixed_joint_contact` for fixed-joint rows plus ordinary contact), `avbd_rigid_revolute_motor` covers the public free-rigid-body velocity-motor row path, `avbd_rigid_breakable_joint` covers the narrow public break-force lifecycle, and `sx_rigid_limited_joints` covers the public one-DOF joint facade path; all source demo scenes, paper figures, parameter sweeps, website demos, video/headline scenes, visual capture packets, and benchmark-linked reproductions remain missing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | D1-D6         |
| Benchmark packets beating reference demos and paper numbers on CPU/GPU                             | One narrow CPU dashboard row now tracks the public free-rigid-body revolute velocity-motor AVBD path alongside the fixed-joint row; complete benchmark packets, matched reference-demo comparisons, CPU/GPU parity, and proof against published paper numbers remain missing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | P1-P4         |
...
After the narrow free-rigid fixed-joint/contact, revolute-motor, and
breakable-joint slices, the highest-risk missing paper capability is still
articulated AVBD: the private extraction path now classifies multibody link
endpoints explicitly but still keeps them on the conservative skip path, so
DART cannot yet reproduce the paper's hard-constrained articulated chains, high
mass-ratio pendulums, or cloth-plus-articulated-rigid coupling scenes. The next
implementation slice should fill the private articulated AVBD state/extraction
bridge before broadening the demo corpus or claiming contact-stage completeness.
...
- Use existing DART foundations first: DART 7 `World`, deformable VBD
