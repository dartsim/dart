...
- a bounded half-space friction-tangent row primitive:
  ...
  missing; and
- a local self-contact normal row kernel:
  ...
  with focused `VbdCombinedDescent.AvbdSelfContact*` coverage; World row
  generation and persistence are still missing; and
...
| Constraint row inventory and storage for contacts, joints, attachments, friction, motors, fracture | Deterministic scalar row inventory started; active static contact-normal, hard-attachment, finite-spring World rows wired; bounded friction-tangent and self-contact-normal primitives started                                | A1            |
| CPU AVBD primal/dual loop over DART VBD vertices and rigid bodies                                  | Narrow serial mass-spring contact-normal, hard-attachment, finite-spring, and optional friction-tangent rows now combine; pure-tet finite-material rows wired; self-contact normal kernel local only; rigid/full rows missing | A2-A4         |
...
| Collision candidate generation, contact persistence, and row warm-start mapping                    | Static half-space row keys started; self-contact row primitive local only; manifolds, dynamic contacts, World self-contact generation, and broad row generation missing                                                       | A7            |
