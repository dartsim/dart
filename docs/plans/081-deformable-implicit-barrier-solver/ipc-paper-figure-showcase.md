...
inventory: the gap audit owns *what kernels and Algorithm 1 phases* must land,
the manifest owns *which upstream scene paths* exist, and this document owns
*which paper figures and benchmark tables a complete DART implementation must
be able to redraw*.
