...
- Operating state: `PLAN-100` in [`dashboard.md`](dashboard.md)
...
- Delivered: v1 implemented (engine unit-tested; app builds and renders the
  experimental scene headless). Remaining follow-ups below. See `dashboard.md`
...
multi-process transport, URDF/SDF/MJCF import into the experimental World,
...
2. **Render bridge + viewport**: render an experimental scene of primitives;
...
- The app can, on the experimental World: create primitive bodies and a simple
...
- The GUI submits all edits as engine commands and reads state via queries/events
  only; no GUI code mutates `sx::World` or scene state directly.
- The simulator uses only `dart::simulation::experimental`; no legacy
...
See `dashboard.md` PLAN-100 for the operating gate. Objective-specific proof:
...
- Experimental World lacks public shape access, model-format loaders,
  ...
  experimental API effort (PLAN-050,
  `docs/dev_tasks/simulation_experimental_api_design/`). The design doc's
