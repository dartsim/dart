...
AVBD row solve, and that internal driver now has a tested bounded
friction-tangent row primitive. The supported frictionless pure-tetrahedral
World envelope now carries finite-stiffness material rows with a dimensionless
Lamé multiplier and separate tet-row diagnostics, while still using the
existing lagged VBD self-contact penalty rather than AVBD self-contact rows.
Explicit fallback coverage keeps unsupported mixed spring-plus-tet, mass-spring
self-contact, frictional World scenes, Chebyshev, Rayleigh-damped, parallel,
and unsupported-row requests on the existing VBD path without partial AVBD row
counters. Those slices are still foundation work; World friction row generation,
hard-contact/friction completeness, AVBD self-contact rows, rigid/soft coupling,
