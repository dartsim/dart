...
- Next step: Continue the active
  ...
  metadata plus compact runtime sweep-buffer endpoint-distance packets,
  endpoint-linear
  point-triangle and
  ...
  packets, and an ABD/FEM coupled micro-solve packet. It still keeps GPU
  sweep-and-prune broad-phase construction, scene-owned runtime GPU candidate
  buffers, rigid curved trajectories, equality-reduced/global sparse assembly
  and factorization, GPU `World::step`, paper-scale assets, full runtime
