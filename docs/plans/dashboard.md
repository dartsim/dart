...
- Next step: Continue the active
  ...
  activates all three external families, plus a reduced lying-flat static-rigid
  surface CCD witness row. Other broader figure/demo scene rows still have zero
  external candidate/check/hit counts, and lying-flat inter-body/moving-rigid
  external counters remain zero. It still keeps
