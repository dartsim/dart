...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  static-rigid/moving-rigid surface CCD witness pair, a reduced ABD/FEM
  external surface CCD sidecar witness set, public built-in external
...
- CPU benchmark/profile packets do not yet reproduce every paper/deck
  ...
  one limited step). The reduced ABD/FEM sidecar now carries one inter-body
