...
| ------------------------ | --------- | ----------------------------------------------------- |
| `paper-deck-manifest.md` | 48        | `in-progress`: 24, `planned`: 24                      |
...
| Every non-planned row links to concrete evidence or rationale | Partial      | In-progress rows name branch-local artifacts, tests, packets, or limitations. Runtime wiring now covers point/hinge constraints, BDF-2, deformable surface obstacles, reduced hanging-bridge, nunchaku, and windmill scenes, and their reduced CPU packets, but not paper-scale behavior or performance parity. |
...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  nunchaku, and windmill smoke scenes plus reduced CPU packets, but paper-scale
  mixed scenes remain unproven.
...
- Launchable py-demo placeholders exist, and the reduced hanging-bridge,
  nunchaku, and windmill scenes have local headless smoke/capture evidence, but
  the remaining paper-scale scenes still need long-horizon visual captures and
  nonblank/motion evidence.
