...
- Private GPU packets do not yet prove same-scene CPU/GPU speedups for contact
  ...
  assembly, independent regularized Newton steps, preallocated 6x6
  off-diagonal pair slots, one symmetric sparse residual matvec,
  fixed-iteration sparse Jacobi solves, capped sparse CG solves, and sparse
  equality-reduced diagonal solves only, but runtime sparse Hessian assembly,
  direct/global sparse factorization, nonlinear equality constraints, runtime
  scene rows, and speedup remain unproven. The reduced
