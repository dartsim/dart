...
| ------------------------ | --------- | ---------------------------------------------------- |
| `paper-deck-manifest.md` | 48        | `in-progress`: 33, `planned`: 15                     |
...
| Every non-planned row links to concrete evidence or rationale | Partial      | In-progress rows name branch-local artifacts, tests, packets, or limitations. Runtime wiring now covers point/hinge constraints, BDF-2, deformable surface obstacles, reduced hanging-bridge, pulley, umbrella, terrain vehicle, ragdoll, nunchaku, nunchaku scaling, windmill, precession, reduced timing-breakdown packets, reduced Table 2 setup/statistics packets, and the sparse equality change-of-variable rigid IPC path, but not paper-scale behavior or performance parity. |
...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  windmill, precession, reduced timing-breakdown, reduced Table 2 setup/statistics
  packets, and the sparse equality change-of-variable rigid IPC path, but
...
- Launchable py-demo placeholders exist, and the reduced hanging-bridge, pulley,
  umbrella, terrain vehicle, ragdoll, nunchaku, windmill, and precession scenes
  have local headless smoke/capture evidence, but the remaining paper-scale scenes still
