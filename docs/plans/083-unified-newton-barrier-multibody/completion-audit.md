...
- Private GPU packets do not yet prove same-scene CPU/GPU speedups for contact
  ...
  off-diagonal surface-edge assembly rows, one symmetric sparse residual
  matvec, a reduced scene-owned sparse residual matvec, fixed-iteration sparse
  Jacobi solves, a reduced scene-owned sparse Jacobi solve, capped sparse CG
  solves, a reduced scene-owned sparse CG solve, and sparse
