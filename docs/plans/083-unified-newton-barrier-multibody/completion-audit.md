...
- Private GPU packets do not yet prove same-scene CPU/GPU speedups for contact
  ...
  fixed-iteration sparse Jacobi solves, and sparse equality-reduced diagonal
  solves only, but runtime sparse Hessian assembly, direct/global sparse
  factorization, nonlinear equality constraints, runtime scene rows, and
  speedup remain unproven. The reduced
