...
| ------------------------ | --------- | ---------------------------------------------------- |
| `paper-deck-manifest.md` | 48        | `in-progress`: 34, `planned`: 14                     |
| `cpu-scene-corpus.json`  | 26        | `in-progress`: 16, `not-applicable`: 1, `planned`: 9 |
...
| Every non-planned row links to concrete evidence or rationale | Partial      | In-progress rows name branch-local artifacts, tests, packets, or limitations. Runtime wiring now covers point/hinge constraints, BDF-2, deformable surface obstacles, reduced hanging-bridge, pulley, umbrella, terrain vehicle, ragdoll, nunchaku, nunchaku scaling, windmill, Candy, precession, reduced timing-breakdown packets, reduced Table 2 setup/statistics packets, and the sparse equality change-of-variable rigid IPC path, but not paper-scale behavior or performance parity. |
| CPU packets exist for performance rows                        | Blocked      | `cpu-scene-corpus.json` records commands and artifact paths, but 9 performance/comparison rows remain `planned` and many others are placeholders.                                                                                                                                                                                                                                                                                                                                             |
...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  windmill, Candy, precession, reduced timing-breakdown, reduced Table 2
  setup/statistics packets, and the sparse equality change-of-variable rigid
  IPC path, but paper-scale mixed scenes plus analytical pulley/sliding
  reproductions remain unproven.
...
- Private GPU packets do not yet prove same-scene CPU/GPU parity or speedups for
  ...
  measured separately.
- Launchable py-demo placeholders exist, and the reduced hanging-bridge, pulley,
  umbrella, terrain vehicle, ragdoll, nunchaku, windmill, Candy, and precession
  scenes have local headless smoke/capture evidence, but the remaining
