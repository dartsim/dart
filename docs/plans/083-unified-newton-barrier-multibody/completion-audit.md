...
| ------------------------ | --------- | ----------------------------------------------------- |
| `paper-deck-manifest.md` | 48        | `in-progress`: 25, `planned`: 23                      |
...
| Every non-planned row links to concrete evidence or rationale | Partial      | In-progress rows name branch-local artifacts, tests, packets, or limitations. Runtime wiring now covers point/hinge constraints, BDF-2, deformable surface obstacles, reduced hanging-bridge, terrain vehicle, nunchaku, and windmill scenes, and their reduced CPU packets, but not paper-scale behavior or performance parity. |
...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  terrain vehicle, nunchaku, and windmill smoke scenes plus reduced CPU
...
- Launchable py-demo placeholders exist, and the reduced hanging-bridge,
  terrain vehicle, nunchaku, and windmill scenes have local headless
