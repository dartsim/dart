...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  self-surface contact candidate and CCD diagnostics, a reduced hanging-bridge
  inter-body/static-rigid/moving-rigid external CCD sidecar witness set, a
  reduced umbrella inter-body/static-rigid/moving-rigid external CCD sidecar
  witness set, a reduced Candy static-rigid/moving-rigid surface CCD witness pair, a reduced
...
- CPU benchmark/profile packets do not yet reproduce every paper/deck
  ...
  sidecar witness (1 hit and one limited step). The reduced umbrella figure row
