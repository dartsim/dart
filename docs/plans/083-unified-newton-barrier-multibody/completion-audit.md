...
| Every non-planned row links to concrete evidence or rationale | Partial      | In-progress rows name branch-local artifacts, tests, packets, or limitations. Runtime wiring now covers point/hinge constraints, BDF-2, deformable surface obstacles, reduced lying-flat, hanging-bridge, pulley, umbrella, terrain vehicle, ragdoll, nunchaku, nunchaku scaling, windmill, Candy, precession, reduced timing-breakdown packets, reduced Table 2 setup/statistics packets, the sparse equality change-of-variable rigid IPC path, reduced ABD house-of-cards and wrecking-ball runtime-step packets, reduced ABD chain-net runtime-step packets, reduced ABD gears/Bullet/complex-geometry runtime-step packets, and a reduced ABD/FEM mixed-candidate packet, but not paper-scale behavior or performance parity. |
...
- Runtime mixed-domain stepping and sparse equality-constraint solving are only
  ...
  packets, and a reduced ABD/FEM mixed-candidate packet, but paper-scale mixed
...
- CPU benchmark/profile packets do not yet reproduce every paper/deck
  ...
  ABD/FEM mixed-candidate packet now exist, but accepted Bullet/reference
  baselines, a true coupled affine/FEM solve, and paper-scale assets remain
  unproven.
- Private GPU packets do not yet prove same-scene CPU/GPU speedups for contact
  ...
  endpoint-linear point-triangle CCD/line-search packet is in-progress because
  parity exists for static-triangle fixtures but edge-edge CCD, rigid curved
  trajectories, runtime candidate sets, and scene-level line-search feasibility
  remain unproven. The scalar barrier/friction local-kernel packet is
  in-progress because parity exists for clamped-log barrier derivatives and
  smoothed friction norm/work only, but primitive distance gradients,
  tangent-basis construction, Hessian assembly, PSD coupling, runtime rows, and
  speedup remain unproven. The reduced assembly/solve packet is in-progress
...
- ABD remains an internal primitive/oracle plus reduced point-triangle
  ...
  mixed-candidate packets with residual/convergence counters, not a paper-scale
  runtime affine solver or a replacement for the rigid IPC correctness oracle.
