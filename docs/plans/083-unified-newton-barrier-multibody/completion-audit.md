...
- CPU benchmark/profile packets do not yet reproduce every paper/deck
  ...
  inter-body/static-rigid/moving-rigid counter evidence (33/68/76 hits on the
  latest median row), but the broader reduced scene fixtures still have zero
  external candidate/check/hit counts; accepted Bullet/reference baselines, full
  runtime affine/FEM coupling, and paper-scale assets remain unproven.
