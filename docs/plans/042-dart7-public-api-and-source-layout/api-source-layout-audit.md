...
| `dart/dynamics`                                         | DART 6 object model and public multibody concepts.                 | Compatibility quarantine now, eventual removal from the DART 7 public contract after DART 6.20+ and later release-6.\* port needs are settled. | Surviving concepts need explicit DART 7 names and owners; legacy `World`, `Skeleton`, `BodyNode`, and orphaned cylindrical joint constraint paths are not first-use. | Freeze on `main`; port needed parity through the DART 6.20+ port lane or later `release-6.*` branches; reintroduce surviving concepts only through DART 7 owners. | `check-dart7-legacy-freeze`; `dart7-legacy-freeze: bugfix-port` only for unavoidable release-6.\* bugfix ports; gz-physics support evidence for compatibility changes. |
...
| `dart/io/{mjcf,sdf,urdf}`                               | Format-specific parser internals and advanced parser APIs.         | Compatibility/advanced parser owner behind `dart/io` common path.                                                                              | Keep parser-specific APIs only as advanced/compatibility surface.                                                                                                    | Avoid teaching as first-use path.                                                                                                                                 | Stubs/docs examples use `dart.io` and avoid a separate `utils` common path.                                                                                            |
