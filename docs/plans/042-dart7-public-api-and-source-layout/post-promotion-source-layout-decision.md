...
| `dart/dynamics`                         | DART 6 object model and legacy public multibody concepts.                                  | Compatibility/quarantine lane; surviving concepts need explicit DART 7 names.                   | `release-6.*` evidence, gz-physics lane policy, migration notes, and no return of DART 6 `World` as a main-branch common path.      |
...
- Legacy dynamics boundary: decide which `dart/dynamics` concepts survive as
  DART 7 concepts, which stay on `release-6.*`, and which are quarantined from
  first-use docs.
