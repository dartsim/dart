...
`rigid_body_modes`, and `loop closure compliance` still routes to
`rigid_loop_closure`, but each tooltip, guide payload, workflow manifest, and
review-card detail now names the missing public API surface before reporting
the route as a `deferred API caveat`. Workflow manifests now publish
`deferred_api_caveat_summary`, and the review index shows a top-level
`Deferred API Caveats` table plus badge for review-packet scanning. Focused
validation:
...
while making capped result sets visible to users. Focused validation:
...
Previous search-discovery implementation slice: rigid workflow search now covers more terms
...
Latest evidence refresh: the full numbered rows 01-36 workflow packet was
regenerated after the direct rigid-body impulse update, phase/focus static
review summary, deferred-API caveat summary, backend diagnostics, and
contact-query review-card improvements.
...
`resolved_solver_identity_count=36`, and 2544 PNG frames. A read-only static
review audit checked 181 local `href`/`src` targets in `review_index.html` with
0 missing links, and the refreshed review cards show row-5 direct impulse
signals, row-8 `backend diagnostics`, row 12-14 contact-query/collision-cast
`latest signals`, the `Workflow Phase Map`, and the top-level
`Deferred API Caveats` table. The in-viewer resilient packet command also now
matches the durable docs at `/tmp/dart_capture_rigid_workflow_resilient`.
...
| Provide reviewer-facing visual evidence.                                                         | `build/captures/rigid_workflow_rows_01_36_1781356342/manifest.json` reports `status=complete`, `capture_count=36`, `completed_count=36`, `failed_count=0`, `guidance_complete=true`, `scene_metrics_complete=true`, `scene_metrics_count=36`, `resolved_solver_identity_complete=true`, `resolved_solver_identity_count=36`, `deferred_api_caveat_count=2`, and 2544 PNG frames. Its `review_index.html` includes row-5 direct impulse signals, the latest backend-diagnostics and contact-query/collision-cast review-card summaries, the `Workflow Phase Map`, and the top-level `Deferred API Caveats` table. `build/captures/rigid_workflow_optional_signal_highlights_1781338541/manifest.json` reports `status=complete`, `capture_count=17`, `completed_count=17`, `failed_count=0`, `guidance_complete=true`, `scene_metrics_complete=true`, `scene_metrics_count=17`, `resolved_solver_identity_complete=true`, `resolved_solver_identity_count=17`, related/IPC-shelf/packet groups enabled, and a review card packet where rows 37-53 now surface optional health signals such as min tunnel margin, frame budget, top drift, differentiable fallback, and AVBD target/measured speed. Static review-index asset audits found 181/181 and 86/86 local links present. | Proven locally                                |
...
- Latest merge check: `git fetch origin main && git merge --no-ff origin/main`
...
- Latest focused validation for current pushed head: the deferred-API
  review-packet guard reported `7 passed`; the prior workflow search/docs
  guard reported `3 passed`, and the prior optional latest-signal guard
  reported `9 passed`.
...
- Full review packet:
  `build/captures/rigid_workflow_rows_01_36_1781356342/review_index.html`
