...
| Follow the DART 7 harness solver-identity evidence rule for generated packets.                 | `py-demo-capture` now promotes `resolved_solver_identity` from latest scene metrics into successful per-scene manifests, attaches it to captured workflow rows, summarizes identity completeness in workflow manifests, and shows it in `review_index.html`; focused unit/integration guards reported `4 passed`. Full GUI packet regeneration attempted at `build/captures/rigid_workflow_rows_01_36_1781311276` but row 1 exited with `return_code=-7` before metrics were written. Direct Python and C++ GUI reproducers now both die with `SIGBUS`; gdb shows the Python path failing in `filament::backend::CircularBuffer::alloc` during `filament::FEngine::create` from `createFilamentRenderContext`, so the failed refresh path is not evidence. | Focused guards proven; full packet refresh blocked locally |
...
- Current refresh blocker: `build/captures/rigid_workflow_rows_01_36_1781311276`
  failed before metrics; direct `py-demos` and C++ `demos` reproducers die
  with `SIGBUS` in Filament engine creation on this host. This is failure
  evidence only. A gdb register snapshot at the crashing `memset` showed
  `rdx=0x7fff371fd000`, pointing the next native debug pass at the Filament
  command-buffer allocation size or ABI boundary.
- PR body seed:
