...
| Follow the DART 7 harness solver-identity evidence rule for generated packets.                 | `py-demo-capture` now promotes `resolved_solver_identity` from latest scene metrics into successful per-scene manifests, attaches it to captured workflow rows, summarizes identity completeness in workflow manifests, and shows it in `review_index.html`; focused unit/integration guards reported `4 passed`. Full GUI packet regeneration attempted at `build/captures/rigid_workflow_rows_01_36_1781311276` but row 1 exited with `return_code=-7` before metrics were written, so that failed path is not evidence.                                         | Focused guards proven; full packet refresh blocked locally |
