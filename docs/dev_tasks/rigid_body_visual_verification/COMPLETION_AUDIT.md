...
date`), pushed commit `7829d9b0487` passed the default command
...
documentation, and CUDA tests. The default CTest log records `test_world` at
`00:02:24` and `test_rigid_ipc_paper_experiments` at `00:04:52`; the CUDA CTest
log records `test_lcp_jacobi_batch_cuda` at `00:03:48`. These broad wrapper
runs cover the current implementation state at `7829d9b0487`; this docs-only
evidence refresh postdates them. After the search-routing,
...
| Provide automated drift guards.                                                                  | Focused docs/API drift guards, scene-metrics route guards, capture/review-index guards, current-head `pixi run lint`, and focused workflow-search/latest-signal guards are recorded in the PR draft and recent local commits. Broad default `pixi run test-all` passed under `DART_SAFE_JOBS=5` at pushed commit `7829d9b0487`, and CUDA `pixi run -e cuda test-all` passed on the visible RTX 4080 Laptop GPU at the same implementation state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Proven locally; rerun broad wrappers only if maintainers require proof including this docs-only evidence commit |
...
- Latest broad default validation for pushed commit `7829d9b0487`:
  ...
  the default CTest log recorded `test_world` at `00:02:24` and
  `test_rigid_ipc_paper_experiments` at `00:04:52`.
- Latest CUDA validation for pushed commit `7829d9b0487`:
  `pixi run -e cuda test-all` passed all seven wrapper gates with
  `DART_SAFE_JOBS=5` on a visible `NVIDIA GeForce RTX 4080 Laptop GPU`; the
