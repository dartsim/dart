...
for the public GUI controls that make the later joint and multibody rows useful
for visual debugging. The comparison-panel test now asserts the editable
Perturbation, motor/limit, screw-pitch, dynamics-term, COM, and Jacobian
slider labels and ranges instead of checking only explanatory text. Row 29,
`rigid_joint_motor_limits`, also has a focused panel-edit guard proving that
the GUI sliders update the controller, joint command velocity, velocity limits,
position upper limit, requested force, effort cap, and capture controls.
Focused guards:
...
reported `2 passed`. This slice adds no new visual packet; use the existing
rows 01-36 packet as the latest broad visual artifact until a maintainer asks
...
`failed_rows=[]`, and 2544 frame PNGs. The current optional rows 37-53 packet
...
from `rigid_contact_solver_compare` as related evidence.
...
- Get explicit maintainer acceptance that the maintained 36-row workflow plus
  optional packet groups, now totaling 53 rows when all optional groups are
...
3. Use
   `build/captures/rigid_workflow_rows_01_36_1781356342/review_index.html` and
