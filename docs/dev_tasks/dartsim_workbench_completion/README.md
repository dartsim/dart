...
- [ ] Phase 4: Add simulation workbench panels for watch values, sensor views,
      ...
      `inspect` commands query Inspector property snapshots without changing
...
- Console automation now uses `dartsim_ui/console_actions` instead of direct
  ...
  snapshots, relationship edit, simulation mode, recording, replay, and watch
  commands through the same headless action helpers as the menu and panel UI.
