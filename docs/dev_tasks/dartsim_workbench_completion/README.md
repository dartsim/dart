...
- [ ] Phase 5: Add viewport controls, visibility filters, camera presets,
      ...
      links, and frames. A tested internal single/quad viewport layout state
      exists for the renderer-side multi-viewport follow-up.
...
| Viewport controls        | rotate/pan/zoom modes, camera lock, front/side/top/perspective, fit view, four-view layout                                                           | View menu camera actions now compute fit scene, focus selection, perspective/front/back/left/right/top/bottom presets, Orbit/Pan/Zoom mouse modes, and selected-object tracking from `dartsim_ui/viewport_actions`, then apply them through renderer-neutral camera seams. A tested internal single/quad layout state is ready for renderer consumption | Expose four-view controls only when renderer-side multi-viewport drawing consumes the layout state             |
...
- Viewport layout actions now provide a tested single-view vs four-view state
  and active pane selection through `dartsim_ui/viewport_actions`. They remain
  internal until renderer-side multi-viewport drawing consumes that state, so
  the View menu does not claim four-view rendering before the renderer can draw
  it.
...
2. Continue Phase 5 by consuming the tested viewport layout state in
   renderer-side multi-viewport drawing.
