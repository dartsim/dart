...
| Project lifecycle        | New/open/save/save-as, current path, dirty marker, recent files, close/load prompts, progress/status events                                          | Phase 1 now has headless dirty/path/new/save/load state, blocks dirty replacement by default, routes Open through a built-in project browser with native Browse support, and keeps Save As behind an injectable `ProjectFileDialog` seam                                        | Add recent files, close/load confirmation prompts, and visible dirty-state affordances                         |
...
- Project open now starts from the backend-hidden in-app project browser so the
  File menu remains usable even when the native picker backend fails. The native
  picker remains available from Browse and retries without a parent window when
  that handle is rejected; extensionless explicit paths resolve to `.dartsim`
  files when present.
