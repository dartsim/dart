...
- [ ] Phase 4: Add simulation workbench panels for watch values, sensor views,
      ...
      session-local Watch state, and chart signal choices are configurable.
- [ ] Phase 5: Add viewport controls, visibility filters, camera presets,
      ...
      independently. The View menu can also fit the full scene or focus the
      selected object across all four remembered pane cameras at once.
...
| Watch/chart panels       | Watch list values and plotted simulation signals                                                                                                     | `dartsim_ui/watch_actions` now owns a session-local watch list, selected-object add/remove/clear actions, object value rows for transforms, rigid-body mass, and single-axis link joints, plus bounded chart samples for configurable simulation time/frame and watched object x/y/z/mass/joint-position series. The editor exposes this through a Watch panel using renderer-neutral `PanelBuilder::plotLines`, and the Console can drive the same watch state through `watch`/`unwatch` and `watch signal` commands.                                                                                                                                                                                                                                   | Add saved watch presets after scene metadata supports them                                                 |
