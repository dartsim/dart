...
| Contacts on multibody links     | links participate in the constraint solve          | links carry shapes, appear in `World::collide()`, and rest on static obstacles via a normal-impulse response (Phase 2-3); link-vs-dynamic/link, friction, LCP pending | PARTIAL                |
