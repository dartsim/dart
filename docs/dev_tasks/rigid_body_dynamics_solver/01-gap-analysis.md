...
| Contacts on multibody links     | links participate in the constraint solve          | links carry shapes, appear in `World::collide()`, and rest/slide/bounce on static obstacles via a normal + two-tangent friction + restitution impulse response (Phase 2-3); link-vs-dynamic/link and LCP pending | PARTIAL                |
