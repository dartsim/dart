...
classic-World references, the gate is re-claimed from a `release-6.*` parity
run rather than a new harness in this tree. The next step is owned by the
PLAN-080 entry in `docs/plans/dashboard.md`; the contact-solver plan below
lists the remaining articulated-contact steps.
...
Then continue from the PLAN-080 entry in `docs/plans/dashboard.md`; the
contact-solver plan above lists the remaining articulated-contact steps.
