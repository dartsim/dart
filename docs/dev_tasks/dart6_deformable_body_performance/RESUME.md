...
- `07-equation-correctness.md` records the first active WP-DB.04 sub-gate:
  ...
  `Skeleton::getMassMatrix()` and `Skeleton::getAugMassMatrix()`, and
  point-mass gravity contributes to `Skeleton::getGravityForces()` and
...
3. Extend `test_SoftDynamics` beyond finite-state and one-thread versus
   ...
   platforms.
4. Continue WP-DB.04 with point-mass inverse-mass and inverse-augmented-mass
   aggregation, then re-enable or replace the disabled equations-of-motion
   checks in `test_SoftDynamics.cpp`.
