...
| Biped push recovery | Same SIMBICON-style controller with rigid contact vs soft contact; soft contact withstands larger perturbations and motor noise while maintaining more ground contact points. | PR #3408 adds the soft-foot SIMBICON regression, with the models normalized to equal mass, one foot collision surface, and the same rest-pose collision tessellation (the rigid control collides as a rigid copy of the soft rest mesh). **Contact spreading reproduces**: 52.0 soft vs 18.45 rigid, a 2.8x spread gate-enforced at 1.5x; representation alone moves the rigid count between 3.6 (analytic box) and 44.4 (STL). **Push recovery does not**: 4000 N soft vs 12000 N rigid. | Two maintainer decisions block the push claim — soft-foot **stiffness** (too compliant; kv 1e6 / damp 5e3 recovers 12000 N, matching the control) and foot **inertia/COM** (point masses leave the soft foot at ~1.8-2.0x the rigid moments). Options and trade-offs in `12-pr3a-soft-foot-simbicon.md` § Open maintainer decisions. Then re-assert the comparison. Motor-noise variant still unbuilt. Owner: WP-DB.03, WP-DB.08, WP-DB.09. |
