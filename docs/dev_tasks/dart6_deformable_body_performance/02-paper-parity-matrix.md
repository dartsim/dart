...
| Biped push recovery | Same SIMBICON-style controller with rigid contact vs soft contact; soft contact withstands larger perturbations and motor noise while maintaining more ground contact points. | PR #3408 adds the soft-foot SIMBICON regression, with the models normalized to equal mass, one foot collision surface, and the same rest-pose box collision geometry. **Contact spreading reproduces**: 52.0 soft vs 3.6 rigid contact points (the earlier 52-vs-44 figure was mostly the STL control's mesh manifold, not compliance). **Push recovery does not**: 4000 N soft vs 8000 N rigid. | Two maintainer decisions block the push claim — soft-foot **stiffness** (too compliant; kv 1e6 / damp 5e3 recovers 12000 N and beats rigid) and foot **inertia/COM** (point masses leave the soft foot at ~1.8-2.0x the rigid moments). Options and trade-offs in `12-pr3a-soft-foot-simbicon.md` § Open maintainer decisions. Then re-assert the comparison. Motor-noise variant still unbuilt. Owner: WP-DB.03, WP-DB.08, WP-DB.09. |
