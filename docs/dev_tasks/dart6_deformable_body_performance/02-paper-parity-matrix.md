...
| Biped walk | 334 total DOFs, 197 +/- 43 simulated DOFs, 16 +/- 3 contacts, 18.5 +/- 4.5 fps, 63 +/- 5% LCP time, 4.0 ms step, stiffness 1e3; LCP solved every 8 SIMBICON steps with a 0.5 ms SIMBICON step. | `soft_foot_simbicon` (#3408/#3423) is a soft-contact SIMBICON scene, but it walks in place; the walking gait with the paper's LCP cadence, contact counts, solver-time share, and deterministic outcome remains unbuilt. | Add a soft-foot walking scene with soft-contact contact counts, solver-time share, and deterministic controller outcome. Owner: WP-DB.08, WP-DB.09. |
...
| Flexible rigid foot comparison | Four-link rigid foot is more stable than a simpler rigid foot, but deformable foot remains more stable because contact changes are more continuous. | #3408/#3423 landed the two-way baseline this row extends: `soft_foot_simbicon` runs the same SIMBICON controller with rigid or deformable feet on ground under scheduled pushes, and `SoftFootSimbiconModelTest` gates that comparison (equal mass/COM/inertia, same collision tessellation, contact spreading, push recovery). No four-link flexible-rigid comparator or three-way paired evidence exists yet. | Add the four-link flexible-rigid foot variant to the existing comparison — three-way, same controller and seeded perturbations. Record stability/contact metrics and deterministic outcomes. Owner: WP-DB.08, WP-DB.09. |
