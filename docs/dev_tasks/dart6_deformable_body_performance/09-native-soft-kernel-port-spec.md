...
- Verified: no parallelism in the direct `NativeCollisionDetector` path — `collide` walks pairs serially. `ConstraintSolver::setNumSimulationThreads()` therefore configures only `DARTCollisionDetector`; the native detector deliberately exposes no inactive thread-count option until this stage is implemented. The DART factory parallelizes soft-soft finite pairs through `processFiniteFiniteCandidatePairs` (gate `kMinParallelSoftSoftPairs=4`) with `prepareParallelSoftSoftCaches` pre-warm.
- Map onto the direct native pipeline: first add the native thread-count option, then collect soft-soft (and soft-primitive) candidate pairs into a vector; when `count >= kMinParallelSoftSoftPairs` and the requested count is greater than one, dispatch pair narrowphase across workers. Per-worker state is required: one `ScratchCollisionResult`/soft pair-result buffer per worker, plus per-worker output buffers merged deterministically in enumeration order to preserve checksums. Pre-warm the lazy face/BVH cache before the parallel region (equivalent to `prepareParallelSoftSoftCaches`) since first-access recompute mutates shape cache and is not thread-safe. Manifold attach stays a serial post-pass (soft bypasses it anyway, §4). Broadphase storage is read-only during the visit, so pairing is safe; the constraints are the shared result and scratch state.
