...
- `tests/integration/test_SoftDynamics.cpp` contains active finite-state,
  deterministic threaded final-state, and representative equations-of-motion
  gates for deformable scenes. WP-DB.04 now covers point-mass mass-matrix,
  augmented-mass, inverse-mass, inverse-augmented-mass, gravity,
  combined-vector, external-force, and representative rigid-only/soft-only/
  mixed-world matrix/vector checks, recorded in
...
- WP-DB.06 scalar/cache optimization keeps public `SoftBodyNode` and
  ...
  changing public headers or class layout. The latest slice backs that view
  with raw point/state/property spans and simplifies the point-mass
  articulated-inertia scalar formula, then fuses several remaining cached
  point-force fill and soft-body spatial aggregation loops. Current checksums
  stay stable, but timing remains too noisy for a hard speedup threshold. The
  branch is now stacked on
