...
- `tests/integration/test_SoftDynamics.cpp` contains an equations-of-motion
  comparison that remains disabled because full soft-body equation parity is
  not complete. WP-DB.02 adds an active finite-state gate around representative
  soft scenes, including ordered final-state comparison between one-thread and
  four-thread runs. WP-DB.04 now also adds an active point-mass mass-matrix,
  augmented-mass, inverse-mass, inverse-augmented-mass, gravity, and
  combined-vector sub-gate, recorded in `07-equation-correctness.md`; the
  broader matrix/vector equation checks remain open.
...
- WP-DB.06 scalar/cache optimization keeps public `SoftBodyNode` and
  ...
  threshold. The branch is now stacked on `origin/dart6-memory-hardening` and
...
| WP-DB.04 coupled equation correctness | Finish or replace stubbed point-mass mass, inverse-mass, gravity, combined-force, and external-force aggregation paths. | Point-mass mass-matrix, augmented-mass, inverse-mass, inverse-augmented-mass, gravity, and combined-vector contributions now have an active regression in `07-equation-correctness.md`; packet completion still requires matrix/vector checks for rigid-only, soft-only, and mixed rigid-soft worlds. |
