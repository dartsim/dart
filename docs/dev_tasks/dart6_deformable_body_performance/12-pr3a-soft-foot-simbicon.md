...
- Reset is bit-for-bit equivalent to a freshly built model: a reset model and a
  ...
  broadphase.
- **Push recovery: superseded by the robustness re-measurement below.**
  The original gate asserted soft ≥ rigid on single-trajectory measurements
  (18000 N soft vs 8000 N rigid); the ensemble re-measurement showed that
  figure was a resonance pocket and no configuration shows a soft
  advantage, so the gates now protect measured ensemble floors and publish
  the response curves instead, and the ordering lives in the parity matrix
  as an open mechanism gap.
...
- `long-measurement` (the three sweeps) excludes them wherever a runtime
  multiplier would have them re-run already-exercised code paths for hours:
  the FreeBSD VM job (the push sweep passed there in 3674 s but consumed
  half the job's wall clock), the ASan ctest (the noisy-floor sweep overran
  7200 s), and every unoptimized aggregate run (`DARTRunCTest.cmake`
  self-excludes outside optimized configurations after the
  CMAKE_BUILD_TYPE=None lane hit CI's six-hour ceiling). The unlabelled
  structure gate settles both foot types on the mesh floor so those
  contexts still execute the collision paths.

Optimized Release lanes (Linux/macOS/arm64/Windows) run all entries, and
FreeBSD still runs the full correctness suite.
...
- Robust thresholds at shipped parameters: clean **soft 4000 N vs rigid
  ...
  rigid arm down to the soft arm's level); paper's 2 cm fold-free floor
  **2000 vs 6000** (roughness degrades both arms, the soft one more). The
  floor number took three review rounds to make honest: the axis-aligned
  box lattice keyed the rigid foot to 14000 N; the first jittered mesh
  read 20000 N but its 0.4-pitch clamp let 10 triangles fold over in
  projection (review counted them; reproduced exactly), so that anchoring
  was folded-geometry artifact too; the fold-free surface (clamp at the
  winding-preservation bound, gate-asserted per triangle) gives 6000 N.
  No configuration shows a soft advantage. The paper's soft-advantage
  ordering does not reproduce robustly; the parity matrix carries the open
  gap, and the noisy-floor row's own course-tracking outcome stays
  unmeasured pending the walking gait.
...
4000/4000, floor 4000/2000 with 6000/2000 measured) and print the full
