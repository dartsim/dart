...
`simple_frames`, `soft_bodies`, `polyhedron_visual`, `heightmap`, and
`g1_puppet` runners now route to focused Filament scenes by default while their
standalone sources remain as legacy OSG comparison material until the promoted
API can replace the missing panel/tool-specific workflows.
