...
`rigid_cubes`, `drag_and_drop`, `empty`, `simple_frames`, `soft_bodies`,
...
route to focused Filament scenes, the default Filament visual-coverage scene, or
the built-in Filament panel by default while their standalone sources remain as
legacy OSG comparison material until the promoted API can replace the missing
