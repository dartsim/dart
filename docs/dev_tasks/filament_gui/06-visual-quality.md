...
inspection views. The default scene now combines cascaded SUN shadows, a
neutral skybox, spherical harmonics indirect lighting, and high-quality color
grading; windowed runs use PCSS/contact shadows while headless screenshot
captures use deterministic PCF shadows without screen-space contact shadows on
the current software OpenGL path. Debug lines for grid, world/body frames,
