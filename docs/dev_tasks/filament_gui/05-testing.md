...
- `UNIT_gui_FilamentSceneExtraction` builds and passes. It covers the
  ...
  points/normals including primitive sphere/ellipsoid/cylinder surface hits,
  renderable set update planning, and debug-line descriptors, without requiring
  Filament or a graphics context.
...
- Renderer-independent nearest-hit picking is unit-tested for visible, hidden,
  hit, miss, bounds-normal, and primitive sphere/ellipsoid/cylinder
  surface-normal cases. The example wires that path into basic click-to-select
  highlighting plus a selection bounds overlay.
...
- `UNIT_gui_FilamentSceneExtraction` also covers run-option normalization,
  ...
  pick-ray generation without requiring a graphics context.
