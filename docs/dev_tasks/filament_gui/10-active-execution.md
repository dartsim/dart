...
- Latest pushed maintenance checkpoint: `b4ed7d9d62f` removes the no-source
  `examples/rerun/` placeholder because the re-opened audit found no concrete
  integration workflow, executable, or downstream user. Local validation
  covered focused build/CTest, aggregate examples build, example-runner tests,
  absence checks, `git diff --check`, mandatory `pixi run lint`, and post-lint
  focused checks.
...
capture` updates Fetch README/audit/marker coverage for the promoted `--out`
  image-sequence path, R24-1 through R24-12 add public
...
- Latest local R24-24 validation passed focused build for `heightmap`,
  ...
  scan, and `git diff --check`. R24-12 validation passed focused build for
  `fetch` plus
  `UNIT_gui_FilamentSceneExtraction`, focused CTest, direct 1280x960
  ...
  `examples` build, mandatory `pixi run lint`, post-lint focused
  rebuild/CTest, post-lint direct screenshot smoke, stale-handle text scan, and
  `git diff --check`. R24-10 validation passed focused build for `tinkertoy`
  plus `UNIT_gui_FilamentSceneExtraction`, focused CTest, direct software-GL
  screenshot analyzer smoke for `tinkertoy`, aggregate `examples` build,
  mandatory `pixi run lint`, post-lint focused rebuild/CTest, post-lint direct
  screenshot smoke, stale-handle text scan, and `git diff --check`. R24-9
  validation passed focused build for `imgui` plus
  `UNIT_gui_FilamentSceneExtraction`, focused CTest, direct software-GL
...
- Active local implementation checkpoint: `examples/hubo_puppet/` whole-body
  solver follow-up. This slice restores the historical source-owned
  `RelaxedPosture` objective, public `BalanceConstraint`, whole-body IK solving
  path, and hold/release R posture/balance optimization. This closes the Hubo
  balance/key-state solver gap without backend or private renderer APIs.
