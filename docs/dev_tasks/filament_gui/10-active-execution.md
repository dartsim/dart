...
capture` updates Fetch README/audit/marker coverage for the promoted `--out`
  image-sequence path, and R24-1 through R24-10 add public
  ...
  `imgui` example, and Tinkertoy's force target. These local checkpoints are
  not pushed.
- Latest local R24-10 validation passed focused build for `tinkertoy` plus
