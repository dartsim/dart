...
| Keep examples out of backend internals.         | The GUI application CMake helper now rejects direct Filament includes and non-minimal application sources for `apps/dartsim`; backend implementation files live under `dart/gui/experimental/detail/filament`.       | Satisfied for the official runner.                                       |
