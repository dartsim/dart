...
| Keep maintained examples free of direct Filament headers after promotion.                | The north-star metric is now explicit: any surviving replacement for `examples/filament_gui` must include zero direct `#include <filament/...>` or `#include "filament/..."` directives. Filament headers should move under private promoted GUI implementation units, with examples using DART-owned viewer, scene, debug, capture, and tool APIs. `UNIT_gui_FilamentSceneExtraction` now guards the incremental state where `examples/filament_gui/*.hpp` and `examples/filament_gui/main.cpp` have no direct Filament header includes, and the render-context, screenshot-readback, render-environment, texture, and renderable implementation slices have moved under `dart/gui/experimental/detail/filament`. The current MVP example still does not satisfy the full metric because the remaining example-local ImGui overlay implementation owns Filament renderer resources as MVP evidence rather than using a promoted DART GUI API.                                                                                                                                                            | Not satisfied; this is a first-class promotion gate.                                  |
| Keep maintained examples minimal after promotion.                                        | The north-star metric now also requires any surviving `examples/filament_gui/` tree to shrink to a minimal executable entry point. Renderer setup, frame lifecycle, material/texture resources, scene synchronization, capture, overlays, input translation, and reusable fixture logic should be encapsulated by `dart::gui` or private GUI implementation units, not preserved as example-local architecture. The current branch has reduced `main.cpp` through focused helper files, but those helpers still live in `examples/filament_gui/` and must migrate or collapse into promoted GUI APIs before completion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Not satisfied; this is a first-class promotion gate.                                  |
