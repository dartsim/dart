...
| Provide package-manager path or explicit fallback.                                       | Pinned CMake fetch works locally with compatible libc++/libc++abi. `pixi search filament` still reports no installable conda-forge package, but `conda-forge/staged-recipes#33297` is an open non-draft PR for Filament 1.71.3 with `filament` and `filament-static` outputs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Tracked externally; package PR is not merged and does not block Linux smoke.          |
...
pixi run python -m pytest python/tests/unit/gui/test_filament_headless_smoke_analyzer.py -q
...
generated by the same headless path, including full-image nonzero analyzer
coverage and a scene-region luminance contrast check from the shadowed fixture.
After adding the neutral skybox,
...
All generated scene-specific headless smokes now run the analyzer in basic
mode, which scans the complete generated PPM for nonzero pixels.
...
The Filament example's selectable `--scene free-joint-cases` fixture now backs
the in-tree `pixi run ex free_joint_cases` runner, so the recommended
free-joint visual workflow exercises descriptor-owned active and transparent
reference boxes plus the private torque-free reference pre-step hook while the
standalone source in `examples/free_joint_cases` remains as legacy OSG/ImGui
comparison material for numeric checks and reference-model controls.
The Filament example's selectable `--scene human-joint-limits` fixture now
backs the in-tree `pixi run ex human_joint_limits` runner, so the recommended
human joint-limit visual workflow exercises descriptor-owned Kima human mesh,
multi-sphere, box, and ground renderables with DART joint-limit enforcement
enabled while the standalone source in `examples/human_joint_limits` remains
legacy comparison material for the custom TinyDNN-backed arm and leg
constraints.
The Filament example's selectable `--scene lcp-physics` fixture now backs the
in-tree `pixi run ex lcp_physics` runner, so the recommended LCP benchmark
visual workflow exercises descriptor-owned contact boxes, falling spheres, and
ground renderables while the standalone source in `examples/lcp_physics`
remains legacy comparison material for solver controls, plots, scenario
switching, and frame recording.
...
The Filament example's selectable `--scene hubo-puppet` fixture now backs the
in-tree `pixi run ex hubo_puppet` runner, so the recommended Hubo IK visual
workflow exercises descriptor-owned Hubo mesh, ground, and selectable hand,
foot, and wrist-peg target renderables while the standalone source in
`examples/hubo_puppet` remains as legacy OSG comparison material for the
teleoperation widget, support-polygon visual, and keyboard controls.
The Filament example's selectable `--scene atlas-simbicon` fixture now backs
the in-tree `pixi run ex atlas_simbicon` runner, so the recommended Atlas
Simbicon visual workflow exercises descriptor-owned Atlas mesh and ground
renderables while the standalone source in `examples/atlas_simbicon` remains
legacy OSG/ImGui comparison material for the gait controller, perturbation
...
The Filament example's selectable `--scene wam-ikfast` fixture now backs the
in-tree `pixi run ex wam_ikfast` runner, so the recommended WAM IKFast visual
workflow exercises descriptor-owned robot, ground, and end-effector target
renderables while the standalone source in `examples/wam_ikfast` remains as
legacy OSG comparison material for the IKFast solver, drag modes, keyboard
shortcuts, and posture reset workflow.
The Filament example's selectable `--scene fetch` fixture now backs the in-tree
`pixi run ex fetch` runner, so the recommended Fetch MJCF visual workflow
exercises descriptor-owned robot, object, and target renderables while the
standalone source in `examples/fetch` remains as legacy OSG/ImGui comparison
material for panel, drag-control, and mocap target-update behavior.
The Filament example's selectable `--scene tinkertoy` fixture now backs the
in-tree `pixi run ex tinkertoy` runner, so the recommended Tinkertoy visual
workflow exercises descriptor-owned block, joint, target, force-line, and
reference-axis renderables while the standalone source in `examples/tinkertoy`
remains as legacy OSG/ImGui comparison material for panel, mouse-picking, and
block-add/delete behavior.
...
- `conda-forge/staged-recipes#33297` is open and non-draft at head
  `d78834e9bcca1c0c5a55f3d2752f39765932c3f2`; it builds Filament 1.71.3 from
