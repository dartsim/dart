...
The strict-audit slice restores the example as a live `dart::gui` world and a
later local follow-up restores the historical neural arm/leg custom
constraints. The implementation reads the checked-in TinyDNN-serialized model
files directly, avoiding a TinyDNN package dependency while preserving the
constraint behavior through DART's public constraint solver.
...
| Install left/right `HumanArmJointLimitConstraint` and `HumanLegJointLimitConstraint` instances.   | Restored.                         | Current source uses a compact source-owned reader for the checked-in serialized neural-network weights.           |
...
| Default launch size 640x480.                                                                      | Restored.                         | Current source uses `ApplicationOptions::runDefaults`.                                                            |
| README documents the promoted runner and no longer says the source is legacy OSG comparison code. | Restored.                         | Current README documents controls, capture, and the restored neural custom constraints.                           |
...
| OSG depth mode toggle.                                                                                                          | Restored through public `dart::gui`. | Public `RenderSettings::outputMode` exposes a renderer-neutral depth visualization mode without OSG or private Filament hooks.      |
...
| README documents the Simbicon controller workflow and promoted runner.                                                          | Restored.                            | Current README documents the promoted runner, depth output control, and `--render-output depth` capture.                            |
...
| Enable mouse drag on G1 body nodes.                                                                      | Restored through public `dart::gui`.     | `BodyNodeDragHandle` opts the G1 body nodes into promoted Alt/Ctrl/Shift articulated-body IK dragging.                                     |
...
| README documents package overrides, kinematic mode, IK targets, controls, build/run instructions.        | Restored.                                | README documents the promoted `dart::gui` target gizmos and body-node drag modes.                                                          |
...
are restored, and the current local follow-up restores the step-time history
plot through public `PanelBuilder::plotLines`. The next local follow-up adds
public `PanelContext::UiState` metrics and restores display/font debug
diagnostics without backend hooks.
...
| Preserve performance/debug information: FPS, contacts, body count, step-time plot, ImGui display/font metrics.  | Restored through public `dart::gui`.      | Render FPS, rendered/skipped frames, contacts, body count, step-time summary, step-time plot, display size, framebuffer scale, font size/scale, and font texture size are shown now. |
...
| Keep marker coverage for scene counts, solver/scenario controls, panel controls, README, and no backend types.  | Restored for this slice.                  | Marker guards now cover scene counts/names, live controls, README options, capture flags, step-time plotting, UI debug metrics, and absence of legacy launcher.                      |
...
Panel plotting follow-up validation passed: focused `lcp_physics` and
`UNIT_gui_FilamentSceneExtraction` build, focused CTest, mandatory
`pixi run lint`, post-lint focused build/CTest, and post-lint
LCP pixi headless screenshot analyzer smoke with 307200/307200 nonzero pixels.

UI metrics follow-up validation passed: focused `lcp_physics` and
`UNIT_gui_FilamentSceneExtraction` build, focused CTest, mandatory
`pixi run lint`, post-lint focused build/CTest, and post-lint LCP pixi headless
screenshot analyzer smoke with 307200/307200 nonzero pixels.
...
| Enter toggles runtime recording to `dataPath("screencap")`.                                                          | Restored through public `dart::gui`. | `ViewerLifecycleState` exposes runtime frame-output capture state, and Tinkertoy toggles it to the historical `screencap` data directory.                               |
...
| Panel Menu/Exit and Help/About DART entries.                                                                         | Restored through public `dart::gui`. | `requestExit` covers Menu/Exit, and public `PanelBuilder` modal helpers cover Help/About DART without backend hooks.                                                    |
...
| README documents summary, controls, in-tree run, build, and execute instructions.                                    | Restored.                            | Current README documents promoted run/capture commands plus standalone build/execute sections and Enter-key recording.                                                  |
| Keep marker coverage for controls, selected hit data, panel options/labels, README, defaults, and no backend types.  | Restored.                            | Marker guards cover the public target gizmo, selection-hit data, panel options, labels, README, defaults, runtime recording, and absence of private scene/defaultScene. |
...
README, and marker guards. A later public `BodyNodeDragHandle` follow-up
restores the old Alt/Ctrl/Shift body-node drag split through promoted
articulated-body IK dragging.
...
| Create a ground plane and kinematic WAM world with physical simulation disabled.                            | Restored through public `dart::gui`. | `ApplicationOptions::simulateWorld = false` skips `World::step()` while keeping the pre-step IK solve callback active.                   |
...
| Alt/Ctrl/Shift drag-mode instructions for translation, rotation, and parent-joint-only manipulation.        | Restored through public `dart::gui`. | `BodyNodeDragHandle` registers WAM body nodes for Alt translation, Ctrl rotation, and Shift parent-joint-only IK dragging.               |
...
| README documents summary, controls, promoted run/capture, build, and execute instructions.                  | Restored in this slice.              | README documents `pixi run ex wam_ikfast`, `--screenshot`, `--out`, kinematic no-world-step execution, and the restored body drag modes. |
...
| `atlas_simbicon`            | Restored by strict re-open.                                                                                                                          | Keep marker guards for controller/state files, pre-step control, perturbation and stride controls, native window title, shadow/headlight/depth controls, camera/defaults, README, and no OSG types.                                                                                                                                   |
...
| `g1_puppet`                 | Restored by strict re-open.                                                                                                                          | Keep marker guards for package loading, gravity, ground, root pose, grid, support overlay, camera/defaults, README, target toggles, body-node drag handles, and no backend types.                                                                                                                                                     |
...
| `human_joint_limits`        | Restored by strict re-open.                                                                                                                          | Keep marker guards for live world, neural custom constraints, joint-limit enforcement, defaults, README, and no backend types.                                                                                                                                                                                                        |
...
| `lcp_physics`               | Restored by strict re-open.                                                                                                                          | Keep marker guards for historical scene counts/names/placements, live solver/scenario/reset/timestep/gravity panel controls, FPS/step-time diagnostics and plot, UI display/font metrics, README options, capture flags, and no backend types.                                                                                        |
...
| `tinkertoy`                 | Restored by strict re-open.                                                                                                                          | Keep marker guards for selected hit point/normal, panel options/help/buttons, headlight checkbox, explicit defaults, README, runtime Enter recording, capture flags, and no private scene handoff.                                                                                                                                    |
...
| `wam_ikfast`                | Restored by strict re-open.                                                                                                                          | Keep marker guards for WAM URDF loading, `SharedLibraryIkFast`, `lh_target`, activation/reset/print keys, defaults, camera, kinematic no-world-step execution, body-node drag handles, README, capture, and no private scene handoff.                                                                                                 |
