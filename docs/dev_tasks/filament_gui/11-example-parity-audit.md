...
source-owned posture/balance recovery. The remaining named gap is Shift
movement amplification through a modifier/key-state API.
...
| `hubo_puppet`               | Restored by strict re-open except Shift modifier gap.                                                                                                | Keep marker guards for load/start pose, analytical IK, target toggles, R balance controls, support actions, support polygon/centroid/COM overlays, camera/defaults, README, and no backend types.                                                                                                                                     |
