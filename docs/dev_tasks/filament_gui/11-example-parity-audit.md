...
| Create an interactive target at `(1.3, 0.75, 0.50)` with a 90-degree Y rotation.                                             | Restored with public target controls; one API gap.  | Current source uses one draggable `SimpleFrame`, a richer source-owned line handle, and keyboard rotation/reset actions. Exact mouse rotation rings/tools remain a renderer-neutral manipulation API gap.                   |
...
| Enable drag-and-drop of the target.                                                                                          | Restored for translation; API gap for mouse rings.  | Current runner supports Ctrl-left drag plus keyboard nudge controls on selectable frames, and the example adds target orientation keyboard controls. Exact mouse rotation rings/tools remain a public manipulation API gap. |
