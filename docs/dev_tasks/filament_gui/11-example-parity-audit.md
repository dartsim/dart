...
as explicit public API follow-ups.
...
| Toggle viewer headlights from the panel.                                                               | Public API gap.                                 | `PanelContext` does not expose renderer-neutral lighting/headlight state. Add a public lighting toggle before restoring this without backend hooks.          |
| Show camera Eye/Center/Up readout in the panel.                                                        | Public API gap.                                 | `PanelContext` does not expose live camera inspection data. Add public camera state access for panels before restoring exact readout parity.                 |
| Print custom keydown/keyup messages for `q`, `Q`, Left, and Right.                                     | Public API gap for key release; partially moot. | Promoted `KeyboardAction` supports keydown actions, but not key-release callbacks or OSG arrow key constants. Keep key-release callbacks as a named API gap. |
...
up vector remains a public camera API gap because `OrbitCamera` does not expose
roll/up-vector selection.
...
| Preserve `s` shadow toggle.                                                                                                                | Public API gap.                      | The promoted public GUI does not expose a renderer-neutral shadow technique toggle; keep this classified rather than reintroducing OSG. |
| Preserve 640x480 launch default and camera eye/target framing.                                                                             | Restored except exact up vector.     | `OrbitCamera` matches target/yaw/pitch/distance; the historical custom up vector remains a public camera roll/up-vector API gap.        |
...
| Drag/move the target and apply clamped force from picked point to target while simulating.                           | Restored.                            | Current force line and `preStep` force application use the restored picked point.                                                                             |
...
| Panel help text, simulation controls, gravity toggle, force slider, target buttons, add/delete buttons.              | Restored.                            | Current panel restores historical labels/layout through `PanelBuilder`.                                                                                       |
| Headlights On/Off checkbox.                                                                                          | Public API gap.                      | `PanelContext` still does not expose renderer-neutral lighting/headlight state.                                                                               |
...
| Keep marker coverage for controls, selected hit data, panel options/labels, README, defaults, and no backend types.  | Restored.                            | Marker guards cover the public selection-hit data, panel options, labels, README, defaults, runtime recording gap, and absence of private scene/defaultScene. |
...
| `atlas_puppet`              | Restored except target-activation/posture/balance/support-visualization gaps; R24 adds public `dart::gui::Gizmo` affordances plus X/Y/Z arrow, rotation-ring, and plane-handle dragging. | Keep remaining gaps explicit until repaired through source-owned state or public debug/IK APIs, and replace the old sphere affordance once gizmo highlighting/ownership is complete.                                                                                                                                                  |
...
| `box_stacking`              | Restored by strict re-open except named viewer API gaps.                                                                                                                                 | Keep marker guards for stack placement, floor geometry, solver/gravity controls, panel text, camera/defaults, README, and no backend types; keep headlight, camera readout, and key-release gaps explicit.                                                                                                                            |
...
| `fetch`                     | Restored by strict re-open after WAM IKFast.                                                                                                                                             | Keep marker guards for target manipulation, panel/help behavior, camera/defaults, README, `--screenshot`, `--out`, and no backend/private-scene handoff.                                                                                                                                                                              |
...
| `hardcoded_design`          | Restored except OSG wireframe public API gap.                                                                                                                                            | Keep number-key joint controls, direction toggle, camera/defaults, README/help text, and marker guards; keep wireframe as a render-style/debug API follow-up.                                                                                                                                                                         |
...
| `operational_space_control` | Restored by strict re-open except shadow-toggle and exact camera up-vector gaps.                                                                                                         | Keep marker guards for KR5 robot/ground, target ball, 1/2/3 axis aliases, defaults, camera, README, instructions, and no WAM/private-scene drift.                                                                                                                                                                                     |
