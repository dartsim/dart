...
| README documents summary, controls, promoted run/capture, build, and execute instructions.                  | Restored in this slice.              | README documents `pixi run ex wam_ikfast`, `--screenshot`, `--out`, kinematic no-world-step execution, and the drag-mode API gap.            |
...
| `wam_ikfast`                | Restored by strict re-open except exact drag-mode API gap.                                                                                           | Keep marker guards for WAM URDF loading, `SharedLibraryIkFast`, `lh_target`, activation/reset/print keys, defaults, camera, kinematic no-world-step execution, README, capture, and no private scene handoff; keep parent-joint drag-mode as a public API follow-up.                                                                  |
