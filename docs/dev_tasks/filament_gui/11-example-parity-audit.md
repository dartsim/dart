...
| Expose Dantzig/PGS LCP solver selection and split-impulse toggling without losing split-impulse state. | Restored through public `dart::gui`.            | Current source uses promoted panel controls plus a launch-time `--solver dantzig                                                                             | pgs` option. |
