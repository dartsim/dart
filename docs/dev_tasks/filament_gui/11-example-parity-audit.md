...
| `atlas_puppet`              | Restored except target-activation/posture/balance/support-visualization gaps; R24 adds public `dart::gui::Gizmo` affordances and X/Y/Z axis-arrow dragging. | Keep remaining gaps explicit until repaired through source-owned state or public debug/IK APIs, and replace the old sphere affordance once rotation/plane gizmo handles land.                                                                                                                                                         |
