...
| `rerun`                     | Removed after maintainer re-open of the no-source placeholder decision.            | Keep removal guards for absent directory, absent CMake subdirectory, and absent README entry. Also audit remaining Rerun support references after the active checkpoint; future Rerun integration should return only as a real sourced example with a concrete use case or known downstream user. |
