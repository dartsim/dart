...
| Create an interactive target at `(1.3, 0.75, 0.50)` with a 90-degree Y rotation.                                             | Restored with promoted selection geometry.          | Current source uses one draggable `SimpleFrame` instead of OSG `InteractiveFrame`.                                                                                |
| Show the target as the visible end-effector control affordance.                                                              | Restored.                                           | Current source uses two transparent green `LineSegmentShape` bars to preserve the historical target wording without child renderables that detach from selection. |
