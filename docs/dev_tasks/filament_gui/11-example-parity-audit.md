...
| `X`/`C` support toggles, `R` posture optimization, `T` relaxed-pose reset, and `P` DOF printing.                | Restored except R solver behavior.   | X/C/P/T are public keyboard actions; hold/release R remains tied to the historical whole-body posture/balance solver.                         |
| Whole-body `RelaxedPosture` objective and `BalanceConstraint` recovery behavior.                                | Solver follow-up.                    | Current source uses active per-target Jacobian-transpose IK but does not restore the historical whole-body objective/constraint stack.        |
...
| Keep marker coverage for load/posture/targets/teleop/defaults/README/no OSG plus named remaining parity gaps.   | Restored for this slice.             | Marker guards now cover active target toggles, support controls, support/COM overlays, README/defaults, and the named R solver gap.           |
...
| Keep marker coverage for load/start pose, target toggles, support overlay, controls, defaults, README. | Restored for this slice.                  | Marker guards now cover the strict target/default/README contract plus the source-owned COM validity overlay.                   |
...
| `atlas_puppet`              | Restored except whole-body posture/balance solver gap; R24 adds public `dart::gui::Gizmo` affordances plus X/Y/Z arrow, rotation-ring, and plane-handle dragging. | Keep marker guards for active target toggles, support controls, support polygon/centroid/COM overlays, camera/defaults, README, and no backend types.                                                                                                                                                                                 |
...
| `hubo_puppet`               | Restored by strict re-open except balance/key-state solver gaps.                                                                                                  | Keep marker guards for load/start pose, target toggles, support actions, support polygon/centroid/COM overlays, camera/defaults, README, and no backend types.                                                                                                                                                                        |
