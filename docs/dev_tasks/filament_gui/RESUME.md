...
and primitive sphere/ellipsoid/cylinder surface hit points/normals. The example
wires those helpers into basic click-to-select highlighting with a selection
bounds overlay, and reports the selected DART shape in the built-in panel.
...
also use primitive sphere/ellipsoid/cylinder intersections for curved surface
hit points and normals before falling back to local bounds for other shape
descriptors.
