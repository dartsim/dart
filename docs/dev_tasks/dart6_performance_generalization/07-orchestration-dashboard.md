...
| --- | --- | --- | --- |
| WS-A constraint/LCP | 02-constraint-lcp-lane.md | PG.10–PG.15 | active/gated (PG.10 #3339; PG.13 evidence-gated by PG.10 census; PG.14 blocked D3; PG.15 evaluator active on #3353, default remediation still D7-gated) |
...
| WP-PG.15 creep vs rest-veto | WS-A | claimed — D7 evaluator on #3353; default fix open | `docs/close-dart6-performance-generalization` / #3353 | Restores the active tracker after mistaken retirement, adds `contact_benchmark` D7 tuning knobs (`--contact-max-erv`, `--sleep-contact-penetration-tolerance`), and adds focused sleeping-tolerance tests. Local S6 sweeps show ERV bounds penetration but does not by itself make the pile sleep; a 20 s `--contact-max-erv 0.1 --sleep-contact-penetration-tolerance 0.005` run ended finite with max penetration ~0.0037 m and 0/71 resting, so the next default decision must address dense-pile final-quiet jitter and/or D3 solve-side behavior before claiming the #3056 closer. |
