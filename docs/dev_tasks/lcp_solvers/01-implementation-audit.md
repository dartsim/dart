...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  6-/8-/12-/16-/24-/32-contact packets, ADMM/SAP/Boxed Semi-Smooth Newton
  single-problem 16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact packets, and 292 matching benchmark rows verified in default,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/' | wc -l`
  ...
  stronger/extreme-coupled packets.
...
- A separate focused SIMD-enabled DART 7 build configured with
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` on all 292 rows, and the
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  32-contact packets, ADMM/SAP/Boxed Semi-Smooth Newton single-problem
  16x-coupled mildly ill-conditioned
