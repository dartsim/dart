...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  all-solver 4-/5-/6-/7-sphere vertical-stack rows, scoped PGS, NNCG, and
  Red-Black Gauss-Seidel 8-/9-/10-sphere rows,
...
- `BM_LCP_COMPARE --benchmark_filter='BM_LcpWorldStackContact|BM_LcpWorldStackContactAssembly' --benchmark_min_time=0.001s --benchmark_repetitions=1`
  ...
  8-sphere subset (`Pgs`, `Dantzig`, `SymmetricPsor`, `BGS`,
  `RedBlackGaussSeidel`, `NNCG`, `SubspaceMinimization`, `Apgd`, `Tgs`,
  `Staggering`, `Admm`, `Sap`, and `BoxedSemiSmoothNewton`), plus 13
  `BM_LcpWorldStackContact/FrictionIndex/<solver>/{9,10}` rows for the same set
  including `RedBlackGaussSeidel`; focused probes reported `contract_ok=0` for
  ...
  runs report `contract_ok=1` for all 13 registered solver rows plus the
...
- DART 7 integration: current tests now include single-contact and two-contact
  ...
  `RedBlackGaussSeidel`, and `NNCG` rows, plus 8-/9-/10-sphere rows for the
  narrower passing solver subset, mixed contact-derived serial/parallel
  batches, the
...
- Performance breadth: manifest benchmarks are apples-to-apples math-layer
  ...
  7-sphere all-solver rows, scoped 8-/9-/10-sphere `Pgs`,
