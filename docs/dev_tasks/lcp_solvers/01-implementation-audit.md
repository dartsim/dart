...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  in single and batch rows, ADMM/SAP/Boxed Semi-Smooth Newton single-problem
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact packets, and 298 matching benchmark rows verified in default,
...
| Boxed semi-smooth Newton            | `02_overview.md`, `05_newton-methods.md`, `07_selection-guide.md` | `dart/math/lcp/newton/boxed_semi_smooth_newton_solver.hpp` exported by `all.hpp`            | S, B, F          | Smoke manifest checks; comparison harness covers standard, boxed, and friction-index fixtures; smoke tests include line-search failure reporting; generated coverage passes coupled mildly ill-conditioned 2-contact and 4-contact friction-index cases after the moving-bound Jacobian fix; generated coverage now also passes 8x-coupled mildly ill-conditioned friction-index packets through 32 contacts and the 16x-coupled slice through 64 contacts | Manifest registers all three benchmark families; focused `BM_LcpBoxedSemiSmoothNewtonLineSearchSweep` rows cover default line search, expanded line-search step budget, and gentle step-reduction settings on standard, boxed, and friction-index fixtures in default, SIMD-enabled, and CUDA-enabled build trees; focused `BM_LcpContactSolverComparisonSweep/BoxedSemiSmoothNewton/*` rows cover separated world-contact, coupled stack-contact, and articulated unified-contact fixtures in those build trees; focused `BM_LcpMildIllConditioned/StrongerCoupledFrictionIndex*/BoxedSemiSmoothNewton` single and batch rows cover 8x-coupled mildly ill-conditioned friction-index packets through 32 contacts; focused `BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex*/BoxedSemiSmoothNewton` single-problem rows cover 16x-coupled packets through 64 contacts | Still needs batch contract evidence for the 16x-coupled slice, longer-running/denser end-to-end contact, and direct backend execution evidence; CUDA-enabled line-search, contact-comparison, and stronger/extreme-coupling rows are CPU solver rows, not CUDA LCP kernel execution |
...
- `UNIT_math_lcp_math_lcp_lcp_generated_coverage` passed 21
  ...
  Semi-Smooth Newton included in the scoped solver set,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/' | wc -l`
  ...
  coupled packets, and `coupling_scale=4`, `8`, or `16` for the
  stronger/extreme-coupled packets. The 8x-coupled slice contributes 90
  single-problem rows and includes `BoxedSemiSmoothNewton`.
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditionedBatch' | wc -l`
  ...
  6-/8-/12-/16-/24-/32-/48-/64-contact mildly ill-conditioned packets. The rows report
  ...
  parallel execution counters on the `ParallelExecutor` rows. The 8x-coupled
  batch slice contributes 180 serial and parallel rows and includes
  `BoxedSemiSmoothNewton`; the 16x-coupled batch rows remain ADMM/SAP-only
  after the Boxed Semi-Smooth Newton contract trial failed. The
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex' | wc -l`
  ...
  Boxed Semi-Smooth Newton serial and parallel batch rows remain unregistered
  for this slice after a default-tree trial reported `contract_ok=0`. The
...
- A separate focused SIMD-enabled DART 7 build configured with
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` on all 298 rows, and the
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  coupled friction-index 24-, 32-, 48-, and 64-contact active-set packets, plus an
  4x/8x-coupled mildly ill-conditioned 6-/8-/12-/16-/24-contact plus 8x
  32-contact packets with Boxed Semi-Smooth Newton included in single and
  batch rows, ADMM/SAP/Boxed Semi-Smooth Newton single-problem 16x-coupled
