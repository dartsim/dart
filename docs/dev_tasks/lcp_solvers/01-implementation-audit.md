...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  single-problem
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact packets, and 338 matching benchmark rows verified in default,
...
- A separate focused SIMD-enabled DART 7 build configured with
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` on all 1014 single and batch rows, and the
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  32-/48-/64-contact packets with Boxed Semi-Smooth Newton included in single rows and
