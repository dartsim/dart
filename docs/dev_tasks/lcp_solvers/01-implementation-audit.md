...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-/96-contact packets, and 539 matching single-problem benchmark rows verified in default,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/' | wc -l`
  ...
  single-problem rows span coupling scales `1/4/8/16`; the 16x rows include
  the 96-contact packet and report the same tuned line-search counters as the
  batch rows.
...
- A separate focused SIMD-enabled DART 7 build configured with
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` on all 1617 single and batch rows, and the
