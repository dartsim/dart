...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  single-problem
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact packets, and 383 matching benchmark rows verified in default,
...
- `UNIT_math_lcp_math_lcp_lcp_generated_coverage` passed 21
  ...
  16-, and 24-contact packets, plus 4x-coupled 6-/8-/12-/16-/24-/32-/48-/64-contact
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex' | wc -l`
  ...
  for 6/8/12/16/24/32/48/64 contacts after focused tuned line-search checks
  passed. The
...
- A separate focused SIMD-enabled DART 7 build configured with
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` on all 1149 single and batch rows, and the
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  4x/8x-coupled mildly ill-conditioned 6-/8-/12-/16-/24-/32-/48-/64-contact packets with Boxed Semi-Smooth Newton included in single rows and
  coupled batch rows, ADMM/SAP/Boxed Semi-Smooth Newton
  single-problem
  16x-coupled
