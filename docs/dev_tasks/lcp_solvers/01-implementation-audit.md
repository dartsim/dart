...
- `BM_LCP_COMPARE --benchmark_filter='BM_LcpWorldBoxStep_BoxedLcp' --benchmark_min_time=0.001s --benchmark_repetitions=1`
  ...
  `BM_LcpWorldBoxStep_BoxedLcp/144/1`. The fifteen-row registry covers
  4/8/16/32/64/96/128/192/256/384/512/576 face contacts on the registered
  horizons.
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpStaggeringContactPipelineSweep' | wc -l`
  listed 19 Staggering contact-pipeline rows. Focused JSON runs for
  `BM_LcpStaggeringContactPipelineSweep` in default, SIMD-enabled, and
  CUDA-enabled build trees reported 19 rows with zero `contract_ok` failures
  across separated world-contact 1/2/4/8/16-contact fixtures, coupled
  vertical-stack 2/3/5/8/16-contact fixtures, and articulated unified ground,
  rigid-impact, and cross-link-impact 4-/8-/16-contact fixtures; the rows
  recorded `staggering_normal_friction_split=1`, normal-row counts
  `1/2/3/4/5/8/16`, friction-row counts `2/4/6/8/10/16/32`,
  coupled-contact flags, contact counts `1/2/3/4/5/8/16`, problem sizes
  `3/6/9/12/15/24/48`, and backend build-state counters.
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpContactSolverComparisonSweep' | wc -l`
  listed 57 ADMM/SAP/Boxed Semi-Smooth Newton contact comparison rows. Focused
  JSON runs for `BM_LcpContactSolverComparisonSweep` in default, SIMD-enabled,
  and CUDA-enabled build trees reported 57 rows with zero `contract_ok`
  failures across separated world-contact 1/2/4/8/16-contact fixtures, coupled
  vertical-stack 2/3/5/8/16-contact fixtures, and articulated unified ground,
  rigid-impact, and cross-link-impact 4-/8-/16-contact fixtures. The list-test
  evidence includes 15 `contact_count=16` rows. The rows recorded solver
  identity counters, 19 rows per solver, normal-row counts `1/2/3/4/5/8/16`,
  friction-row counts `2/4/6/8/10/16/32`, contact counts `1/2/3/4/5/8/16`,
  problem sizes `3/6/9/12/15/24/48`, and backend build-state counters. The
  combined Staggering/contact-comparison filter reported 76 rows with zero
  `contract_ok` failures, 20 `contact_count=16` rows, `simd_rows=76` in the
  SIMD-enabled build, and `cuda_rows=76` in the CUDA-enabled build. The
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpContactNormalStandardSweep' | wc -l`
  listed 156 contact-normal standard-LCP rows. Focused JSON runs for
  `BM_LcpContactNormalStandardSweep` in default, SIMD-enabled, and CUDA-enabled
  build trees reported 156 rows with zero `contract_ok` failures across
  ...
  4-/8-/16-contact fixtures. The list-test evidence includes 40 rows sourced from
  the five 8-contact fixtures and 40 rows sourced from the five 16-contact
  fixtures. The rows compare `Dantzig`, `Lemke`, `Baraff`,
  ...
  normal-row counts `1/2/3/4/5/8/16`, source contact problem sizes
  `3/6/9/12/15/24/48`, standard problem sizes `1/2/3/4/5/8/16`, coupled-fixture
