...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  48-, 64-, 96-, and 128-contact generated packets, plus 27 matching
  single-problem and 54 batch benchmark rows verified in default,
  ...
  friction-index 16-/24-/32-/48-/64-/96-/128-contact packets, plus 45
  matching benchmark rows verified in default, SIMD-enabled, and CUDA-enabled
  build trees. It also includes 60
  ...
  128-contact, 384-row friction-index packets, plus 96 matching
  benchmark rows verified in default, SIMD-enabled, and CUDA-enabled build
  trees. It now also includes 486 serial and DART 7 `ParallelExecutor` batch
  rows over the standard 32/64/128-row, boxed 32/64/128-row, and coupled
  friction-index 8-/12-/16-/24-/32-/48-/64-/96-/128-contact active-set packets, verified in
...
- `UNIT_math_lcp_math_lcp_lcp_generated_coverage` passed 21
  ...
  coupled friction-index 16-/24-/32-/48-/64-/96-/128-contact known-solution
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpProductionActiveSetTransition/' | wc -l`
  ...
  `problem_size=384`, `coupling_scale=32`, and `coupled=1`. The previous
  full-slice
  ...
  72-row, 32-contact, 96-row, 48-contact, 144-row, 64-contact, 192-row, 96-contact, 288-row, and 128-contact, 384-row friction-index
  active-set transition packets.
  The rows report
  `active_set_transition=1`, `production_active_set_transition=1`,
  `contact_count=24/32/48/64/96/128`, `problem_size=72/96/144/192/288/384`,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpProductionActiveSetTransitionBatch' | wc -l`
  ...
  32/64/128-row, and coupled friction-index 8-/12-/16-/24-/32-/48-/64-/96-/128-contact
  active-set packets. The rows
  report `production_active_set_transition_batch=1`, `batch_size=4`,
  `problem_size=24/32/36/48/64/72/96/128/144/192/288/384`,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpNearSingular/' | wc -l`
  reports 27 rows. Previous full-slice rows through the 96-contact packet
  ...
  report 2 rows with `contract_ok=1`. These rows cover near-singular standard
  8-row, boxed 8-row, and coupled friction-index 3-, 6-, 9-, 12-, 16-, 24-,
  32-, 48-, 64-, 96-, and 128-contact packets over the scoped robust solver set. The rows report
  `near_singular=1`, backend build-state counters, contact counts
  `3/6/9/12/16/24/32/48/64/96/128`, problem sizes `9/18/27/36/48/72/96/144/192/288/384`, and
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpNearSingularBatch' | wc -l`
  reports 54 rows. Previous JSON benchmark checks through the 96-contact
  ...
  same build trees report 4 batch rows with `contract_ok=1`.
  ...
  6-, 9-, 12-, 16-, 24-, 32-, 48-, 64-, 96-, and 128-contact packets. They cover batch-size-4
  ...
  `32/36/72/108/144/192/288/384/576/768/1152/1536`, backend build-state counters, and
  parallel execution counters on the `ParallelExecutor` rows. The
  friction-index rows additionally report
  `contact_count=3/6/9/12/16/24/32/48/64/96/128`,
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpSingularDegenerateFrictionIndexBatch' | wc -l`
  ...
  friction-index 6-, 8-, 12-, 16-, 24-, 32-, 48-, 64-, 96-, and 128-contact
  packets. The rows report `singular_degenerate_batch=1`, `rank_deficient=1`,
  `batch_size=4`, `contact_count=6/8/12/16/24/32/48/64/96/128`,
  `total_contact_count=24/32/48/64/96/128/192/256/384/512`,
  `problem_size=18/24/36/48/72/96/144/192/288/384`,
  `total_problem_size=72/96/144/192/288/384/576/768/1152/1536`, backend
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  friction-index 16-/24-/32-/48-/64-/96-/128-contact cases. Active-set transition coverage now includes
