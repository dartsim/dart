...
- Current evidence is mostly math-layer evidence, with DART 7
  ...
  in single rows and non-16x batch rows, ADMM/SAP/Boxed Semi-Smooth Newton
  single-problem
  16x-coupled
...
- `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditionedBatch' | wc -l`
  ...
  rows over coupling scales `1/4/8` and contact counts `6/8/12/16/24/32`; the
  16x-coupled batch rows remain ADMM/SAP-only after the Boxed Semi-Smooth
  Newton contract trial failed. The
...
- Correctness breadth: the comparison fixtures prove known small solutions, and
  ...
  32-contact packets with Boxed Semi-Smooth Newton included in single rows and
  non-16x batch rows, ADMM/SAP/Boxed Semi-Smooth Newton single-problem
  16x-coupled
