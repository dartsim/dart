...
link-vs-link LCP snapshots and 256-contact packets. It now also adds mixed
...
unified-contact all-solver benchmark registration through 256-contact,
768-row link-ground, link-vs-dynamic-rigid, and cross-multibody link-vs-link
packets. Focused default, SIMD-enabled, and CUDA-enabled build-tree filters
over those 256-contact rows report `contract_ok=1` for all 48 rows, with
expected backend build-state counters. The focused summaries report zero
...
articulated evidence from 1536 to 2048 contacts/pairs/chains for fixed-base
...
regression tests pass for the four 2048-case public-step tests, and the matching
focused benchmark filter reports 6 rows with `invariant_ok=1`,
`contact_count=2048`, zero failures, and expected build-state counters. The
384+ link-ground and Cartesian-chain scene factories size the positive-quadrant
ground support from the grid dimensions after a fixed-ground probe clipped the
first 384-contact attempt to 289 and 272 contacts. These
...
reports `build_cuda_enabled=1`. The new
`BM_LcpWorldBoxStep_BoxedLcp/128/75` row reports `invariant_ok=1`,
`dense_box_contact=1`, `box_count=128`, `contact_count=512`, `step_count=75`,
`min_tangential_speed_drop=0.0201629`, `max_height_error<=2.77e-4`, and
`max_vertical_speed<=9.89e-2` in default, SIMD-enabled, and CUDA-enabled build
trees; the default and CUDA-enabled rows are close to the strict `0.1`
vertical-rest gate at `max_vertical_speed=0.098815`, the SIMD row reports
...
all-solver rows from 192-contact to 256-contact packets for link-ground,
link-vs-dynamic-rigid, and cross-multibody link-vs-link snapshots. Focused
default, SIMD-enabled, and CUDA-enabled filters over the new 256-contact rows
all report `contract_ok=1`, `contact_count=256`, `problem_size=768`, and the
expected backend build-state counters. The focused summaries report zero
residual, complementarity, and bound-violation maxima. The CUDA-enabled rows
are CPU solver rows in a CUDA-enabled build, not CUDA LCP kernel execution.
...
- `BM_LcpWorldBoxStep_BoxedLcp/{1,2,4,8}/200` plus
  ...
  `build_simd_enabled=1`, and the CUDA-enabled row reported
  `build_cuda_enabled=1`. Focused default, SIMD-enabled, and CUDA-enabled
  128-box 75-step rows reported `invariant_ok=1`, `contact_count=512`,
  `step_count=75`, `min_tangential_speed_drop=0.0201629`,
  `max_height_error<=2.77e-4`, and `max_vertical_speed<=9.89e-2`; the default
  and CUDA-enabled rows were close to the strict `0.1` vertical-rest gate at
...
- `BM_LcpWorldArticulatedGroundStep_BoxedLcp/{1,4,8,16,24,32,64,96,128,192,256,384,512,768,1024,1536,2048}/200` rebuild fixed-base
  ...
  2048-contact/pair follow-up filter reported 6 rows with `invariant_ok=1` and
  zero failures in default, SIMD-enabled, and CUDA-enabled build trees.
- `BM_LcpWorldArticulatedRigidImpactStep_BoxedLcp/{1,4,8,16,24,32,64,96,128,192,256,384,512,768,1024,1536,2048}/1` rebuild
  ...
  now cover `{16,32,64,96,128,192,256,384,512,768,1024,1536,2048}/200`. The 2048-pair
  rigid-impact rows report `contact_count=2048`,
  `articulated_link_count=2048`, `max_momentum_error=0`, and
  `min_target_velocity=0.786667`.
- `BM_LcpWorldArticulatedLinkImpactStep_BoxedLcp/{1,4,8,16,24,32,64,96,128,192,256,384,512,768,1024,1536,2048}/1` rebuild
  cross-multibody fixed-base prismatic striker/target link worlds, enters
  simulation mode inside the world factory so link kinematics are current
  before `World::collide()`, advances one public boxed-LCP `World::step()`,
  and reports end-to-end invariant counters. The registered long-horizon rows
  now cover `{16,32,64,96,128,192,256,384,512,768,1024,1536,2048}/200`. The 2048-pair
  cross-link rows report `contact_count=2048`,
  `articulated_pair_count=2048`, `articulated_link_count=4096`,
  `articulated_dof_count=4096`,
...
- `BM_LcpWorldArticulatedCartesianGroundStep_BoxedLcp/{1,4,8,16,24,32,64,96,128,192,256,384,512,768,1024,1536,2048}/200`
  ...
  focused 2048-chain follow-up passed the unit regression in all three build
  trees and its benchmark row reported `invariant_ok=1`,
  `cartesian_chain_count=2048`, `contact_count=2048`,
  `articulated_dof_count=6144`, `step_count=200`, `max_height_error=1e-4`,
  `max_abs_joint_velocity=4.45e-15`, and `max_planar_joint_speed=0`. The SIMD
  2048-contact/pair follow-up reported 6 rows with `build_simd_enabled=1`, and
...
- `BM_LcpArticulatedUnifiedContact/FrictionIndex/{Ground,RigidImpact,CrossLinkImpact}/<solver>/{1,4,8,16,24,32,48,64,96,128,192,256}`
  ...
  `multibody_count=128/256`, and expected backend build-state counters. The
  ...
  `multibody_count=192/384`, and expected backend build-state counters. The
  focused row summaries reported zero residual, complementarity, and
  bound-violation maxima in default, SIMD-enabled, and CUDA-enabled build
  trees, with 48 `build_simd_enabled=1` rows and 48 `build_cuda_enabled=1`
  rows in the corresponding builds.
  ...
  `multibody_count=256/512`, and expected backend build-state counters. The
  focused row summaries reported zero residual, complementarity, and
  bound-violation maxima in default, SIMD-enabled, and CUDA-enabled build
  trees, with 48 `build_simd_enabled=1` rows and 48 `build_cuda_enabled=1`
  rows in the corresponding builds.
