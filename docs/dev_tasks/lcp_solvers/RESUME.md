...
6-/8-/12-/16-/24-/32-/48-/64-contact packets, and 308 matching
`BM_LcpMildIllConditioned` benchmark rows verified in
default, SIMD-enabled, and CUDA-enabled build trees. It also
adds 616 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` rows for batch-size-4
...
reported in counters, verified in default, SIMD-enabled, and CUDA-enabled
build trees.
It also adds 46 `BM_LcpNearSingularBatch(Serial|Parallel)` rows for
...
3-/6-/9-/12-/16-/24-/32-/48-/64-contact packets, verified in default,
...
It now also adds an ADMM/SAP/Boxed Semi-Smooth Newton 16x-coupled mildly
ill-conditioned friction-index single-problem slice: the generated
known-solution coverage and
`BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex*` benchmark rows now
include a 64-contact packet. Focused default, SIMD-enabled, and CUDA-enabled
build-tree runs passed
`LcpGeneratedCoverage.LargerMildlyIllConditionedKnownSolutionsForScopedSolvers`
with the 64-contact generated case included, and passed with `contract_ok=1`
on the 64-contact single rows for `Admm`, `Sap`, and
`BoxedSemiSmoothNewton`, plus the ADMM/SAP serial and parallel batch rows. The
benchmark rows recorded
`coupling_scale=16`, `contact_count=64`, `problem_size=192`,
`total_contact_count=256`, `total_problem_size=768`, and backend build-state
counters. Later focused batch checks register Boxed Semi-Smooth Newton for the
6-/8-/12-/16-/24-/32-/48-/64-contact 16x rows with tuned line-search settings
after the default settings failed the 24-/64-contact batch contract. The
...
friction-index 64-contact packets and adds 23 matching `BM_LcpNearSingular`
benchmark rows for standard 8-row, boxed 8-row, and coupled friction-index
3-, 6-, 9-, 12-, 16-, 24-, 32-, 48-, and 64-contact packets verified in default,
...
packets, plus 39 matching `BM_LcpExtremeSingularDegenerate` benchmark rows
...
8-/12-/16-/24-/32-/48-/64-contact active-set packets, verified in the default,
SIMD-enabled, and CUDA-enabled build trees. The CUDA-enabled rows are CPU
solver rows in a CUDA-enabled build, not CUDA LCP kernel execution.
...
Move from the now-verified extreme 128-row/64-contact exact rank-deficient,
production active-set transition 64-contact, coupled mildly ill-conditioned
24-contact, near-singular 64-contact CPU solver rows, near-singular
...
standard-LCP sweep rows, plus the ADMM/SAP/Boxed Semi-Smooth Newton
single-problem 16x-coupled mildly ill-conditioned friction-index slice, toward broader solver-specific
...
- Current proof is intentionally narrow: 10 all-solver smoke/advanced tests, 44
  ...
  16-/24-/32-/48-/64-contact packets over the same scoped robust solver set.
  ...
  72-row, 32-contact, 96-row, 48-contact, 144-row, and 64-contact, 192-row friction-index active-set packets over all 16
  ...
  `contact_count=24/32/48/64`, `problem_size=72/96/144/192`, `coupling_scale=2/4/8/16`, and
  `coupled=1`. The CUDA-enabled rows are CPU solver rows in a CUDA-enabled
  build, not CUDA LCP kernel execution.
  `BM_LCP_COMPARE` also lists 422
  `BM_LcpProductionActiveSetTransitionBatch(Serial|Parallel)` rows for
  batch-size-4 serial and DART 7 `ParallelExecutor` runs over standard
  32/64/128-row, boxed 32/64/128-row, and coupled friction-index
  8-/12-/16-/24-/32-/48-/64-contact active-set packets. Focused default,
  SIMD-enabled, and CUDA-enabled JSON checks report 422 rows with
  `contract_ok=1`, 211 serial rows, and 211 parallel rows; the SIMD run reports
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
  `BM_LCP_COMPARE` also lists 39 `BM_LcpExtremeSingularDegenerate` rows for
  exact rank-deficient standard 128-row, boxed 128-row, and coupled
  friction-index 16-/24-/32-/48-/64-contact packets over the generated extreme
  ...
  contact counts `16/24/32/48/64`, problem sizes `48/72/96/144/192`, and `coupled=1` for the
  coupled friction-index packets. The
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
  `BM_LCP_COMPARE` also lists 308 `BM_LcpMildIllConditioned` rows for standard
  ...
  coupled single-problem rows contribute 24 rows over coupling scales
  ...
  6-/8-/12-/16-/24-/32-contact with Boxed Semi-Smooth Newton included across
  those coupled rows, including the 16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact rows; focused default, SIMD-enabled, and
  ...
  contribute 48 serial and parallel rows over coupling scales `1/4/8/16` and
  contact counts `6/8/12/16/24/32/48/64`. The 16x-coupled Boxed Semi-Smooth
  Newton rows report `boxed_ssn_max_line_search_steps=50`,
  `boxed_ssn_step_reduction=0.8`, and
  `boxed_ssn_jacobian_regularization=1e-8`.
  `BM_LCP_COMPARE` also lists 23 `BM_LcpNearSingular` rows for standard
  8-row, boxed 8-row, and coupled friction-index 3-, 6-, 9-, 12-, 16-, 24-,
  32-, 48-, and 64-contact
  ...
  8-row, and coupled friction-index 3-/6-/9-/12-/16-/24-/32-/48-/64-contact
...
- `BM_LCP_COMPARE` rows now report `build_simd_enabled`,
  ...
  308 focused `BM_LcpMildIllConditioned` rows, all 49 focused
  ...
  `BM_LcpProductionActiveSetTransition` rows, and all 39 focused
  ...
  all 308 rows. The focused `BM_LcpStressActiveSetTransition` slice also passes
  with `build_cuda_enabled=1`, `build_simd_enabled=0`, and `contract_ok=1` on
  ...
  `production_active_set_transition_batch=1`, and `contract_ok=1` on all 422
  rows, and the focused
  `BM_LcpExtremeSingularDegenerate` slice passes with
  `build_cuda_enabled=1`, `build_simd_enabled=0`, `rank_deficient=1`, and
  `contract_ok=1` on all 39 rows. The same CUDA tree now also builds
...
- `tests/unit/math/lcp/test_lcp_generated_coverage.cpp` is the current
  ...
  6-/8-/12-/16-/24-contact and 8x-coupled 6-/8-/12-/16-/24-/32-contact with
  ...
  ill-conditioned 6-/8-/12-/16-/24-/32-/48-/64-contact,
  near-singular standard 8-row,
  boxed 8-row, coupled friction-index 3-, 6-, 9-, 12-, 16-, 24-, 32-, 48-, and
  64-contact, and exact rank-deficient
  ...
  128-row, and coupled friction-index 16-/24-/32-/48-/64-contact known-solution cases, plus
  ...
  stronger-coupled friction-index 24-/32-/48-/64-contact known-solution cases for
...
- The generated LCP coverage now includes ADMM/SAP/Boxed Semi-Smooth Newton
  16x-coupled mildly ill-conditioned friction-index packets at 6, 8, 12, 16,
  24, 32, 48, and 64 contacts, and `BM_LCP_COMPARE` now lists 24 matching
  `BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex*` single-problem rows.
  Default, SIMD, and CUDA-enabled focused benchmark runs report
  `contract_ok=1`,
  `coupling_scale=16`, contact counts
  `6/8/12/16/24/32/48/64`, problem sizes `18/24/36/48/72/96/144/192`, and
  backend build-state counters. The 64-contact focused run covers 7 rows:
  three single-problem rows for `Admm`, `Sap`, and `BoxedSemiSmoothNewton`,
  plus ADMM/SAP serial and parallel batch rows; later focused batch checks add
  Boxed Semi-Smooth Newton rows at 6/8/12/16/24/32/48/64 contacts with tuned
  line-search settings. Treat the CUDA-enabled rows
...
- The benchmark target now also registers 422
  ...
  32/64/128-row, boxed 32/64/128-row, and coupled friction-index
  8-/12-/16-/24-/32-/48-/64-contact active-set packets. Focused JSON benchmark
  checks in default, SIMD-enabled, and CUDA-enabled build trees report 422 rows
  with `contract_ok=1`. Each row reports
  `production_active_set_transition_batch=1`, `batch_size=4`,
  `problem_size=24/32/36/48/64/72/96/128/144`, and
  `total_problem_size=96/128/144/192/256/288/384/512/576/768`; friction-index rows
  also report `contact_count=8/12/16/24/32/48/64`,
...
- `tests/benchmark/lcpsolver/bm_lcp_compare.cpp` registers 72 scoped dense box
  ...
  tree, not CUDA LCP kernel execution.
- `tests/benchmark/lcpsolver/bm_lcp_compare.cpp` also registers 36 dense
  ...
  with `contract_ok=1`, `dense_box_contact_batch=1`, `box_count=96`,
  ...
  rows in that build tree, not CUDA LCP kernel execution.
...
- CUDA was enabled and validated through the documented DART 7
  ...
  reports `cuda_dense_box_contact_batch=1` and `contract_ok=1`; the 96-box row
  reports `box_count=96`, `contact_count=384`, `problem_size=1152`,
  `batch_size=4`, `total_contact_count=1536`, `total_body_count=384`, and
  `total_problem_size=4608`. `BM_LcpCudaPgsWorldBoxContactGroupedBatch_FrictionIndex/2`
  ...
  probe failed two 48-box variants at 1024 fixed iterations with
...
./build/cuda/cpp/Release/bin/BM_LCP_COMPARE \
  ...
  "--benchmark_repetitions=1"
./build/default/cpp/Release/bin/BM_LCP_COMPARE \
  "--benchmark_filter=^BM_LcpWorldBoxContactBatch(Serial|Parallel)/FrictionIndex/.+/96/4$" \
  "--benchmark_min_time=0.001s" \
  "--benchmark_repetitions=1"
```
