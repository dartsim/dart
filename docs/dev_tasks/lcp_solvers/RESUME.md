...
packets, plus 30 matching `BM_LcpExtremeSingularDegenerate` benchmark rows
...
- Current proof is intentionally narrow: 10 all-solver smoke/advanced tests, 44
  ...
  16- and 24-contact packets over the same scoped robust solver set.
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
  `BM_LCP_COMPARE` also lists 30 `BM_LcpExtremeSingularDegenerate` rows for
  exact rank-deficient standard 128-row, boxed 128-row, and coupled
  friction-index 16- and 24-contact packets over the generated extreme
...
- `BM_LCP_COMPARE` rows now report `build_simd_enabled`,
  ...
  `build_cuda_enabled=1`, `build_simd_enabled=0`, `rank_deficient=1`, and
  `contract_ok=1` on all 30 rows. The same CUDA tree now also builds
...
- `tests/unit/math/lcp/test_lcp_generated_coverage.cpp` is the current
  ...
  128-row, and coupled friction-index 16- and 24-contact known-solution cases, plus
