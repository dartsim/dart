...
link-vs-link LCP snapshots and 128-contact packets. It now also adds mixed
...
unified-contact all-solver benchmark registration through 128-contact,
384-row link-ground, link-vs-dynamic-rigid, and cross-multibody link-vs-link
packets. Focused default, SIMD-enabled, and CUDA-enabled build-tree filters
over those 128-contact rows report `contract_ok=1` for all 48 rows, with
...
all-solver rows from 96-contact to 128-contact packets for link-ground,
link-vs-dynamic-rigid, and cross-multibody link-vs-link snapshots. Focused
default, SIMD-enabled, and CUDA-enabled filters over the new 128-contact rows
all report `contract_ok=1`, `contact_count=128`, `problem_size=384`, and the
expected backend build-state counters. The focused summaries report zero
...
- `BM_LcpArticulatedUnifiedContact/FrictionIndex/{Ground,RigidImpact,CrossLinkImpact}/<solver>/{1,4,8,16,24,32,48,64,96,128}`
  ...
  72-row, 96-row, 144-row, 192-row, 288-row, and 384-row LCPs. The cross-link rows complete a second articulated endpoint for a separate
