...
The DART 7 LCP solver evidence campaign is complete for this PR. The branch
contains manifest-driven correctness and benchmark coverage for all 26 LCP
solvers across standard, boxed, friction-index, near-singular, rank-deficient,
ill-conditioned, batch, SIMD-enabled, threaded CPU, CUDA-enabled build-tree,
and direct CUDA Jacobi/PGS batch slices, plus end-to-end public simulation
evidence through the documented scoped contact systems. The remaining
open-ended expansion areas have been scope-capped in `README.md` and are
deferred to follow-up planning/issue tracking instead of more evidence rows.

## Current Branch

`feature/dart7-lcp-solver-evidence` - local evidence branch ready for PR review
after final lint, build, and unit-test validation. The branch intentionally keeps
`docs/dev_tasks/lcp_solvers/` as working evidence documentation; cleanup is
reserved for the merge/completion PR step.

## Immediate Next Step

Open the evidence PR against `main`, then file the deferred-work issue for the
scope-capped stretch items. Do not add more evidence rows, sizes, contacts, or
solver expansions on this branch.
...
- `tests/unit/math/lcp/test_lcp_generated_coverage.cpp` is the current
  ...
  friction-index cases over exact-solution scoped solvers, batch-shaped solver
  reuse across families, pivoting-solver near-singular standard cases, and invalid
...
Then keep the branch in closeout mode: open the evidence PR, file the deferred
coverage issue, and stop adding evidence rows or solver expansions.
