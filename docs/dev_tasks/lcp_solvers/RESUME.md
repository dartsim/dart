...
cross-contact coupling, and 188 matching `BM_LcpMildIllConditioned` benchmark
...
friction-index slice: the generated known-solution coverage includes 16- and
24-contact packets, and `BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex*`
adds 4 benchmark rows for the same packets. Focused default, SIMD-enabled, and
CUDA-enabled build-tree runs passed with `contract_ok=1` on every benchmark row
and recorded `coupling_scale=16`, contact counts `16/24`, problem sizes
`48/72`, and backend build-state counters. The CUDA-enabled rows are CPU
...
- Current proof is intentionally narrow: 10 all-solver smoke/advanced tests, 44
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
  `BM_LCP_COMPARE` also lists 188 `BM_LcpMildIllConditioned` rows for standard
...
- `tests/unit/math/lcp/test_lcp_generated_coverage.cpp` is the current
  ...
  16-/24-contact plus a 32-contact 8x case, near-singular standard 8-row, boxed 8-row,
...
- The generated LCP coverage now includes ADMM/SAP-only 16x-coupled mildly
  ill-conditioned friction-index packets at 16 and 24 contacts, and
  `BM_LCP_COMPARE` now lists 4 matching
  ...
  `coupling_scale=16`, contact counts `16/24`, problem sizes `48/72`, and
