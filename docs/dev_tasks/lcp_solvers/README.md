...
- [x] Added Boxed Semi-Smooth Newton generated correctness and single/batch
      benchmark rows for 1x-/4x-coupled mildly ill-conditioned DART 7
      friction-index packets at 6, 8, 12, 16, and 24 contacts and 8x-coupled
      packets at 6, 8, 12, 16, 24, and 32 contacts, plus
      ADMM/SAP/Boxed Semi-Smooth Newton generated correctness and
      single-problem benchmark rows for 16x-coupled 6-, 8-, 12-, 16-, 24-,
      32-, 48-, and 64-contact packets. The 16x batch rows now include
      Boxed Semi-Smooth Newton at 6, 8, 12, 16, 24, 32, 48, and 64 contacts
      with tuned line-search settings reported in benchmark counters.
...
- [x] Added a larger mildly ill-conditioned generated known-solution slice for
      ...
      6-/8-/12-/16-/24-/32-contact cases with Boxed Semi-Smooth Newton
      included across those coupled rows, plus an
      ADMM/SAP/Boxed Semi-Smooth Newton 16x-coupled
      6-/8-/12-/16-/24-/32-/48-/64-contact single-problem slice. `MPRGP` is
      intentionally
...
- [x] Added production active-set transition coverage and 64
...
- [x] Added 308 `BM_LcpMildIllConditioned` benchmark rows for the scoped larger
      ...
      packets, 4x-coupled 6-/8-/12-/16-/24-contact packets, and 8x-coupled
      6-/8-/12-/16-/24-/32-contact packets with Boxed Semi-Smooth Newton
      included across those coupled rows, plus
      ADMM/SAP/Boxed Semi-Smooth Newton 16x-coupled
      6-/8-/12-/16-/24-/32-/48-/64-contact single-problem
      packets, verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 616 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` benchmark rows
      ...
      friction-index 6-/8-/12-/16-/24-contact, 4x-coupled
      6-/8-/12-/16-/24-contact, and 8x-coupled
      6-/8-/12-/16-/24-/32-contact with Boxed Semi-Smooth Newton included
      across those coupled rows, including the 16x-coupled
      6-/8-/12-/16-/24-/32-/48-/64-contact rows with tuned line-search
      settings reported in counters, verified in default,
      SIMD-enabled, and CUDA-enabled build trees.
...
- [x] Added extreme exact rank-deficient singular-degenerate coverage and 39
      `BM_LcpExtremeSingularDegenerate` rows for scoped standard 128-row,
      boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 48
      ...
      friction-index 6-/8-/12-/16-/24-/32-/48-/64-contact packets, verified in
      default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 192
...
- [x] Added DART 7 articulated unified-contact benchmark rows that compare all
      ...
      cross-multibody link-vs-link LCP snapshots through 24-contact packets.
...
- [x] Added DART 7 dense box-face contact evidence: a 4-contact, 12-row,
      ...
      `World::collide()`, APGD-verified in the unit test, plus 66 scoped
      `BM_LcpWorldBoxContact/FrictionIndex` benchmark rows over
      1/2/4/8/16/24/32/48/64/96/128-box snapshots verified in default, SIMD-enabled, and
      CUDA-enabled build trees.
...
- Verified focused SIMD-enabled CPU slice:
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` for all 308
...
- Generated correctness coverage:
  ...
  24-contact known-solution cases, plus an ADMM/SAP/Boxed Semi-Smooth Newton
  16x-coupled 6-/8-/12-/16-/24-/32-/48-/64-contact mildly ill-conditioned
  friction-index single-problem slice;
  ...
  friction-index 16-/24-/32-/48-/64-contact known-solution cases for solver sets selected by observed
...
- Verified SAP regularization benchmark slice:
  ...
  in a CUDA-enabled build, not CUDA LCP kernel execution.
- Verified ADMM/SAP/Boxed Semi-Smooth Newton extreme-coupling slice:
  `BM_LCP_COMPARE --benchmark_list_tests | rg '^BM_LcpMildIllConditioned/ExtremeCoupledFrictionIndex' | wc -l`
  now reports 24 single-problem rows, and focused checks for
  `BM_LcpMildIllConditioned(BatchSerial|BatchParallel)?/ExtremeCoupledFrictionIndex64`
  reported 7 rows with
  `contract_ok=1` in the default, SIMD-enabled, and CUDA-enabled build trees.
  The single-problem rows cover `Admm`, `Sap`, and `BoxedSemiSmoothNewton`.
  Later focused batch checks register Boxed Semi-Smooth Newton for the
  6-/8-/12-/16-/24-/32-/48-/64-contact 16x rows with tuned line-search
  settings after the default settings failed the 24-/64-contact batch
  contract. These rows cover 16x-coupled mildly ill-conditioned
...
- Verified production active-set transition benchmark slice:
  ...
  build-state counters, `contact_count=24/32/48/64`,
  `problem_size=72/96/144/192`, `coupling_scale=2/4/8/16`, and `coupled=1`. The CUDA-enabled rows are CPU solver
  rows in a CUDA-enabled build, not CUDA LCP kernel execution.
- Verified production active-set transition batch benchmark slice:
  ...
  8-/12-/16-/24-/32-/48-/64-contact active-set packets.
  The rows report `production_active_set_transition_batch=1`,
  `batch_size=4`, `problem_size=24/32/36/48/64/72/96/128/144`,
  `total_problem_size=96/128/144/192/256/288/384/512/576/768`, and backend
  build-state counters. Friction-index rows also report
  `contact_count=8/12/16/24/32/48/64`, `total_contact_count=32/48/64/96/128/192/256`,
  and `coupling_scale=1/2/4/8/16`. Parallel rows also report `profile_enabled=1`,
...
- Verified larger mildly ill-conditioned benchmark slice:
  ...
  friction-index 8-contact, and coupled friction-index 6-, 8-, 12-, 16-, and
  24-contact packets, plus 4x-coupled 6-/8-/12-/16-/24-contact packets and
  8x-coupled 6-/8-/12-/16-/24-/32-contact packets with Boxed Semi-Smooth
  Newton included across those coupled rows, plus ADMM/SAP/Boxed Semi-Smooth
  Newton single-problem
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact packets over the scoped solver set. The friction-index rows
  ...
  counters. The `BoxedSemiSmoothNewton` coupled single-problem rows contribute
  24 rows over coupling scales `1/4/8/16` and contact counts
  `6/8/12/16/24/32/48/64` alongside the prior scoped solvers. The 16x
...
- Verified larger mildly ill-conditioned batch benchmark slice:
  ...
  reported 616 rows, and JSON benchmark checks for
  `BM_LcpMildIllConditionedBatch(Serial|Parallel)` reported 616 rows with
  `contract_ok=1` in the default, SIMD-enabled, and CUDA-enabled build trees.
  These rows cover batch-size-4 serial and DART 7 `ParallelExecutor` runs over
  ...
  included across those coupled rows, including the 16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact rows. The rows report
  `mildly_ill_conditioned_batch=1`, `batch_size=4`, problem sizes
  `16/18/24/32/36/48/72/96/144/192`, total problem sizes
  `64/72/96/128/144/192/288/384/576/768`, and backend build-state counters. The
  ...
  `BoxedSemiSmoothNewton` coupled batch rows contribute 48 serial and parallel
  rows over coupling scales `1/4/8/16` and contact counts
  `6/8/12/16/24/32/48/64`. The 16x-coupled Boxed Semi-Smooth Newton rows
  report `boxed_ssn_max_line_search_steps=50`,
...
- Verified extreme singular-degenerate benchmark slice:
  ...
  ran 39 rows with `contract_ok=1` in the default, SIMD-enabled, and
  CUDA-enabled build trees. These rows cover exact rank-deficient standard
  128-row, boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-contact packets
  over the same scoped robust solver set as the generated extreme
  singular-degenerate correctness slice. The rows report
  `singular_degenerate=1`, `rank_deficient=1`, backend build-state counters,
  contact counts `16/24/32/48/64`, problem sizes `48/72/96/144/192`, and `coupled=1`
...
- Verified singular-degenerate friction-index batch benchmark slice:
  ...
  `total_problem_size=72/96/144/192/288/384/576/768`, and backend build-state
...
- DART 7 dense box-contact benchmark evidence:
  `tests/benchmark/lcpsolver/bm_lcp_compare.cpp` now registers 66 scoped
  `BM_LcpWorldBoxContact/FrictionIndex/<solver>/{1,2,4,8,16,24,32,48,64,96,128}` rows for
  `Pgs`, `RedBlackGaussSeidel`, `NNCG`, `Apgd`, `Tgs`, and `Admm`. Each solver
  uses the same 1/2/4/8/16/24/32/48/64/96/128-box dense face-contact snapshots,
  covering 4/8/16/32/64/96/128/192/256/384/512 contacts,
  12/24/48/96/192/288/384/576/768/1152/1536 rows, and
  1/2/4/8/16/24/32/48/64/96/128 dynamic bodies. Focused default, SIMD-enabled, and
  CUDA-enabled
  ...
  runs each reported `contract_ok=1`, `dense_box_contact=1`, `box_count=128`,
  `contact_count=512`, and `problem_size=1536` for the six new 128-box rows, with
...
- Added
  ...
  8x-coupled 6-/8-/12-/16-/24-/32-contact cases with Boxed Semi-Smooth Newton
  included across those coupled rows, plus ADMM/SAP/Boxed Semi-Smooth Newton
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact single-problem cases, over a scoped
  solver set.
...
- Added
  ...
  friction-index 16-/24-/32-/48-/64-contact known-solution cases over the same
...
- Added 308 `BM_LcpMildIllConditioned` rows for larger mildly ill-conditioned
  ...
  included across those coupled rows, plus ADMM/SAP/Boxed Semi-Smooth Newton
  16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact single-problem packets over the
...
- Added 616 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` rows for
  ...
  6-/8-/12-/16-/24-/32-contact with Boxed Semi-Smooth Newton included across
  those coupled rows, including the 16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-contact rows with tuned line-search settings
  reported in counters. These rows report
...
- Added 27 `BM_LcpStressSingularDegenerate` rows for exact rank-deficient
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 39 `BM_LcpExtremeSingularDegenerate` rows for exact rank-deficient
  ...
  The CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA
  LCP kernel execution.
- Added 48 `BM_LcpSingularDegenerateFrictionIndexBatch(Serial|Parallel)` rows
  for batch-size-4 serial and DART 7 `ParallelExecutor` runs over exact
  rank-deficient coupled friction-index 6-/8-/12-/16-/24-/32-/48-/64-contact
...
- Added 49 `BM_LcpExtremeActiveSetTransition` rows for the scoped scalable
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 64 `BM_LcpProductionActiveSetTransition` rows for stronger-coupled
  24-contact, 72-row, 32-contact, 96-row, 48-contact, 144-row, and
  64-contact, 192-row
  ...
  `production_active_set_transition=1`, `contact_count=24/32/48/64`,
  `problem_size=72/96/144/192`, `coupling_scale=2/4/8/16`, backend build-state counters,
...
- Added 422
  ...
  8-/12-/16-/24-/32-/48-/64-contact active-set packets. These rows
...
- Added DART 7 articulated unified-contact all-solver benchmark evidence for
  ...
  link-vs-link snapshots and 24-contact packets.
...
- Coverage breadth: extend deterministic generated fixtures beyond the current
  ...
  singular-degenerate through the current 128-row/64-contact slice, and
...
- Harder friction-index conditioning: the current all-solver generated
  ...
  well-conditioned 12-contact, mildly ill-conditioned 24-contact, 4x-coupled
  mildly ill-conditioned 6-, 8-, 12-, 16-, and 24-contact cases, 8x-coupled
  mildly ill-conditioned 6-, 8-, 12-, 16-, 24-, and 32-contact cases with
  ...
  ill-conditioned 6-, 8-, 12-, 16-, 24-, 32-, 48-, and 64-contact cases,
  ADMM/SAP batch rows for that 16x slice plus Boxed Semi-Smooth Newton batch
  rows at 6, 8, 12, 16, 24, 32, 48, and 64 contacts, near-singular 64-contact
  cases, singular-degenerate 64-contact cases, and production active-set
  transition 24-, 32-, 48-, and 64-contact cases. Broader harder-conditioned
  coupled friction-index grids beyond the current Boxed Semi-Smooth Newton
  coupled mild 1x/4x/8x/16x batch slices still need solver-specific evidence.
- Real-world cases: the current DART 7 world-contact evidence includes
  ...
  all-solver benchmark rows through 16 contacts for link-ground,
