...
- [x] Added a near-singular generated known-solution slice for a robust scoped
      ...
      friction-index 3-, 6-, 9-, and 12-contact cases.
...
- [x] Added a larger mildly ill-conditioned generated known-solution slice for
      ...
      24-contact cases with 4x and 8x cross-contact coupling. `MPRGP` is intentionally
      excluded from the stricter standard known-solution slice after the 32-row
      trial satisfied the LCP contract but missed the selected
      expected-solution tolerance.
...
- [x] Added production active-set transition coverage and 32
      `BM_LcpProductionActiveSetTransition` rows for stronger-coupled
      24-contact/72-row and 32-contact/96-row friction-index packets, verified
...
- [x] Added 64
      ...
      SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 170 `BM_LcpMildIllConditioned` benchmark rows for the scoped larger
      ...
      packets, and stronger-coupled 16-/24-contact packets with 4x and 8x
      cross-contact coupling, verified in default, SIMD-enabled, and
...
- [x] Added 56 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` benchmark rows
      ...
      SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 13 `BM_LcpNearSingular` benchmark rows for the scoped robust
      near-singular standard 8-row, boxed 8-row, and coupled friction-index
      3-, 6-, 9-, and 12-contact packets, verified in default, SIMD-enabled,
...
- [x] Added dense box-face CUDA contact-batch evidence for DART 7:
      homogeneous 4-problem and grouped variable-size 1/2/4-box batches of
      box-face `World::collide()` snapshots pass fixed-iteration CUDA PGS unit
      and benchmark coverage on the visible GPU. Fixed-iteration CUDA Jacobi was
      tried on the dense 4-contact patch and is not claimed: 4096 iterations
      with relaxation 1.0 failed the LCP contract with
...
- Generated correctness coverage:
  ...
  24-contact known-solution cases; near-singular standard 8-row, boxed 8-row,
  and coupled friction-index 3-, 6-, 9-, and 12-contact known-solution cases;
...
- Verified production active-set transition benchmark slice:
  ...
  over all 16 friction-index-capable manifest solvers. The rows report
  `active_set_transition=1`, `production_active_set_transition=1`, backend
  build-state counters, `contact_count=24/32`, `problem_size=72/96`,
  `coupling_scale=2/4`, and `coupled=1`. The CUDA-enabled rows are CPU solver
...
- Verified production active-set transition batch benchmark slice:
  ...
  the same stronger-coupled 24-contact/72-row and 32-contact/96-row packets.
  The rows report `production_active_set_transition_batch=1`,
  `batch_size=4`, `contact_count=24/32`, `total_contact_count=96/128`,
  `problem_size=72/96`, `total_problem_size=288/384`,
  `coupling_scale=2/4`, and backend build-state counters. Parallel rows also
...
- DART 7 dense box-contact CUDA batch evidence:
  ...
  snapshots and verifies fixed-iteration CUDA PGS against the LCP contract.
  `CudaLcpPgsBatch.DenseBoxWorldContactGroupedBatchSatisfiesLcpContract` extends
  that to grouped variable-size 1/2/4-box packets. The
  focused
  ...
  `dense_box_contact=1` for both rows. The homogeneous row reported
  `batch_size=4`, `total_contact_count=16`, `total_body_count=4`, and
  `total_problem_size=48`; the grouped row reported `batch_size=6`,
  ...
  failed Jacobi probe covered the same homogeneous 4-problem and grouped
...
- Added `LcpGeneratedCoverage.NearSingularKnownSolutionsForRobustSolverSlice`
  ...
  12-contact near-singular known-solution cases over a scoped robust solver
  set. Trial
...
- Added 13 `BM_LcpNearSingular` rows for near-singular standard 8-row, boxed
  8-row, and coupled friction-index 3-, 6-, 9-, and 12-contact packets over the
  generated robust near-singular solver scope. These rows report
...
- Added 49 `BM_LcpExtremeActiveSetTransition` rows for the scoped scalable
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 32 `BM_LcpProductionActiveSetTransition` rows for stronger-coupled
  24-contact, 72-row and 32-contact, 96-row friction-index active-set
  ...
  `production_active_set_transition=1`, `contact_count=24/32`,
  `problem_size=72/96`, `coupling_scale=2/4`, backend build-state counters,
...
- Added 64
  ...
  stronger-coupled 24-contact/72-row and 32-contact/96-row packets. These rows
...
- Harder friction-index conditioning: the current all-solver generated
  ...
  production active-set transition 24-contact and 32-contact cases. Harder
  conditioned coupled friction-index grids beyond this 8x-coupled slice still
  need solver-specific evidence.
...
- Benchmarks: extend `tests/benchmark/lcpsolver/` beyond generated math
  ...
  path-parameter sweep rows, ADMM rho/adaptive-rho sweep rows, SAP
