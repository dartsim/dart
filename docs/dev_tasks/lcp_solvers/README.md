...
- [x] Added coupled DART 7 world-contact stack benchmark rows for 2- and
      ...
      `RedBlackGaussSeidel`, plus 8-sphere rows for the narrower 10-solver
      passing subset, so the comparison now includes larger boxed/findex
...
- DART 7 world-contact evidence:
  ...
  run passed 11 stack tests, including the 3-/4-/5-/6-/7-/8-sphere snapshots
...
- DART 7 dense box-contact CUDA batch evidence:
  ...
  128-box batch-size-4 CUDA PGS row remains unclaimed. A fixed-iteration CUDA
  Jacobi dense-box trial failed the LCP contract, so Jacobi dense-box CUDA
  execution remains unclaimed. The earlier failed Jacobi probe covered the
  previous homogeneous 4-problem and
...
- DART 7 coupled world-contact stack benchmark evidence:
  ...
  `ShockPropagation`, so those rows are not claimed.
  ...
  `BM_LcpWorldStackContactAssembly_BoxedLcp/{2,3,4,5,6,7,8}`, which
...
- Added coupled DART 7 world-contact stack benchmark rows that compare all
  ...
  `RedBlackGaussSeidel`, plus 8-sphere rows for the narrower passing solver
...
- Real-world cases: the current DART 7 world-contact evidence includes
  ...
  solvers except `NNCG` and `RedBlackGaussSeidel`, 8-sphere stack rows for a
  narrower 10-solver subset, mixed serial and `ParallelExecutor` batch rows
  ...
  coupled multi-contact systems beyond the current 8-sphere snapshot, and
...
2. Extend DART 7 boxed-LCP world-contact evidence from current separated
   ...
   current 8-sphere vertical-stack snapshots and dense box face-contact
