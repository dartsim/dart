...
- [x] Added scoped solver generated correctness and single/batch benchmark rows
      ...
      16x-coupled packets through 128 contacts. Boxed
...
- [x] Added experimental CUDA LCP execution paths: fixed-iteration projected
      ...
      world-contact packets, homogeneous 5-/6-/7-sphere coupled stack-contact
      packets, grouped variable-size 1/2/4/8/16-contact sphere-ground packets,
      grouped variable-size 2/3/4/5/6/7-sphere coupled stack-contact packets, and
...
- [x] Added a larger mildly ill-conditioned generated known-solution slice for
      ...
      12-, 16-, 24-, 32-, 48-, and 64-contact cases, plus 1x-/4x-/8x-coupled
      6-/8-/12-/16-/24-/32-/48-/64-/96-contact cases and 16x-coupled
      6-/8-/12-/16-/24-/32-/48-/64-/96-/128-contact cases with Boxed
      Semi-Smooth Newton included across those coupled rows. `MPRGP` is
      intentionally
...
- [x] Added 486
      ...
      coupled friction-index 8-/12-/16-/24-/32-/48-/64-/96-/128-contact active-set packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 599 `BM_LcpMildIllConditioned` benchmark rows for the scoped larger
      ...
      16x-coupled 6-/8-/12-/16-/24-/32-/48-/64-/96-/128-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 1198 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` benchmark rows
...
- [x] Added 25 `BM_LcpNearSingular` benchmark rows for the scoped robust
      near-singular standard 8-row, boxed 8-row, and coupled friction-index
      3-, 6-, 9-, 12-, 16-, 24-, 32-, 48-, 64-, and 96-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 50 `BM_LcpNearSingularBatch(Serial|Parallel)` benchmark rows for
      batch-size-4 runs over near-singular standard 8-row, boxed 8-row, and
      coupled friction-index 3-, 6-, 9-, 12-, 16-, 24-, 32-, 48-, 64-, and
      96-contact packets, verified in default, SIMD-enabled, and CUDA-enabled
      build trees.
...
- [x] Added extreme exact rank-deficient singular-degenerate coverage and 42
      `BM_LcpExtremeSingularDegenerate` rows for scoped standard 128-row,
      boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-/96-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 54
      ...
      friction-index 6-/8-/12-/16-/24-/32-/48-/64-/96-contact packets, verified in
      default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 192
...
- [x] Extended DART 7 boxed-LCP coupled stack snapshot evidence to a 7-sphere,
      ...
      benchmark row, plus 14 friction-index solver benchmark rows. This excludes
      `NNCG` and `RedBlackGaussSeidel` at 7 spheres because focused probes did
      not satisfy the LCP contract for those rows; a 7-sphere public-step
...
- [x] Added coupled DART 7 world-contact stack benchmark rows for 2- and
      ...
      `RedBlackGaussSeidel`, so the comparison now includes larger boxed/findex
      snapshots where contacts share dynamic bodies.
...
- [x] Added contact-derived CUDA batch evidence for homogeneous 5-/6-/7-sphere
...
- [x] Extended contact-derived CUDA batch evidence to homogeneous and grouped
      variable-size 7-sphere DART 7 coupled stack-contact packets. Focused
...
- [x] Added grouped variable-size CUDA contact-batch evidence for DART 7
      ...
      visible GPU.
- [x] Added grouped variable-size CUDA contact-batch evidence for DART 7
      2/3/4/5/6/7-sphere coupled stack-contact packets, covering fixed-iteration
...
- Verified focused SIMD-enabled CPU slice:
  ...
  `BM_LcpMildIllConditioned` single and batch rows; the new 128-contact PGS
  single and batch rows also pass in that build tree. The focused
...
- Verified CUDA LCP execution slice:
  ...
  homogeneous contact-derived world-contact CUDA batches, homogeneous 5-/6-/7-sphere
  coupled stack-contact CUDA batches, and grouped variable-size 1/2/4/8/16-contact
  separated, 2/3/4/5/6/7-sphere coupled stack world-contact, and manually assembled
  ...
  separated sphere-ground and 2/3/4/5/6/7-sphere coupled stack CUDA batches. A
  ...
  `scripts/run_benchmark_smoke.py build/cuda/cpp/Release/bin/BM_LCP_COMPARE --benchmark_filter='BM_LcpCuda(Jacobi|Pgs)WorldStackContact(Batch_FrictionIndex/7/4|GroupedBatch_FrictionIndex/2$)' --benchmark_min_time=0.001s`
  reported four stack CUDA rows with `contract_ok=1`; the homogeneous 7-sphere
  rows reported `sphere_count=7`, `contact_count=7`, `problem_size=21`,
  `batch_size=4`, `total_contact_count=28`, and `total_problem_size=84`.
  A focused
  ...
  reported two stack CUDA rows with `contract_ok=1`, `batch_size=12`,
  `cuda_group_count=6`, `contact_shape_count=6`, `min_problem_size=6`,
  `max_problem_size=21`, `total_contact_count=54`, and
  `total_problem_size=162`. A focused articulated unified-contact follow-up
...
- Generated correctness coverage:
  ...
  24-contact known-solution cases, plus 1x-/4x-/8x-coupled
  ...
  friction-index 16-/24-/32-/48-/64-/96-contact known-solution cases for solver sets selected by observed
...
- Verified production active-set transition benchmark slice:
  ...
  reports 96 rows. Previous full-slice runs through the 96-contact packet
  ...
  `coupled=1`. The previous full-slice
  ...
  ran 534 production single and batch rows with `contract_ok=1` in the default, SIMD-enabled, and
  CUDA-enabled build trees. These rows cover stronger-coupled 24-contact,
  72-row, 32-contact, 96-row, 48-contact, 144-row, 64-contact, 192-row, and
  96-contact, 288-row friction-index
...
- Verified production active-set transition batch benchmark slice:
  ...
  `parallel_units=4` on parallel rows. These rows cover batch-size-4 serial and
  DART 7 `ParallelExecutor` runs over standard 32/64/128-row, boxed
  32/64/128-row, and coupled friction-index
  8-/12-/16-/24-/32-/48-/64-/96-/128-contact active-set packets.
  The rows report `production_active_set_transition_batch=1`,
  `batch_size=4`, `problem_size=24/32/36/48/64/72/96/128/144/192/288/384`,
  `total_problem_size=96/128/144/192/256/288/384/512/576/768/1152/1536`, and backend
  build-state counters. Friction-index rows also report
  `contact_count=8/12/16/24/32/48/64/96/128`, `total_contact_count=32/48/64/96/128/192/256/384/512`,
...
- Verified larger mildly ill-conditioned benchmark slice:
  ...
  set. A focused default/SIMD-enabled/CUDA-enabled smoke also passes the new
  128-contact PGS row with `contract_ok=1`, `contact_count=128`,
  `problem_size=384`, and `coupling_scale=16`. The friction-index rows
  ...
  coupling scales `1/4/8/16`; the 16x scale now includes the 128-contact packet and
...
- Verified larger mildly ill-conditioned batch benchmark slice:
  ...
  `64/72/96/128/144/192/288/384/576/768/1152/1536`, and backend build-state counters. The
  friction-index rows additionally report `contact_count=6/8/12/16/24/32/48/64/96/128`
...
- Verified near-singular batch benchmark slice:
  ...
  These rows cover batch-size-4 serial and DART 7 `ParallelExecutor` runs over
  near-singular standard 8-row, boxed 8-row, and coupled friction-index 3-, 6-,
  9-, 12-, 16-, 24-, 32-, 48-, 64-, and 96-contact packets. The rows report
  `near_singular_batch=1`, `batch_size=4`, problem sizes
  `8/9/18/27/36/48/72/96/144/192/288`, total problem sizes
  `32/36/72/108/144/192/288/384/576/768/1152`, and backend build-state counters. The
  friction-index rows additionally report `contact_count=3/6/9/12/16/24/32/48/64/96`,
  `total_contact_count=12/24/36/48/64/96/128/192/256/384`,
...
- Verified extreme singular-degenerate benchmark slice:
  ...
  ran 42 rows with `contract_ok=1` in the default, SIMD-enabled, and
  CUDA-enabled build trees. These rows cover exact rank-deficient standard
  128-row, boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-/96-contact packets
  over the same scoped robust solver set as the generated extreme
  singular-degenerate correctness slice. The rows report
  `singular_degenerate=1`, `rank_deficient=1`, backend build-state counters,
  contact counts `16/24/32/48/64/96`, problem sizes `48/72/96/144/192/288`, and `coupled=1`
...
- Verified singular-degenerate friction-index batch benchmark slice:
  ...
  reported 54 rows, and JSON benchmark checks for
  `BM_LcpSingularDegenerateFrictionIndexBatch(Serial|Parallel)` reported 54
  rows with `contract_ok=1` in the default, SIMD-enabled, and CUDA-enabled
  ...
  8-, 12-, 16-, 24-, 32-, 48-, 64-, and 96-contact packets. The rows report
  `singular_degenerate_batch=1`, `rank_deficient=1`, `batch_size=4`,
  `contact_count=6/8/12/16/24/32/48/64/96`,
  `total_contact_count=24/32/48/64/96/128/192/256/384`,
  `problem_size=18/24/36/48/72/96/144/192/288`,
  `total_problem_size=72/96/144/192/288/384/576/768/1152`, and backend build-state
...
- DART 7 coupled world-contact stack benchmark evidence:
  ...
  `BM_LcpWorldStackContactAssembly_BoxedLcp/{2,3,4,5,6}`, which
...
- Added
  ...
  8-contact, 1x-/4x-/8x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-/96-contact cases, and 16x-coupled
  6-/8-/12-/16-/24-/32-/48-/64-/96-/128-contact cases.
...
- Added
  ...
  friction-index 16-/24-/32-/48-/64-/96-contact known-solution cases over the same
...
- Added 599 `BM_LcpMildIllConditioned` rows for larger mildly ill-conditioned
  ...
  6-/8-/12-/16-/24-/32-/48-/64-/96-/128-contact single-problem packets. These rows are solve-to-tolerance benchmark
...
- Added 1198 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` rows for
...
- Added 25 `BM_LcpNearSingular` rows for near-singular standard 8-row, boxed
  ...
  execution.
- Added 50 `BM_LcpNearSingularBatch(Serial|Parallel)` rows for batch-size-4
  serial and DART 7 `ParallelExecutor` runs over near-singular standard 8-row,
  boxed 8-row, and coupled friction-index 3-/6-/9-/12-/16-/24-/32-/48-/64-/96-contact
...
- Added 27 `BM_LcpStressSingularDegenerate` rows for exact rank-deficient
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 42 `BM_LcpExtremeSingularDegenerate` rows for exact rank-deficient
  standard 128-row, boxed 128-row, and coupled friction-index 16-/24-/32-/48-/64-/96-contact
  ...
  The CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA
  LCP kernel execution.
- Added 54 `BM_LcpSingularDegenerateFrictionIndexBatch(Serial|Parallel)` rows
  for batch-size-4 serial and DART 7 `ParallelExecutor` runs over exact
  rank-deficient coupled friction-index 6-/8-/12-/16-/24-/32-/48-/64-/96-contact
...
- Added 49 `BM_LcpExtremeActiveSetTransition` rows for the scoped scalable
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 96 `BM_LcpProductionActiveSetTransition` rows for stronger-coupled
  24-contact, 72-row, 32-contact, 96-row, 48-contact, 144-row, and
  64-contact, 192-row, 96-contact, 288-row, and 128-contact, 384-row
  ...
  `production_active_set_transition=1`, `contact_count=24/32/48/64/96/128`,
  `problem_size=72/96/144/192/288/384`, `coupling_scale=2/4/8/16/32`, backend build-state counters,
...
- Added 486
  ...
  8-/12-/16-/24-/32-/48-/64-/96-/128-contact active-set packets. These rows
  report `contract_ok=1`, `production_active_set_transition_batch=1`,
  `batch_size=4`, `problem_size=24/32/36/48/64/72/96/128/144/192/288/384`,
  `total_problem_size=96/128/144/192/256/288/384/512/576/768/1152/1536`, backend build-state
  counters, and parallel execution counters on the `ParallelExecutor` rows.
  Friction-index rows also report `contact_count=8/12/16/24/32/48/64/96/128`,
...
- Added coupled DART 7 world-contact stack benchmark rows that compare all
  ...
  `RedBlackGaussSeidel`. Stack assembly/solve benchmark rows cover 2-, 3-, 4-,
  5-, 6-, and 7-sphere scenes.
...
- Added CUDA LCP execution evidence through
  ...
  CUDA unit and benchmark evidence, plus homogeneous 5-/6-/7-sphere and grouped
  variable-size 2/3/4/5/6/7-sphere coupled stack-contact CUDA unit and benchmark
...
- Coverage breadth: extend deterministic generated fixtures beyond the current
  ...
  active-set transition through the current stronger-coupled 128-contact slice
...
- Real-world cases: the current DART 7 world-contact evidence includes
  ...
  solvers except `NNCG` and `RedBlackGaussSeidel`, mixed serial and `ParallelExecutor`
...
1. Extend solver-specific friction-index conditioning/coupling grids beyond the
   current exact rank-deficient 128-row/96-contact and production active-set
   transition 128-contact slices.
