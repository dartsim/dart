...
- [x] Added scoped solver generated correctness and single/batch benchmark rows
      ...
      rows; SIMD/CUDA 192-contact batch rows remain unclaimed.
...
- [x] Added 518
      ...
      8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact active-set packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 614 `BM_LcpMildIllConditioned` benchmark rows for the scoped larger
      ...
      verified in default, SIMD-enabled, and CUDA-enabled build trees. The
      SIMD-enabled SAP 192-contact row is contract-correct but slow and reports
      20k iterations, so no speedup is claimed.
- [x] Added 1228 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` benchmark rows
      ...
      SIMD-enabled, and CUDA-enabled build trees. SIMD/CUDA 192-contact batch
...
- [x] Added extreme exact rank-deficient singular-degenerate coverage and 48
      `BM_LcpExtremeSingularDegenerate` rows for scoped standard 128-row,
      boxed 128-row, and coupled friction-index
      16-/24-/32-/48-/64-/96-/128-/192-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 66
      ...
      friction-index 6-/8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact packets,
      verified in default, SIMD-enabled, and CUDA-enabled build trees.
- [x] Added 192
...
- [x] Extended DART 7 boxed-LCP coupled stack snapshot evidence to an 8-sphere,
      ...
      and 10 friction-index solver benchmark rows. This excludes `NNCG`,
      `RedBlackGaussSeidel`, `Pgs`, `Jacobi`, `BlockedJacobi`, and
      `ShockPropagation` at 8 spheres because focused probes did not satisfy the
      LCP contract for those rows; no 8-sphere public-step invariant is claimed.
...
- [x] Added dense box-face CUDA contact-batch evidence for DART 7:
      homogeneous 4-problem 1-/4-/8-/16-box CUDA Jacobi rows, homogeneous
      4-problem 1-/4-/8-/16-/24-/32-/48-/64-/96-box CUDA PGS rows, and grouped
      variable-size 1/2/4/8/16/24/32/48-box PGS batches with two and three
      ...
      GPU; CUDA unit coverage now includes homogeneous Jacobi 1-/4-/8-/16-box
      packets, homogeneous PGS 1-/16-/24-/32-/48-/64-/96-box packets, the
      grouped 1/2/4/8/16/24/32/48-box PGS packet, and a bounded homogeneous
      128-box single-problem CUDA PGS packet.
      ...
      Broader dense-box CUDA Jacobi rows beyond 16 boxes and grouped dense-box
      CUDA Jacobi rows remain unclaimed.
...
- Verified focused CUDA-enabled DART 7 build/runtime slice:
  ...
  `has_simulation_experimental=1`, and `contract_ok=1` for the previous 1752
  `BM_LcpMildIllConditioned` single and batch rows; the new 192-contact single
  rows also pass in that build tree, while CUDA-enabled 192-contact batch rows
...
- Verified production active-set transition benchmark slice:
  ...
  `problem_size=576`, `coupling_scale=32`, and `coupled=1`. The previous
  full-slice
  ...
  192-contact, 576-row friction-index active-set transition packets over all
  16 friction-index-capable manifest solvers. The rows report
  `active_set_transition=1`, `production_active_set_transition=1`, backend
  build-state counters, `contact_count=24/32/48/64/96/128/192`,
...
- Verified production active-set transition batch benchmark slice:
  ...
  `coupling_scale=32`, and `parallel_units=4` on parallel rows. These rows
  cover batch-size-4 serial and DART 7 `ParallelExecutor` runs over standard
  32/64/128-row, boxed 32/64/128-row, and coupled friction-index
  8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact active-set packets.
  The rows report `production_active_set_transition_batch=1`,
  `batch_size=4`, `problem_size=24/32/36/48/64/72/96/128/144/192/288/384/576`,
  `total_problem_size=96/128/144/192/256/288/384/512/576/768/1152/1536/2304`, and backend
  build-state counters. Friction-index rows also report
  `contact_count=8/12/16/24/32/48/64/96/128/192`,
  `total_contact_count=32/48/64/96/128/192/256/384/512/768`,
...
- Verified larger mildly ill-conditioned benchmark slice:
  ...
  coupling scales `1/4/8/16`; the 16x scale now includes the 128-contact and
  192-contact packets and reports the same tuned line-search counters as the
  batch rows.
- Verified larger mildly ill-conditioned batch benchmark slice:
  ...
  on parallel rows. The SIMD/CUDA 192-contact batch rows are not claimed. The
  rows report
  ...
  `contact_count=6/8/12/16/24/32/48/64/96/128/192`
  and `total_contact_count=24/32/48/64/96/128/192/256/384/512/768`;
  coupled rows report
...
- Verified singular-degenerate friction-index batch benchmark slice:
  ...
  96-, 128-, and 192-contact packets. The rows report `singular_degenerate_batch=1`,
...
- DART 7 dense box-contact CUDA batch evidence:
  `CudaLcpJacobiBatch.DenseBoxWorldContactSmallBatchSatisfiesLcpContract`
  builds homogeneous 1-/4-/8-/16-box dense face-contact batches with four
  variants per box count and verifies fixed-iteration CUDA Jacobi against the
  LCP contract with 8192 iterations and relaxation 0.25.
  ...
  `BM_LCP_COMPARE --benchmark_filter='^BM_LcpCudaJacobiWorldBoxContactBatch_FrictionIndex/(1|4|8|16)/4$' --benchmark_min_time=0.001s --benchmark_repetitions=1 --benchmark_format=json`
  CUDA run reported 4 rows with `contract_ok=1`, `cuda_lcp_execution=1`,
  `cuda_batch_execution=1`, `cuda_dense_box_contact_batch=1`,
  `dense_box_contact=1`, `cuda_fixed_iterations=8192`,
  `cuda_relaxation=0.25`, `box_count=1/4/8/16`,
  `contact_count=4/16/32/64`, `problem_size=12/48/96/192`,
  `batch_size=4`, `total_contact_count=16/64/128/256`,
  `total_problem_size=48/192/384/768`, and
  `max_residual=max_complementarity` up to `6.94e-18`. The focused
  homogeneous PGS
  ...
  the bounded 1-/4-/8-/16-box homogeneous CUDA Jacobi rows pass. Broader
  dense-box CUDA Jacobi rows beyond 16 boxes and grouped dense-box CUDA Jacobi
  rows remain unclaimed.
- DART 7 coupled world-contact stack benchmark evidence:
  ...
  It also registers 10
  `BM_LcpWorldStackContact/FrictionIndex/<solver>/{8,9}` rows for `Dantzig`,
  `SymmetricPsor`, `BGS`, `SubspaceMinimization`, `Apgd`, `Tgs`,
  `Staggering`, `Admm`, `Sap`, and `BoxedSemiSmoothNewton`; focused 8-sphere
  probes reported `contract_ok=0` for `Pgs`, `Jacobi`, `BlockedJacobi`, and
  `ShockPropagation`, so those rows remain unclaimed at 8 and 9 spheres.
  ...
  `BM_LcpWorldStackContactAssembly_BoxedLcp/{2,3,4,5,6,7,8,9}`, which
...
- Added 614 `BM_LcpMildIllConditioned` rows for larger mildly ill-conditioned
  ...
  6-/8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact single-problem packets.
...
- Added 1228 `BM_LcpMildIllConditionedBatch(Serial|Parallel)` rows for
...
- Added 27 `BM_LcpStressSingularDegenerate` rows for exact rank-deficient
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 48 `BM_LcpExtremeSingularDegenerate` rows for exact rank-deficient
  standard 128-row, boxed 128-row, and coupled friction-index
  16-/24-/32-/48-/64-/96-/128-/192-contact packets over the generated extreme
  ...
  The CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA
  LCP kernel execution.
- Added 66 `BM_LcpSingularDegenerateFrictionIndexBatch(Serial|Parallel)` rows
  for batch-size-4 serial and DART 7 `ParallelExecutor` runs over exact
  rank-deficient coupled friction-index
  6-/8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact packets. These rows report
...
- Added 49 `BM_LcpExtremeActiveSetTransition` rows for the scoped scalable
  ...
  CUDA-enabled rows are CPU solver rows in a CUDA-enabled build, not CUDA LCP
  kernel execution.
- Added 112 `BM_LcpProductionActiveSetTransition` rows for stronger-coupled
  24-contact, 72-row, 32-contact, 96-row, 48-contact, 144-row, and
  64-contact, 192-row, 96-contact, 288-row, 128-contact, 384-row, and
  192-contact, 576-row friction-index active-set transition packets over every
  friction-index-capable manifest solver. These rows report `contract_ok=1`,
  `active_set_transition=1`,
  `production_active_set_transition=1`, `contact_count=24/32/48/64/96/128/192`,
  `problem_size=72/96/144/192/288/384/576`,
...
- Added 518
  ...
  32/64/128-row, boxed 32/64/128-row, and coupled friction-index
  8-/12-/16-/24-/32-/48-/64-/96-/128-/192-contact active-set packets. These rows
  ...
  `total_problem_size=96/128/144/192/256/288/384/512/576/768/1152/1536/2304`,
...
- Coverage breadth: extend deterministic generated fixtures beyond the current
  ...
  singular-degenerate through the current 128-row/192-contact slice, and
...
- Compute backends: current benchmark rows now self-report scalar/SIMD/CUDA
  ...
  dense box-face CUDA batches through 16 boxes, homogeneous PGS dense box-face
  CUDA batches through 96 boxes, two-/three-variant grouped PGS dense box-face
  CUDA batches through 48 boxes, plus a bounded 128-box homogeneous
  batch-size-1 CUDA PGS packet pass. The 128-box batch-size-4 CUDA PGS row and
  broader/grouped dense-box CUDA Jacobi rows remain unclaimed.
  ...
  CPU paths, general CUDA LCP solver execution, broader/grouped CUDA Jacobi
...
1. Extend solver-specific friction-index conditioning/coupling grids beyond the
   current exact rank-deficient 128-row/192-contact and production active-set
