...
uploaded bytes remain audited evidence for `333a`, but are not post-merge
...
| PR integration | PR #3374 is historically merged at `fa17fad`. PR #3377 is open and draft. Code/runtime checkpoint `6fb4d0dbd8c` contains current target `6c88ac1d774`; later documentation-only descendants do not change its binary identity. The inspected net diff had 264 files, no tracked task assets, and no generated video/CSV/JSON/log paths. Mutable head/check/review state must be verified live | #3377 is work in progress, not completion evidence. Fresh schema-v3 media binds captured checkpoint `333a809f14a`, demo `3d685d2c...`, core DART `3b66436b...`, and its resolved runtime closure; it is not current-head binary evidence |
...
| Source-row reseal | Historical `assets/pr_media_current_head_67073/` binds the compact Figures 1-2/4-5 and long Figure 6/7/five-level rows to its recorded pre-merge binary. Fresh `assets/pr_media_current_head_333a/` replaces Figure 3 with captured-checkpoint schema-v3 exact/boxed evidence; both finish 240 steps and pass independent verification and the bounded outcome slice | Historical members retain their audited media bytes but are not current-binary reuse evidence. Fresh Figure 3 exact/group hashes are `b2c268aa...` / `e321c711...`; both solvers pass, so no superiority or paper-parity claim follows |
...
| Captured-checkpoint Figure 3 schema-v3 reseal | Ignored `assets/pr_media_current_head_333a/` passes capture and independent verification for exact, boxed, and their synchronized checker-texture group. Both complete 240 steps and pass the bounded outcome slice. Manual inspection confirms the requested checker texture makes orientation change and spin visible. Exact/group MP4 SHA-256 values are `b2c268aa...` and `e321c711...` | Demo `3d685d2c...`, core DART `3b66436b...`, and the resolved runtime closure are bound unchanged at `333a`. Both solvers pass; no superiority, source-backend/full-trajectory, historical Figure 3, or paper-parity claim follows. Slots 02/16 are PR-referenced and re-download/hash verified. This is not `6fb4` current-binary evidence |
