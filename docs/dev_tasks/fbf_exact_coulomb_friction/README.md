...
The clean pushed current-head checkpoint is `333a809f14a`; it contains fetched
target `d65e88ebda4` through merge `d209da2cbf1`. The target subsequently
advanced to `6c88ac1d774` and must be merged before the next push; the merge
will create a new binary identity. Captured `dart-demos` and exact build-tree
core-DART SHA-256 values for checkpoint `333a809f14a` are `3d685d2c...` and
`3b66436b...`. Fresh schema-v3 Figure 3, Figure 8, and complete ten-level
evidence under `assets/pr_media_current_head_333a/` binds that full runtime
closure.
...
| PR integration | PR #3374 is historically merged at `fa17fad`. PR #3377 is open and draft at pushed checkpoint `333a809f14a`, containing target `d65e88ebda4` through merge `d209da2cbf1`. The inspected net diff has 264 files, no tracked task assets, and no generated video/CSV/JSON/log paths. Mutable head/check/review state must be verified live | #3377 is work in progress, not completion evidence. Fresh schema-v3 media binds demo `3d685d2c...`, core DART `3b66436b...`, and the resolved runtime closure |
...
| Browser-upload handoff | `PAPER_DEMO_VIDEO_MATRIX.md` owns 16 intended clips and the exact URL/hash ledger. All 16 are referenced from the draft PR and re-download/hash verified. Slot 09 is the fresh schema-v3 current-runtime group; the historical provisional bytes were replaced | All 16 published byte streams pass. Mandatory exact-only, proxy, non-strict, frozen-prefix, visual-sampling, and continuation-policy caption boundaries remain in force |
...
| Current-runtime Figure 3 schema-v3 reseal | Ignored `assets/pr_media_current_head_333a/` passes capture and independent verification for exact, boxed, and their synchronized checker-texture group. Both complete 240 steps and pass the bounded outcome slice. Manual inspection confirms the requested checker texture makes orientation change and spin visible. Exact/group MP4 SHA-256 values are `b2c268aa...` and `e321c711...` | Demo `3d685d2c...`, core DART `3b66436b...`, and the resolved runtime closure are bound unchanged. Both solvers pass; no superiority, source-backend/full-trajectory, historical Figure 3, or paper-parity claim follows. Slots 02/16 are PR-referenced and re-download/hash verified |
