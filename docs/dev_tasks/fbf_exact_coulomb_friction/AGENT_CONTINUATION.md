...
- The captured-checkpoint evidence runtime hashes are
  ...
  bytes but are not post-merge current-binary reuse evidence.
- Fresh captured-checkpoint Figure 3 exact/boxed capture and independent
...
| PR #3377 | Open and draft at the last live query. Post-merge code/runtime checkpoint `6fb4d0dbd8c` contains current target `6c88ac1d774`; later documentation-only descendants do not change its binary identity. The inspected net diff had 264 files, zero tracked task-asset paths, and zero generated video/CSV/JSON/log paths. Requery mutable head/check state before reporting it | WIP, not completion evidence. Fresh schema-v3 media binds captured checkpoint `333a809f14a`, demo SHA-256 `3d685d2c...`, and its runtime-library closure; it is not post-merge current-binary evidence |
...
| Captured-checkpoint Figure 3 schema-v3 reseal | Ignored `assets/pr_media_current_head_333a/` passes capture and independent verification for exact, boxed, and their synchronized checker-texture group. Both complete 240 steps and pass the bounded outcome slice; exact/group MP4 SHA-256 values are `b2c268aa...` / `e321c711...`. Manual inspection confirms the checker texture makes rotation visible | Captured demo SHA-256 `3d685d2c...`, exact core DART SHA-256 `3b66436b...`, and the resolved runtime closure are bound and unchanged. Both solvers pass; no superiority or paper-parity claim follows. Slots 02/16 are PR-referenced and re-download/hash verified. This is not `6fb4` current-binary evidence |
