...
`fbf_exact_coulomb_friction` dev task as of 2026-07-18. It supersedes older
...
threshold evidence while retaining their partial status; Fig. 5 and video.05
...
| Small figures and video | Incline `fig01_02_incline_current_v1`, Painleve `current_v1`, backspin `fig03_backspin_current_v3`, and author-pinned turntable `fig04_turntable_author_current_v1` are repository-finalized within narrow DART-only boundaries | Preserve and reverify all four small-figure bundles; retain the incline 8-versus-6 contact mismatch and strict-lane failure, then finalize only the remaining affected rows before promotion |
...
2. Preserve the P-core, standing-visual, finalized current-source Painleve-proxy,
   finalized incline and backspin, frozen impact-v1/arch101-v1 negatives, and
   card-manifold-v2 diagnostic while
