...
| Tables 6-7 / ten-level card house | Source-supported `fbf_author_card_house_10_impact_current_source`; `card_house_author_10_impact_current_source`. The older `fbf_paper_card_house_10_dynamic`; `card_house_10_dynamics` reconstruction remains diagnostic-only | Paired exact/boxed capture is automated with Native `FourPointPlanar`, capacity 4,096, and four contacts per pair. The new lane represents the pinned source's one `0.1 m` ground gap and 159 `0.005 m` card/cube gaps while keeping broad source gap semantics false | Blocked at strict completed step 1: exact records 18 attempts / 17 solves, one 39-contact max-iteration failure at residual `8.891154e-6` versus `1e-6`, zero fallbacks, and 264 total contacts, so capacity is not the immediate blocker. Boxed completes the same one-step control. The pinned source 30-frame control is finite but reports 87/120 non-converged substeps, first at source index 33. No historical Tables 6-7 invocation, strict source trajectory oracle, or corresponding source-video segment exists. See `CARD_HOUSE_10_CURRENT_SOURCE_DIAGNOSIS.md` | Blocked; no valid long media |
