...
data, primitive or mesh geometry, link gravity mode, passive joint dynamics
metadata (damping, Coulomb friction, spring reference, and spring stiffness),
local root/joint/shape poses, and explicit visual material colors. It also
checks unsupported-shape diagnostics, non-finite visual material diagnostics,
...
coverage beyond material colors/link gravity mode/passive joint dynamics/local
poses or choosing the next accepted writer target (URDF or PLAN-101 project
save/load).
