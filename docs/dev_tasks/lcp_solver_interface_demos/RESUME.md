...
- Row-list validation passed and listed representative manifest batch, grouped
  batch, world contact, world stack contact, and serial world-contact batch
  rows.
- The sampled manifest/grouped/contact rows passed with `contract_ok=1`; scalar
  world-contact rows also reported `solver_supports_problem=1`.
...
## Current Reality - 2026-06-14 Pathological Benchmark Runtime Guards
...
- Row-list validation passed and listed representative pathological scalar and
  serial batch rows.
- A focused singular friction-index serial batch row list passed and confirmed
  the concrete registered solvers are `Admm`, `Sap`, and
  `BoxedSemiSmoothNewton`.
- The sampled scalar and serial batch rows passed with `contract_ok=1`; scalar
  rows also reported `solver_supports_problem=1`.
- Lint, whitespace checks, and the default build passed.
...
## Current Reality - 2026-06-14 Parallel Batch Fixture Runtime Guard
...
- Row-list validation passed and listed representative generic parallel-batch
  rows across the shared fixture.
- The sampled manifest, grouped, production active-set, ill-conditioned,
  near-singular, singular-degenerate, world contact, and dense world-box
  contact parallel rows passed with `contract_ok=1`.
- Lint, whitespace checks, and the default build passed.
...
- The stacked base branch `feature/lcp-solver-interface-demos` remains at
  `80b3e60e3c5` and has no associated PR.
- Current known `origin/main` is `9de4ac6af873`; this continuation fetched and
  merged `origin/main` with `Already up to date.`
...
- `RunActiveSetTransitionBenchmark()` now rechecks the generated active-set
  transition problem with the selected solver's concrete
  `supportsProblem(problem)` predicate before timing the row.
- `RunActiveFrictionIndexContactBenchmark()` now rechecks the generated active
  friction-index contact problem with the selected solver's concrete
  `supportsProblem(problem)` predicate before timing the row.
- The registration paths already filter these rows through concrete support;
  the new runtime guards make stale or manual registrations fail explicitly
  instead of timing unsupported cases.
...
- `origin/main` remained `9de4ac6af873`, and the merge was already up to
  date.
- Row-list validation passed and listed representative manifest, active-set
  transition, and active friction-index contact rows.
- The sampled rows passed with `contract_ok=1` and
...
- `BM_LcpWorldBoxContactBatchSerial/FrictionIndex` and
  `BM_LcpWorldBoxContactBatchParallel/FrictionIndex` now prebuild each
  candidate batch once, preserve the existing solver-specific candidate rows,
  and publish only args whose generated batch problems are all supported by the
  concrete solver.
- The single and serial-batch runners now keep runtime support guards for the
  same concrete cases.
- This checkpoint does not intentionally change solver implementations, public
  APIs, Python demos, bindings, stubs, generated profile/evidence CSVs, or
...
- Row-list validation passed and listed the expected Pgs and Admm single,
  serial-batch, and parallel-batch rows.
...
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   label cleanup does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/examples/demos/scenes/lcp_physics.py` now labels manifest-level
  support as native forms and the representative solver profile as concrete
  native cases.
- The representative solver-suite table now labels native/delegated summary
  columns as `Native OK` and `Delegated OK`, matching their displayed
  ok-count/total-count values.
- `python/tests/unit/test_py_demo_panels.py` now locks those visible table
  headers.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, runtime solve
...
- Focused LCP py-demo panel metadata/display test passed with 1 test and 154
  deselected.
- Full py-demo panel unit test file passed with 155 tests.
...
## Current Reality - 2026-06-14 Profile Evidence Support Diagnostics
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   diagnostics cleanup does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `scripts/lcp_performance_profile.py` now distinguishes invalid native form
  support counters from an invalid concrete `solver_supports_problem` counter
  when writing profile evidence CSV rows.
- `python/tests/unit/test_lcp_performance_profile.py` now locks both diagnostic
  paths so future profile-writer changes cannot collapse them back into a
  generic solver-support error.
- This checkpoint does not intentionally change solver implementations,
...
- Focused current-schema counter diagnostics passed with 6 tests and 62
  deselected.
- Full LCP performance profile unit test file passed with 68 tests.
...
## Current Reality - 2026-06-14 Evidence Schema Form Support Naming
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   evidence schema naming cleanup does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/examples/demos/scenes/lcp_physics.py` now documents performance
  evidence support counters as native form support plus concrete problem
  support.
- The demo helper map is renamed to `_PROFILE_FORM_SUPPORT_FIELDS`, and
  `scripts/check_lcp_solver_roster.py` guards that name in the static schema
  check.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, or public APIs.
...
- Focused roster schema tests passed with 2 tests.
- Focused panel schema tests passed with 2 tests.
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   standalone surface classification guard does not complete the broad
   objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/examples/demos/scenes/lcp_physics.py` removes the redundant
  `support_key` field from `_StandaloneProblemCase`; standalone case metadata
  now records the surface classification once.
- `scripts/check_lcp_solver_roster.py` now parses and validates the standalone
  case `surface` field directly.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers the simplified
  metadata and stale-row rejection path.
- This checkpoint does not intentionally change solver implementations,
  concrete support predicates, benchmark registration code, generated
  profile/evidence CSVs, bindings, stubs, or public APIs.
...
## Current Reality - 2026-06-14 Concrete Native Count Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   concrete native count guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now checks standalone problem
  summary and solver profile native/delegated counts against the concrete
  `native_supported` rows.
- It removes the duplicate hardcoded native-count table and the
  manifest-category profile count formula while keeping explicit sentinels for
  Direct, MPRGP, Dantzig, and known delegated problem cases.
- This checkpoint does not intentionally change solver implementations, demo
  runtime behavior, benchmark registration code, generated profile/evidence
  CSVs, bindings, stubs, or public APIs.
...
- Focused LCP panel metadata test passed with 1 test and 154 deselected.
- Full panel test passed with 155 tests.
- Lint passed, including the LCP solver roster check.
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   overview table row display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies the remaining LCP
  overview/profile/smoke tables as exact grouped rows instead of loose text
  membership.
- The rendered-table guard covers `lcp_live_packet_cards`,
  `lcp_solver_manifest`, `lcp_benchmark_packets`,
  `lcp_representative_requirement_coverage`, `lcp_performance_profiles`,
...
## Current Reality - 2026-06-14 Advanced Solver Parameter Row Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   advanced solver parameter row display guard does not complete the broad
   objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies
  `lcp_advanced_solver_parameters` as exact grouped rows instead of loose text
  membership.
- The rendered-table guard covers each advanced solver parameter row's solver,
  surface, parameter summary, defaults summary, and benchmark-filter cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver selection guide row display guard does not complete the broad
   objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies
  ...
  membership.
- The rendered-table guard covers each solver guidance row's family, solvers,
  best-fit, strength, tradeoff, and evidence-cue cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Representative Detail Row Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   representative detail row display guard does not complete the broad
   objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies
  ...
  text membership.
- The rendered-table guard covers each representative problem/solver row's
  problem label, solver, route, status, contract result, iterations, solution
  error, residual, complementarity, and elapsed-us cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Representative Suite Row Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   representative suite row display guard does not complete the broad
   objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies
  ...
  membership.
- The rendered-table guard covers each representative problem's label, type,
  LCP row count, friction-index contact count, challenge, native/delegated OK
  counts, max error, max complementarity, max residual, fastest solver, fastest
  native solver, and slowest solver cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Live Metrics Table Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   live metrics table display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now reconstructs rendered table
  rows from the fake panel event stream and verifies the complete
  `lcp_physics_metrics` table.
- The rendered-table guard covers step time, contacts, sliding speed, ramp
  slide, billiard momentum error, billiard energy error, billiard symmetry
...
## Current Reality - 2026-06-14 Live Plot Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   live plot display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies the complete ordered
  set of live LCP plot labels rendered by the panel.
- The rendered-plot guard covers sequential and boxed LCP step time, ramp
  slide, billiard momentum error, billiard energy error, billiard symmetry
  error, stack drift, and card spread series.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Evidence Schema Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   evidence schema display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  performance-profile evidence-schema row shows exact field and meaning cells.
- This replaces broad substring checks with exact rendered-cell checks tied to
...
## Current Reality - 2026-06-14 Solver Manifest Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver manifest display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  solver manifest row shows the row's computed values.
- The rendered-table guard covers solver names, solver families, and coverage
  labels derived from the solver support flags.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Standalone Solver Smoke Display Guard
...
3. This work is a local stacked follow-up on
   ...
   unless the maintainer explicitly chooses that scope.
4. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   standalone solver smoke display guard does not complete the broad objective.
5. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current local tip before this edit:
  `80b3e60e3c5 Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`.
- The stacked base branch `feature/lcp-solver-interface-demos` is also at
  `80b3e60e3c5` and has no associated PR.
- Current known `origin/main` is `9de4ac6af87`; it was merged into the
  checkpoint branch by `80b3e60e3c5`.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  standalone solver smoke row shows the row's computed values.
- The rendered-table guard covers solver names, families, native coverage
  labels, native/delegated routes, status plus iteration counts, solution
  errors, residuals, and complementarities.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Performance Evidence Summary Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   ...
   `29cdaccf29a Guard LCP performance profile display`; if this section is
...
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   performance evidence summary display guard does not complete the broad
   objective.
...
- Current local tip before this edit:
  `29cdaccf29a Guard LCP performance profile display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred fourteen commits.
- Current known `origin/main` is `c4c5ed87eae`; it was merged into this branch
  by `0043424aff7`.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  performance evidence-summary table row shows the row's computed values.
- The rendered-table guard covers surfaces, row counts, solver counts, LCP
  dimensions, contacts, OK counts, max iterations, max residuals, max
  complementarities, and max bound violations.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
- `pixi run lint` passed, including `lint-lcp-solver-roster`,
  `lint-avbd-packets`, and AI command sync.
...
If files change again, rerun `pixi run lint`, `git diff --check`, and any
broader gate warranted by the final diff before committing. Then continue the
broader LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-14 Performance Profile Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `8883be91dc6 Guard LCP benchmark packet display`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   performance profile display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `8883be91dc6 Guard LCP benchmark packet display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred thirteen commits.
- Current known `origin/main` is `c4c5ed87eae`; it was merged into this branch
  by `0043424aff7`.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  performance profile table row shows the row's computed values.
- The rendered-table guard covers surfaces, profile CSV paths, evidence CSV
  paths, problem sizes, current leaders, current laggards, and takeaways.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Benchmark Packet Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0043424aff7 Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   benchmark packet display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred twelve commits.
- `origin/main` was fetched and advanced from `5f7478cff6b` to
  `c4c5ed87eae`; it was merged into this branch by `0043424aff7` after
  resolving `python/examples/demos/registry.py` and
  `python/tests/unit/test_py_demo_panels.py`.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  benchmark packet table row shows the row's computed values.
- The rendered-table guard covers packet labels, surfaces, benchmark filters,
  and coverage cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Requirement Coverage Display Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `3ceb53b69e9 Guard LCP live packet display`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   requirement coverage display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `3ceb53b69e9 Guard LCP live packet display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred nine commits.
- Current known `origin/main` is `5f7478cff6b`; it was already merged into this
  branch by `2599a19ac3c`.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  representative requirement coverage row shows the row's computed values.
- The rendered-table guard covers requirement labels, live packet cells,
  benchmark packet cells, metrics, and evidence cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
- Focused LCP panel test passed with 1 test and 77 deselected.
- `pixi run lint` passed, including `lint-lcp-solver-roster`,
  `lint-avbd-packets`, and AI command sync.
...
## Current Reality - 2026-06-14 Live Packet Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ac9080fcdde Guard LCP solver selection display`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   live packet display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `ac9080fcdde Guard LCP solver selection display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred eight commits.
- Current known `origin/main` is `5f7478cff6b`; it was already merged into this
  branch by `2599a19ac3c`.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that the rendered live
  packet cards table shows every row from `info["live_packet_rows"]`.
- The rendered-table guard covers packet labels, metric descriptions, and
  benchmark analog cells.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Solver Selection Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `2599a19ac3c Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver selection display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred seven commits.
- `origin/main` was fetched and advanced from `a3509146afe` to
  `5f7478cff6b`; `git merge origin/main` completed cleanly.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  solver selection guide row shows the row's computed values.
- The rendered-table guard covers solver families, solver lists, best-fit
  guidance, strengths, tradeoffs, and evidence cues.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-14 Representative Detail Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `35f8787fe9c Guard LCP advanced parameter display`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   representative detail display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `35f8787fe9c Guard LCP advanced parameter display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  representative solver-detail row shows the row's computed values.
- The rendered-table guard covers problem labels, solvers, solve routes,
  statuses, contract results, iteration counts, error/residual/complementarity
  metrics, and elapsed times.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
`pixi run lint`, `git diff --check`, and any broader gate warranted by the final
diff before committing. Then continue the broader LCP interface/demo audit from
the next concrete gap.

## Current Reality - 2026-06-13 Advanced Parameter Display Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `b5ec09771a6 Guard LCP solver profile display`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   advanced parameter display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `b5ec09771a6 Guard LCP solver profile display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred three commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  advanced solver parameter row shows the row's computed values.
- The rendered-table guard covers solver names, surfaces, parameter lists,
  defaults, and benchmark filters.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-13 Solver Profile Display Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `72283643a44 Guard LCP representative suite display`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver profile display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `72283643a44 Guard LCP representative suite display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred two commits.
- `origin/main` was fetched after the previous checkpoint commit, and
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  solver comparison profile row shows the row's computed values.
- The rendered-table guard covers solver names, native surface coverage, OK
  counts, native/delegated OK counts, total runtime, worst
  error/residual/complementarity metrics, slowest case, and slowest runtime.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
## Current Reality - 2026-06-13 Representative Suite Display Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `d5add3a5005 Guard LCP smoke coverage display`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   representative suite display guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `d5add3a5005 Guard LCP smoke coverage display`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now verifies that every rendered
  representative solver-suite summary row shows the row's computed values.
- The rendered-table guard covers labels, problem types, dimensions,
  friction-index contact counts, challenges, native/delegated OK counts, max
  error/complementarity/residual metrics, fastest solver, fastest native
...
focused panel test, `pixi run lint`, `git diff --check`, and any broader gate
warranted by the final diff. Then continue the broader LCP interface/demo audit
from the next concrete gap.

## Current Reality - 2026-06-13 Standalone Smoke Coverage Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `74786a9c876 Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   standalone smoke coverage guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.

Current branch after the requested latest `origin/main` merge:

- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `74786a9c876 Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`
- `origin/main` was fetched at `a3509146afe`, and
  `git merge --no-edit origin/main` reported `Already up to date.`.
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by one hundred commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/tests/unit/test_py_demo_panels.py` now verifies that every standalone
  smoke solver row renders its solver family and native-coverage label in the
  panel table.
- The expected coverage label is derived from
  `lcp_physics._smoke_coverage_label(row)`, which keeps the visible text tied
  to the row's `native_standard`, `native_boxed`, and `native_findex` support
  flags.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
- `git fetch origin main` passed and `git merge --no-edit origin/main`
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `a29b0121062 Pin LCP demo benchmark README command`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   representative requirement-link guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- `feature/lcp-solver-interface-demos`
- Current local tip before this edit:
  `a29b0121062 Pin LCP demo benchmark README command`
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-two commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` now uses one
  `_STANDALONE_PROBLEM_SUITE_LABEL` constant for the virtual representative
  solver-suite row used by requirement coverage and the UI.
- `scripts/check_lcp_solver_roster.py` now parses the LCP demo live packets,
  representative requirement rows, benchmark packet IDs, standalone problem
  case IDs, and standalone suite label, then rejects stale requirement
  references.
- `python/tests/unit/test_check_lcp_solver_roster.py` covers the valid parser
  path and the stale live-packet / stale benchmark-reference failure paths.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
```bash
PYTHONPATH=python pixi run python -m pytest python/tests/unit/test_check_lcp_solver_roster.py -q -k 'representative_requirement or benchmark_filter or uncovered'
PYTHONPATH=python pixi run python scripts/check_lcp_solver_roster.py
PYTHONPATH=python pixi run python -m pytest python/tests/unit/test_check_lcp_solver_roster.py -q
PYTHONPATH=build/default/cpp/Release/python:python pixi run python -m pytest python/tests/unit/test_py_demo_panels.py -q
pixi run lint
...
- Focused roster representative-link tests: passed with 4 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Full roster checker unit file: passed with 33 tests.
- Full demo-panel unit file: passed with 78 tests.
- Lint, diff whitespace check, default build, and the full CUDA `test-all`
  gate passed. The CUDA documentation phase still emitted the known four
  `dartpy._world_render_bridge` autodoc warnings, but the full gate passed.
...
## Current Reality - 2026-06-13 Demo README Benchmark Command Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `f3594d980f3 Guard LCP benchmark packet coverage`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   README benchmark-command guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/tests/unit/test_py_demo_panels.py` now checks the LCP demo README
  benchmark smoke command against the `_BENCHMARK_COMMAND` metadata in
  `python/examples/demos/scenes/lcp_physics.py`.
- `python/examples/demos/README.md` now tells readers to use the scene
  metadata's derived `representative_benchmark_command` for the full
  representative packet filter, keeping the long generated filter list out of
  the README.
- This checkpoint does not intentionally change solver implementations,
  benchmark registration code, generated profile/evidence CSVs, bindings,
  stubs, public APIs, or demo runtime behavior.
...
- Focused README/metadata demo-panel tests: passed with 2 tests.
- Full demo-panel unit file: passed with 78 tests.
- Lint: passed, including LCP solver roster and generated AI command checks.
- Whitespace check: passed.
- Default build: passed.
- CUDA full gate: passed end to end. The documentation phase still emitted the
  known four `dartpy._world_render_bridge` autodoc warnings, but the full gate
  passed.
...
If this checkpoint is still uncommitted and files change again, rerun the
focused demo-panel metadata test, `pixi run lint`, `git diff --check`, and any
broader gate warranted by the final diff, then commit the focused
README/test/docs change. Then continue the broader LCP interface/demo audit from
the next concrete gap.

## Current Reality - 2026-06-13 Benchmark Coverage Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `9bcb6d09ff5 Expose LCP CUDA benchmark packets`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   benchmark coverage-guard fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/check_lcp_solver_roster.py` now turns the previous one-off
  uncovered benchmark audit into a permanent roster invariant.
- The roster checker still rejects demo benchmark filters that do not map to a
  registered `BM_LCP_COMPARE`/`BM_Lcp*` base, and now also rejects registered
  benchmark bases that have no demo-side representative filter prefix.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers that missing
  representative-filter drift path with a focused negative test.
- This checkpoint does not add benchmark packets or intentionally change solver
  implementations, benchmark registration code, generated profile/evidence
  CSVs, bindings, stubs, public APIs, or demo scene metadata.
...
- Focused benchmark-filter guard tests: passed with 3 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Full roster unit test file: passed with 30 tests.
...
the focused script/test/docs change. Then continue the broader LCP
interface/demo audit from the next concrete gap.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `b66e3c59a3d Expose LCP validation benchmark packet`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   boxed world-step benchmark-packet fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-eight commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Validation Benchmark Packet
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ca63ca866d3 Expose LCP Newton warm-start batch filters`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   validation benchmark-packet fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-seven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Newton Warm-Start Batch Filters
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `34fb7b74010 Expose LCP near-singular batch filters`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Newton warm-start batch benchmark-filter fix does not complete the broad
   objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-six commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Near-Singular Batch Benchmark Filters
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `5c4444ed18c Expose LCP mild conditioning benchmark packet`; if this section
   is committed, inspect `git log --oneline --decorate -8` for the new exact
   tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   near-singular batch benchmark-filter fix does not complete the broad
   objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-five commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Mild Ill-Conditioned Benchmark Packet
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `8f738daf0cf Point LCP parameter rows at threading benchmarks`; if this
   section is committed, inspect `git log --oneline --decorate -8` for the new
   exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   mild ill-conditioned benchmark-packet fix does not complete the broad
   objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Threaded Parameter Benchmark Filters
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `b34a44bab66 Show LCP manifest support counts separately`; if this section
   is committed, inspect `git log --oneline --decorate -8` for the new exact
   tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   threaded parameter benchmark-filter fix does not complete the broad
   objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-three commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Solver Manifest Summary Counts
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `2a63696d3cc Render LCP coverage labels from support flags`; if this section
   is committed, inspect `git log --oneline --decorate -8` for the new exact
   tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver manifest summary-count fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-two commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
`pixi run build`, and any broader gate warranted by the final diff, then commit
the focused demo/test/docs change. Then continue the broader LCP interface/demo
audit from the next concrete gap.

## Current Reality - 2026-06-13 Solver Manifest Coverage Labels
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0c072e73fd7 Use manifest names for LCP parameter rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   solver manifest coverage-label fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty-one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- The LCP panel now labels manifest-only friction-index support as
  `findex only; std/boxed delegate`, fixing the `Staggering` row that
  previously looked standard-only.
- `python/tests/unit/test_py_demo_panels.py` now checks the rendered
  solver-manifest table for all-surface, standard-only, and
  friction-index-only coverage labels.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, and generated
  profile/evidence CSVs were not intentionally changed.
...
## Current Reality - 2026-06-13 Parameter Row Manifest Names
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0603e78891e Guard LCP demo benchmark filters`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   parameter row manifest-name fix does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifty commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now asserts that advanced
  parameter rows are a subset of the manifest-derived solver name set.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `9b395763617 Guard LCP README profile commands`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   partial profile help guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty-eight commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `scripts/lcp_performance_profile.py` now exposes `build_arg_parser()` so the
  profile generator's CLI help can be unit-tested without shelling out through
  `main()`.
- The `--allow-partial` help text now explains that partial runs must use a
  scratch `--output` directory and that checked
  `docs/background/lcp/figures` output is refused.
- `python/tests/unit/test_lcp_performance_profile.py` verifies that
...
- Focused partial-output/help tests: passed with 2 tests.
- Full LCP performance-profile generator test file: passed with 68 tests.
- Direct CLI help check: showed the `--allow-partial` scratch-output warning.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- `pixi run lint`: passed, including the LCP solver roster and generated AI
  command checks.
...
If this checkpoint is still uncommitted and files change again, rerun the
profile-generator unit test, roster check, `pixi run lint`, `git diff --check`,
`pixi run build`, and any broader gate warranted by the final diff, then commit
the focused script/test/docs change. Then continue the broader LCP
interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 README Profile Command Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `7f9f2c6f519 Expose LCP profile smoke command`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   README profile command guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty-seven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/tests/unit/test_py_demo_panels.py` now parses the README profile
  command fenced blocks and compares them against the scene-owned
  `_PERFORMANCE_PROFILE_REFRESH_COMMAND` and
  `_PERFORMANCE_PROFILE_SMOKE_COMMAND` constants.
- Future README drift in either the checked refresh command or scratch-output
  smoke command now fails the demo-panel regression.
- Generated profile/evidence CSVs, bindings, stubs, solver predicates, public
  APIs, benchmark generator code, and C++ solver code were not intentionally
  changed.
...
- Focused LCP README/profile metadata tests: passed with 4 tests.
- Full demo-panel unit test file: passed with 78 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- `pixi run lint`: passed, including the LCP solver roster and generated AI
...
demo-panel unit test, roster check, `pixi run lint`, `git diff --check`,
`pixi run build`, and any broader gate warranted by the final diff, then commit
the focused README/test/docs change. Then continue the broader LCP
interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Profile Smoke Command
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ca1f794c514 Guard partial LCP profile output`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   profile smoke command does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty-six commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `python/examples/demos/scenes/lcp_physics.py` now exposes
  `performance_profile_smoke_command` alongside the full checked
  `performance_profile_refresh_command`.
- The smoke command runs the profile-compatible
  `BM_LcpCompare/Standard/Dantzig/12` benchmark subset with `--allow-partial`,
  caches to `build/lcp_profile_smoke.json`, and writes profile/evidence
  artifacts under `build/lcp_profile_smoke`.
- The LCP panel renders the smoke command in the Performance profiles section,
  and `python/examples/demos/README.md` documents the same scratch-output smoke
  path separately from the checked artifact refresh.
- `python/tests/unit/test_py_demo_panels.py` verifies the exact smoke command,
  its `--allow-partial` flag, its scratch output target, absence of the checked
  `docs/background/lcp/figures` output target, and rendered panel text.
- Generated profile/evidence CSVs, bindings, stubs, solver predicates, public
  APIs, benchmark generator code, and C++ solver code were not intentionally
  changed.
...
- Focused LCP demo metadata/schema tests: passed with 3 tests.
- Direct profile smoke command: exited successfully, warned about incomplete
  native solver coverage, cached results to `build/lcp_profile_smoke.json`, and
  wrote scratch artifacts under `build/lcp_profile_smoke/`.
- Full demo-panel unit test file: passed with 77 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- `pixi run lint`: passed, including the LCP solver roster and generated AI
  command checks.
...
If this checkpoint is still uncommitted and files change again, rerun the full
demo-panel unit test, direct scratch profile smoke command, roster check,
`pixi run lint`, `git diff --check`, `pixi run build`, and any broader gate
warranted by the final diff, then commit the focused demo/docs/test change.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `cd873ba119c Cover LCP demo evidence schema fields`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   rendered evidence schema guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty-four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Focused LCP demo schema tests: passed with 3 tests.
- Full demo-panel unit test file: passed with 77 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- `pixi run lint`: passed, including the LCP solver roster and generated AI
...
If this checkpoint is still uncommitted and files change again, rerun the full
demo-panel unit test, roster check, `pixi run lint`, `git diff --check`, and
`pixi run build`, plus any broader gate warranted by the final diff, then
commit the focused test/docs change. Then continue the broader LCP
interface/demo audit from the next concrete gap.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `26182605777 Guard LCP empty evidence export`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   profile CSV preflight guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty-one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 63 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
...
## Current Reality - 2026-06-13 Generator Empty Evidence Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `6d1d0434e04 Guard LCP semantic evidence rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator empty-evidence guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by forty commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 56 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Generator Semantic Evidence Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `6a33317bb05 Guard LCP evidence writer rows`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator semantic evidence-writer guard does not complete the broad
   objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-nine commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 55 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ce521817bcf Guard LCP generator contact counts`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator LCP-dimension guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-seven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 42 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Generator Contact Count Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0925a74253b Guard LCP generator problem sizes`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator contact-count guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-six commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 37 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Generator Positive Size Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `07c1654453f Guard LCP roster nonfinite counters`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator positive-size guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-five commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP performance profile generator tests: passed with 33 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Roster Non-Finite Counter Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `3289fb847b6 Guard LCP demo fractional counters`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   roster non-finite counter guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP solver roster unit tests: passed with 27 tests.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
`scripts/check_lcp_solver_roster.py`, `pixi run lint`, and `git diff --check`,
then commit the focused checker/test/docs change. Then continue the broader
LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Demo Fractional Counter Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ba85115c4c2 Normalize LCP support counters`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   demo fractional-counter guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-three commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- LCP demo evidence-summary tests: passed with 32 tests and 43 deselected.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `52620aa9b83 Normalize LCP profile ratio inputs`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   quality-metric guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty-one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Ratio Value Normalization
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `443350bf895 Guard LCP invalid timing rows`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   ratio-normalization guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirty commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Invalid Timing Row Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `8a7c4df24ce Guard LCP failed contract rows`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   invalid timing row guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-nine commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Failed Contract Row Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0b570dc2ea3 Guard LCP benchmark duplicate rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   failed contract row guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-eight commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Duplicate Benchmark Row Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `be935a3936f Guard LCP profile CSV rows`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   duplicate benchmark row guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-seven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
## Current Reality - 2026-06-13 Profile CSV Row Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `79f213659ac Guard LCP evidence duplicate rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   profile CSV row guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-six commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
`python/tests/unit/test_check_lcp_solver_roster.py`,
`scripts/check_lcp_solver_roster.py`, `pixi run lint`, and `git diff --check`,
then commit the focused checker/test/docs change. Then continue the broader LCP
interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Duplicate Evidence Row Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0e6b67546e9 Guard strict LCP evidence export`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   duplicate evidence row guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-five commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Focused checker duplicate-row test: passed.
- Focused py-demo duplicate-row test: passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
If this checkpoint is still uncommitted and files change again, rerun the
focused checker test, focused py-demo test, `scripts/check_lcp_solver_roster.py`,
`pixi run lint`, and `git diff --check`, then commit the focused
checker/demo/test/docs change. Then continue the broader LCP interface/demo
audit from the next concrete gap.

## Current Reality - 2026-06-13 Strict Evidence Export Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `52c5a236d9a Guard LCP evidence header duplicates`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   strict evidence export guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Profile generation/export: passed after running `BM_LCP_COMPARE`, caching
...
`scripts/check_lcp_solver_roster.py`, `pixi run lint`, and `git diff --check`,
then commit the focused generator/test/docs/artifact change. Then continue the
broader LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Evidence Header Duplicate Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `5d2c772867d Guard LCP profile header duplicates`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   evidence-header duplicate guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-three commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Focused roster tests: 21 tests passed.
- Duplicate evidence-column demo test: passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
`scripts/check_lcp_solver_roster.py`, `pixi run lint`, and `git diff --check`,
then commit the focused checker/demo/test/docs change. Then continue the
broader LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Profile Header Duplicate Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `d9d9650f875 Test LCP profile header guards`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   profile-header duplicate guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-two commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Focused roster tests: 20 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
and `git diff --check`, then commit the focused checker/test/docs change. Then
continue the broader LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Profile Header Guard Tests
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `f711df5312a Guard exact LCP stub classes`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   profile-header test guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty-one commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- Focused roster tests: 19 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
If this checkpoint is still uncommitted and files change again, rerun the
focused roster tests, `scripts/check_lcp_solver_roster.py`, `pixi run lint`,
and `git diff --check`, then commit the focused test/docs change. Then continue
the broader LCP interface/demo audit from the next concrete gap.

## Current Reality - 2026-06-13 Exact Stub Class Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `453ee4669fc Guard LCP math stub exports`;
   if this section is committed, inspect `git log --oneline --decorate -8` for
   the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   exact stub-class guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by twenty commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/check_lcp_solver_roster.py` now rejects dartpy-bound LCP solver
  classes that are not present in the C++ manifest.
- The roster check now rejects extra `python/stubs/dartpy/math.pyi` subclasses
  of `LcpSolver` that are not present in the manifest.
- `python/tests/unit/test_check_lcp_solver_roster.py` covers both extra
  bound-solver and extra math-stub solver class regressions.
...
- Focused roster tests: 16 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Math Stub Export Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `4a0a5c4292f Tighten LCP init stub guard`; if
   this section is committed, inspect `git log --oneline --decorate -8` for the
   new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   math-stub export guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by nineteen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/check_lcp_solver_roster.py` now parses stub `__all__` lists through
  a shared helper and applies it to `python/stubs/dartpy/math.pyi` in addition
  to `python/stubs/dartpy/__init__.pyi`.
- The roster check now rejects manifest solver classes that are present as
  `math.pyi` class definitions but missing from `math.pyi.__all__`.
- `python/tests/unit/test_check_lcp_solver_roster.py` covers the missing
  `math.pyi.__all__` export case.
- No generated stubs, bindings, solver predicates, demo runtime behavior,
  public APIs, checked profile CSVs, or performance timings were intentionally
  changed.
...
- Focused roster tests: 14 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Init Stub Export Guard
...
1. Read `AGENTS.md`, `docs/ai/principles.md`, this `RESUME.md`, and
   `docs/dev_tasks/lcp_solver_interface_demos/README.md`.
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was `0a48814439a Test LCP stub roster guard`; if
   this section is committed, inspect `git log --oneline --decorate -8` for the
   new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   init-stub export guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by eighteen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `scripts/check_lcp_solver_roster.py` now parses
  `python/stubs/dartpy/__init__.pyi` structurally and checks LCP solver classes
  in both the `from .math import (...)` re-export list and `__all__`.
- The roster check no longer accepts a raw name hit elsewhere in the init stub
  as proof that the solver is re-exported correctly.
...
- Focused roster tests: 13 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Python Stub Guard Tests
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `e0431b507da Reuse LCP demo profile test schema`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   stub-guard test hardening does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by seventeen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/check_lcp_solver_roster.py` now exposes the dartpy stub coverage
  assertion as `check_python_stub_solver_classes()`, while keeping
  `check_roster()` behavior unchanged.
- `python/tests/unit/test_check_lcp_solver_roster.py` now directly covers
  missing solver classes from `python/stubs/dartpy/math.pyi` and
  `python/stubs/dartpy/__init__.pyi`.
- No generated stubs, bindings, solver predicates, demo runtime behavior,
  public APIs, checked profile CSVs, or performance timings were intentionally
  changed.
...
- Focused roster tests: 12 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Demo Test Schema Reuse
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `a948e32d42e Guard LCP demo profile schema`; if this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   test-schema reuse does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixteen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/tests/unit/test_py_demo_panels.py` no longer duplicates the LCP
  performance-profile evidence required-column tuple for test fixtures.
- The panel-test evidence writer and missing/empty-column checks now use
  `lcp_physics._PERFORMANCE_PROFILE_EVIDENCE_REQUIRED_COLUMNS`, matching the
  demo runtime validator and roster-guarded schema source.
...
- Focused LCP demo panel tests: 27 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Demo Profile Schema Guard
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `19ebc6db6cb Share LCP profile counter schema`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   demo schema guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by fifteen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/check_lcp_solver_roster.py` now parses all Python LCP demo
  profile-evidence schema constants used by the display-side validator, not
  only the required-column tuple.
- The roster checker compares those demo constants against the roster-owned
  profile category, support, problem-type, identity, family, and required-column
  schema.
- `python/tests/unit/test_check_lcp_solver_roster.py` covers stale demo
...
- Focused profile/roster tests: 24 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `b79eb8892e7 Guard LCP profile native coverage`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   generator-schema reuse does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by thirteen commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `scripts/lcp_performance_profile.py` now imports
  `REQUIRED_EVIDENCE_COLUMNS` from `scripts/check_lcp_solver_roster.py` and
  uses it as the evidence CSV header.
- The profile generator, roster checker, and Python demo display checks now
  share one source of truth for the required evidence columns.
...
- Focused profile/roster tests: 22 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `af41c467aad Validate LCP demo profile surfaces`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   schema-drift guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by eleven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `scripts/check_lcp_solver_roster.py` now parses the Python LCP demo's
  computed `_PERFORMANCE_PROFILE_EVIDENCE_REQUIRED_COLUMNS` tuple from the AST.
- The roster lint gate compares the demo's required evidence columns with
  `REQUIRED_EVIDENCE_COLUMNS`, so checked evidence CSV schema and demo
  display-side schema drift is rejected locally.
- `python/tests/unit/test_check_lcp_solver_roster.py` covers a stale demo
  column regression by simulating a missing `time_ns` column.
- No checked profile CSVs, benchmark registrations, solver predicates, demo
  runtime behavior, public APIs, or performance timings were intentionally
  changed.
...
- Focused roster tests: 7 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
...
## Current Reality - 2026-06-13 Python Demo Profile Surface Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `0b8bbb74564 Validate LCP demo profile schema`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo surface guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by ten commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` requires Standard, Boxed, and
  FrictionIndex evidence rows before summarizing the LCP demo profile evidence
  table.
- The rejected cases now include present-but-partial evidence files that would
  otherwise silently omit a profile surface from the displayed summary.
- `python/tests/unit/test_py_demo_panels.py` covers missing evidence surfaces
  alongside the schema, category, identity, support, problem-type, and numeric
  guards.
...
- Focused py-demo panel tests: 27 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
## Current Reality - 2026-06-13 Python Demo Profile Schema Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `e97bb97469d Validate LCP demo profile metrics`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo schema guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by nine commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` validates the performance
  profile evidence CSV header before summarizing rows in the LCP demo panel.
- The rejected cases now include missing required evidence columns and
  header-only evidence files.
- `python/tests/unit/test_py_demo_panels.py` covers missing-column and empty
  evidence-file rows alongside the category, identity, support, problem-type,
  and numeric guards.
- No checked profile CSVs, benchmark registrations, solver predicates, public
  APIs, or performance timings were intentionally changed.
...
- Focused py-demo panel tests: 26 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `ee9ab77a1fb Validate LCP demo profile identities`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo support-counter guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by seven commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` validates Standard, Boxed, and
  FrictionIndex support counters against the demo solver manifest before
  summarizing performance profile evidence rows in the LCP demo panel.
- The rejected cases now include stale all-form support counters on otherwise
  valid rows, while preserving separate failures for non-native rows and
  concrete `solver_supports_problem=0` native rows.
- `python/tests/unit/test_py_demo_panels.py` covers stale support counters,
  non-native boxed `Lemke`, and unsupported boxed `Dantzig` evidence rows.
- No checked profile CSVs, benchmark registrations, solver predicates, public
  APIs, or performance timings were intentionally changed.
...
- Focused py-demo panel tests: 14 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
## Current Reality - 2026-06-12 Python Demo Profile Identity Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `52f22d39d30 Validate LCP demo profile problem types`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo identity guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by six commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` validates solver identity
  schema version, manifest index, and family one-hot counters before
  summarizing performance profile evidence rows in the LCP demo panel.
- The rejected cases now include stale identity schema versions, mismatched
  manifest indices, mismatched solver-family counters, and unknown profile
  evidence solver names.
- `python/tests/unit/test_py_demo_panels.py` uses complete evidence-schema
  rows for the profile-summary regressions and covers the new stale identity
  cases.
- No checked profile CSVs, benchmark registrations, solver predicates, public
  APIs, or performance timings were intentionally changed.
...
- Focused py-demo panel tests: 11 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
## Current Reality - 2026-06-12 Python Demo Profile Problem-Type Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `fdfcf322e26 Validate LCP demo profile evidence rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo problem-type guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by five commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` validates the full one-hot
  problem-type counter set before summarizing performance profile evidence rows
  in the LCP demo panel.
- The rejected cases now include unknown profile categories, category/problem
  type mismatches, `problem_type_invalid=1` rows, non-native solver/category
  rows, and concrete `solver_supports_problem=0` rows.
...
- Focused py-demo panel tests: 6 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
## Current Reality - 2026-06-12 Python Demo Profile Evidence Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `04cc867b958 Keep partial LCP profile checks strict`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
   Python demo guard does not complete the broad objective.
4. Do not push, open a PR, retry CI, or mutate GitHub state unless the user
   explicitly asks in the new session.
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by four commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.

What this checkpoint changes:

- `python/examples/demos/scenes/lcp_physics.py` rejects invalid performance
  profile evidence rows before summarizing them in the LCP demo panel.
- The rejected cases include unknown profile categories, non-native
  solver/category rows, concrete `solver_supports_problem=0` rows, and
  category/problem-type counter mismatches.
- `python/tests/unit/test_py_demo_panels.py` covers non-native and unsupported
...
- The `dartpy` target rebuilt successfully after the existing extension was
  stale against the local shared libraries.
- Focused py-demo panel tests: 3 tests passed.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
## Current Reality - 2026-06-12 Profile Evidence Partial-Run Validation
...
2. Treat current repository state as authoritative. The latest completed local
   tip before this checkpoint was
   `914e3cf4a47 Reject non-native LCP profile rows`; if this section is
   committed, inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP interface/demo audit from a fresh bounded gap; this
...
- Current relationship before this edit:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by three commits.
- There is no associated PR. Do not push, open a PR, or mutate GitHub state
  without explicit maintainer/user approval.
...
- `scripts/lcp_performance_profile.py --allow-partial` only relaxes missing
  native solver coverage for smoke runs. Invalid rows, unsupported
...
- `python/tests/unit/test_lcp_performance_profile.py` adds the corresponding
  boxed `Lemke` historical-row regression plus strict `--allow-partial`
  coverage.
...
- Focused roster/profile tests: 18 tests passed.
- LCP solver roster check: passed with 24 solvers, 23 standard, 15 boxed, and
  16 findex.
- Cached profile regeneration was not rerun because
  `build/lcp_profile_full.json` does not exist in this checkout.
- Repository lint: passed, including `lint-lcp-solver-roster` and
  `sync-ai-commands`.
- Whitespace check: passed.
...
If this checkpoint is still uncommitted and files change again, rerun
`pixi run lint` and `git diff --check`, then commit the focused
checker/test/docs change. Then continue the broader LCP interface/demo audit
from the next concrete gap. Avoid a full performance-profile refresh while the
host is under unrelated heavy build load unless the user explicitly asks for
it.
...
- Local branch cleanup check found only `feature/lcp-solver-interface-demos`
  and `main`; no extra local branches created by this session remain to delete.
- The current branch is the consolidated resume branch.
- The user requested no further implementation work. Keep future work stopped
  unless explicitly resumed.
- The user approved pushing this branch after the hand-off docs are updated.

Latest completed implementation checkpoint:

- `8b571a91031 Expose LCP problem contact metadata` is committed locally.
- It exposes `LcpProblem::getFrictionIndexRowCount()` and
  `LcpProblem::getFrictionIndexContactCount()`, dartpy bindings for both, Python
  representative-suite metadata/table columns, benchmark `contact_count`
  emission from `LcpProblem`, and focused tests for the new metadata.
- Verification for that checkpoint already passed before this stop hand-off:
  focused C++ LCP type test, dartpy rebuild, focused Python LCP/demo tests, LCP
  benchmark target compile, `pixi run lint`, and `git diff --check`.
...
Continue only if the user explicitly resumes implementation. The latest
bounded next area inspected, but not implemented, was DART 7 harness-aligned LCP
solver identity/support metadata in benchmark/demo evidence. Avoid retrying the
rejected SAP FrictionIndex exact shortcut or ShockPropagation exact-path probe
