...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `29cdaccf29a Guard LCP performance profile display` as the latest
...
Performance evidence summary display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered
  performance evidence-summary row, not only the table header and a couple of
  aggregate count tokens.
- The guard covers each row's surface, rows, solvers, LCP dimensions, contacts,
  OK count, max iterations, max residual, max complementarity, and max
  bound-violation cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-14 Current Continuation - Performance Profile Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `8883be91dc6 Guard LCP benchmark packet display` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Performance profile display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered
  performance profile row, not only the performance profile table header,
  profile-smoke command, and metadata.
- The guard covers each row's surface, profile CSV, evidence CSV, problem
  sizes, current leaders, current laggards, and takeaway cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-14 Current Continuation - Benchmark Packet Display Guard
...
2. Treat
   ...
   as the latest completed local tip before this checkpoint. If this section is
...
Benchmark packet display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered benchmark
  packet row, not only the benchmark packet table header and metadata.
- The guard covers each row's packet, surface, benchmark filter, and coverage
  cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
## 2026-06-14 Current Continuation - Requirement Coverage Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `3ceb53b69e9 Guard LCP live packet display` as the latest completed
...
Requirement coverage display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered
  representative requirement coverage row, not only the requirement labels.
- The guard covers each row's requirement, live packet, benchmark packet,
  metrics, and evidence cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
- `PYTHONPATH=build/default/cpp/Release/python:python pixi run python -m pytest python/tests/unit/test_py_demo_panels.py -q -k lcp_physics_exposes_solver_manifest_and_benchmark_metadata`
  passed with 1 test and 77 deselected.
- `pixi run lint` passed, including `lint-lcp-solver-roster`,
  `lint-avbd-packets`, and AI command sync.
...
## 2026-06-14 Current Continuation - Live Packet Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `ac9080fcdde Guard LCP solver selection display` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Live packet display status:

- `python/tests/unit/test_py_demo_panels.py` now checks the rendered live packet
  cards table, not only the `info["live_packet_rows"]` metadata.
- The guard covers each live packet row's packet label, metric description, and
  benchmark-analog cell.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-14 Current Continuation - Solver Selection Display Guard
...
Solver selection display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered solver
  selection guide row, not only the family-name cells.
- The guard covers each row's family, solver list, best-fit guidance, strength,
  tradeoff, and evidence-cue cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-14 Current Continuation - Representative Detail Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `35f8787fe9c Guard LCP advanced parameter display` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Representative detail display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered
  representative solver-detail row, not only the table header and a generic
  `OK` cell.
- The guard covers each row's problem label, solver name, solve route, status,
  contract result, iteration count, error, residual, complementarity, and
  elapsed-time cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
2. If this checkpoint is still uncommitted and files change again, rerun
   `pixi run lint`, `git diff --check`, and any broader gate warranted by the
   final diff before committing.
3. Continue the broader LCP interface/demo audit from the next concrete gap.
   Do not treat the broad LCP objective as complete.

## 2026-06-13 Current Continuation - Advanced Parameter Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `b5ec09771a6 Guard LCP solver profile display` as the latest completed
   local tip before this checkpoint. If this section is committed, inspect
   `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Advanced parameter display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered advanced
  solver parameter row, not only the table header.
- The guard covers each row's solver name, surface label, parameter list,
  default values, and benchmark filter cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-13 Current Continuation - Solver Profile Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `72283643a44 Guard LCP representative suite display` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Solver profile display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered solver
  comparison profile row, not only Dantzig/MPRGP sample cells.
- The guard covers each row's solver name, native surface coverage, OK counts,
  native/delegated OK counts, total runtime, worst error, worst residual, worst
  complementarity, slowest case, and slowest runtime cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
## 2026-06-13 Current Continuation - Representative Suite Display Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `d5add3a5005 Guard LCP smoke coverage display` as the latest completed
   local tip before this checkpoint. If this section is committed, inspect
   `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
Representative suite display status:

- `python/tests/unit/test_py_demo_panels.py` now checks every rendered
  representative solver-suite summary row, not only the standard-SPD sample.
- The guard covers each row's label, problem type, LCP dimension, friction-index
  contact count, challenge text, native/delegated OK counts, max error,
  complementarity, residual, fastest solver, fastest native solver, and slowest
  solver cells.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
## 2026-06-13 Current Continuation - Standalone Smoke Coverage Guard
...
Current branch state after the requested latest `origin/main` merge:
...
Standalone smoke coverage status:

- `python/tests/unit/test_py_demo_panels.py` now checks every standalone smoke
  solver row renders its family and native-coverage label.
- The guard derives the expected native-coverage text from
  `lcp_physics._smoke_coverage_label(row)`, so the visible table stays aligned
  with each smoke solver row's `native_standard`, `native_boxed`, and
  `native_findex` flags.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
## 2026-06-13 Current Continuation - Demo Command Metadata Guard
...
Current branch state after the local `origin/main` merge and before this
checkpoint commit:

- Branch: `feature/lcp-solver-interface-demos`.
- Current local tip before this edit:
  `a137ebb67a6 Merge remote-tracking branch 'origin/main' into feature/lcp-solver-interface-demos`.
- `origin/main` was fetched at `a122e8e0f3e` and merged locally. The only
  conflict was in `pixi.toml`; resolution kept both the LCP roster and AVBD
  packet lint/check tasks.
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by seventy-three commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Demo command metadata status:

- `scripts/check_lcp_solver_roster.py` now statically parses the LCP demo's
  benchmark and performance-profile command constants.
- The roster guard now rejects stale `_BENCHMARK_COMMAND`,
  `_REPRESENTATIVE_BENCHMARK_FILTER`, `_REPRESENTATIVE_BENCHMARK_COMMAND`,
  `_PERFORMANCE_PROFILE_REFRESH_COMMAND`, and
  `_PERFORMANCE_PROFILE_SMOKE_COMMAND` values, including drift between the
  representative benchmark command and the union of benchmark packet filters.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers both the valid
  command metadata path and stale command/filter drift failures.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
- `PYTHONPATH=python pixi run python -m pytest python/tests/unit/test_check_lcp_solver_roster.py -q -k 'command_metadata or benchmark_filter'`
  passed with 4 tests before the merge; the full roster file below covers the
  same tests after the merge.
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `e0783d892bb Guard LCP standalone problem metadata` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-six commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Live packet status:

- `scripts/check_lcp_solver_roster.py` now validates the LCP demo
  `_LIVE_PACKET_ROWS` table directly, instead of relying only on
  representative requirement cross-link failures.
- The roster guard now rejects empty live-packet tables, duplicate packet
  labels, blank packet/metric/benchmark fields, and live packets that are not
  covered by at least one representative requirement row.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers both the valid
  live-packet parser/check path and stale live-packet metadata failures.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
- `pixi run -e cuda test-all` passed end-to-end on that GPU: lint, build,
  unit tests, simulation tests, Python tests, documentation, and CUDA C++ tests
  plus benchmark smoke. The documentation phase emitted the known four
  `dartpy._world_render_bridge` autodoc warnings and still passed.
...
## 2026-06-13 Current Continuation - Standalone Problem Case Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `cdcec488d8d Guard LCP advanced parameter metadata` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-five commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Standalone problem-case status:

- `scripts/check_lcp_solver_roster.py` now parses the full
  `_STANDALONE_PROBLEM_CASES` table from
  `python/examples/demos/scenes/lcp_physics.py`, instead of only extracting
  case names for representative requirement links.
- The roster guard now rejects blank suite labels, duplicate case names or
  labels, blank required fields, unknown surfaces/support keys, surface/support
  mismatches, unknown `make_problem` function references, non-positive or
  non-finite tolerances, malformed case entries, and missing representative
  standard/boxed/findex surfaces.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers both the valid
  standalone-case parser path, malformed case entries, and stale case metadata
  failures.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
- `PYTHONPATH=python pixi run python -m pytest python/tests/unit/test_check_lcp_solver_roster.py -q -k 'standalone_problem or representative_requirement or advanced_solver_parameter or solver_guidance'`
  passed with 8 tests, and passed again after `pixi run lint` reformatted
...
## 2026-06-13 Current Continuation - Advanced Parameter Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `a63bcabbcb6 Guard LCP solver guidance metadata` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-four commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Advanced solver-parameter status:

- `scripts/check_lcp_solver_roster.py` now parses
  `_advanced_solver_parameter_rows()` from
  `python/examples/demos/scenes/lcp_physics.py` and checks that the demo's
  solver-parameter table names only manifest solvers, advertises only supported
  surfaces, and points at registered solver-parameter benchmark filters.
- The guard also checks that each advanced row's benchmark filter is covered by
  the demo's `solver_parameter_sweeps` packet, so the overview packet and the
  per-solver tuning table cannot drift independently.
- The new guard exposed that the advanced Newton rows referenced
  `BM_LcpNewtonWarmStart` while the `solver_parameter_sweeps` packet listed
  only the batch warm-start filters. The packet now includes
  `BM_LcpNewtonWarmStart`, and
  `python/tests/unit/test_py_demo_panels.py` was updated to pin that user-facing
  metadata.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers both the valid
  advanced-parameter parser path and stale solver, benchmark-filter, and
  surface references.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
...
- `nvidia-smi --query-gpu=name,compute_cap --format=csv,noheader` reported
  `NVIDIA RTX 5000 Ada Generation Laptop GPU, 8.9`.
- `pixi run -e cuda test-all` passed end to end on that CUDA host: linting,
  build, unit tests, simulation tests, Python tests, documentation, and CUDA
  tests all passed. The documentation phase emitted the known four
  `dartpy._world_render_bridge` autodoc warnings and still completed
  successfully.
...
## 2026-06-13 Current Continuation - Solver Guidance Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `5eb3b26a8a0 Guard LCP requirement metadata links` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-three commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Solver guidance status:

- `scripts/check_lcp_solver_roster.py` now parses
  `_SOLVER_GUIDANCE_ROWS` from `python/examples/demos/scenes/lcp_physics.py`
  and checks the demo's solver selection guide against the C++ solver manifest.
- The guard rejects stale guide rows with unknown solver names, duplicate solver
  entries, missing manifest solvers, duplicate guide families, or blank required
  fields.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers both the valid
  guide parser path and a stale guide row that reports unknown, duplicate, and
  missing solver entries.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
- `pixi run lint` passed, including the LCP solver roster check and generated
  AI command sync.
- `git diff --check` passed.
- `pixi run build` passed.
- `pixi run -e cuda test-all` passed end to end on the visible
  NVIDIA RTX 5000 Ada Generation Laptop GPU. The documentation phase still
  emitted the known four `dartpy._world_render_bridge` autodoc warnings, then
  passed.
...
## 2026-06-13 Current Continuation - Requirement Link Guard
...
1. Start from the current checkout, not from older WIP wording. Read
   `AGENTS.md`, `docs/ai/principles.md`, this file, and `RESUME.md`.
2. Treat `a29b0121062 Pin LCP demo benchmark README command` as the latest
   completed local tip before this checkpoint. If this section is committed,
   inspect `git log --oneline --decorate -8` for the new exact tip.
3. Continue the broader LCP solver/interface/demo audit from one concrete gap
   at a time. Do not retire this dev-task folder yet.
...
- Current relationship:
  `feature/lcp-solver-interface-demos...origin/feature/lcp-solver-interface-demos`
  with the local branch ahead by sixty-two commits before this edit.
- This branch has no associated PR. Do not push, open a PR, or mutate GitHub
  state without explicit maintainer/user approval.

Representative requirement-link status:

- `python/examples/demos/scenes/lcp_physics.py` now uses one local
  `_STANDALONE_PROBLEM_SUITE_LABEL` for the virtual representative solver-suite
  row referenced by requirement coverage and the UI.
- `scripts/check_lcp_solver_roster.py` now validates that every
  representative requirement row points only at known live packet labels, known
  benchmark packet IDs, the standalone solver-suite label, or known standalone
  problem-case IDs.
- `python/tests/unit/test_check_lcp_solver_roster.py` now covers the valid link
  parser plus stale live-packet and benchmark/standalone reference failures.
- Solver implementations, solver support predicates, benchmark registration
  code, profile artifacts, bindings, stubs, public APIs, generated profile
  CSVs, generated evidence CSVs, and demo runtime behavior were not
  intentionally changed.
...
- `git diff --check` passed.
- `pixi run build` passed.
- `pixi run -e cuda test-all` passed end to end on the visible NVIDIA RTX
  5000 Ada Generation Laptop GPU. The documentation phase still emitted the
