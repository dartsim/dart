...
| Feature-level collision correctness covers the current DART surface.                | `09-test-coverage-matrix.md` records 200 DONE, 0 PARTIAL, 0 GAP, and 25 DEFERRED rows; focused raw, convex, mesh, batch, `hello_world`, Atlas Simbicon, and capsule-box accumulated-result validations are recorded in `03-evidence-gates.md`.                                                                                                                             | Locally satisfied for the feature-level pass; deferred rows are explicitly outside this pass or belong to the performance wave.     |
