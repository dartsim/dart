...
convex / mesh batch narrow-phase coverage, mixed batch-dispatcher coverage,
mesh-heavy benchmark output consumption, pair-local capsule-box duplicate
filtering, ten-box stack, thin-box, and slender-capsule stability coverage
without changing the no-PR boundary. The latest local build-surface follow-up
also cleans stale legacy collision
artifacts and stale package export snippets from reused build directories after
the retained FCL/Bullet/ODE component names became native-backed interface
facades. This dev-task folder remains active because final PR/CI evidence
transfer and folder deletion are explicitly out of the current scope.
...
| ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Native `dart` detector is the normal runtime path.                                  | `README.md` current-status checklist, default-detector tests recorded in `03-evidence-gates.md`, `pixi run test-all` on `ca0201e67f4`, the evidence-hardening recheck on `5627a80a0a2`, the stale-artifact cleanup validation on `35578ad2f8a`, pre-record validations on `59769b3ee58` and `6d3224426ce`, and the latest recorded full local validation on `b5801f6c84c`.                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Locally satisfied; final PR validation still required.                                                                              |
...
| Feature-level collision correctness covers the current DART surface.                | `09-test-coverage-matrix.md` records 201 DONE, 0 PARTIAL, 0 GAP, and 25 DEFERRED rows; focused raw, convex, mesh, batch, `hello_world`, Atlas Simbicon, capsule-box accumulated-result, and benchmark-output-consumption validations are recorded in `03-evidence-gates.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Locally satisfied for the feature-level pass; deferred rows are explicitly outside this pass or belong to the performance wave.     |
| gz-physics compatibility remains green.                                             | `pixi run -e gazebo test-gz` passed 65/65 on `6742a21ab0f`, again after capsule-box fix head `c1f03f23147`, and again on current pre-record head `9cc3fe99008`; dependency scans showed the gz DART plugin depends on `libdart-collision-native.so` without old collision/reference runtime dependencies.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Locally satisfied; final PR/downstream CI evidence still required.                                                                  |
| Benchmark/profiling guardrails exist without making optimization part of this pass. | `pixi run -e collision-reference bm-collision-check` passed before benchmark-evidence head `4b155655890` and again after capsule-box fix head `c1f03f23147`; the raw narrow-phase harness audit on `0b425b3f7af` passed the focused narrow and raw-reference benchmark guards with output/contact-count consumption; the focused high-margin raw-row audit on `184c8be739d` emitted `native_collision_suspicious_win_raw_audit.json`; the convexity benchmark harness audit on `1f59af53d30` emitted algorithm, CCD, and convex-convex batch JSON with output/result consumption active; the mesh benchmark harness audit on `2c6a29eaf0d` passed the mesh-heavy guard and mesh-mesh scalar/batch JSON with output/contact-count consumption active; local benchmark JSON artifacts are recorded in `03-evidence-gates.md`. | Locally satisfied for guardrails; final benchmark artifact evidence on the completing PR surface remains open.                      |
...
| Key task insights are extracted to durable onboarding docs.                         | `docs/onboarding/architecture.md` records the built-in runtime collision stack and reference harness boundary, `docs/onboarding/build-system.md` records native-backed package facades and `dart-test-reference-*` targets, `docs/onboarding/python-bindings.md` records the clean `DartCollisionDetector` dartpy API, and `docs/onboarding/building.md` records FCL/Bullet/ODE as optional reference-comparison dependencies.                                                                                                                                                                                                                                                                                                                                                                                              | Locally satisfied; dev-task folder deletion still waits for the completing PR.                                                      |
| Completion packaging is done in the same PR.                                        | `PR-DRAFT.md` and `07-pr-evidence-transfer.md` stage the PR packet; `docs/dev_tasks/README.md` requires deleting `docs/dev_tasks/native_collision/` in the completing PR after evidence is transferred. Read-only GitHub checks on `92919c91863` found no workflow runs and only closed PR #2652 anchored to old head `714d220d82a`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Not complete by design under current user scope; requires explicit maintainer/user approval to open/select the final PR/CI surface. |
...
- Newest full pre-record validation head: `6d3224426ce`
  (`Clarify native collision validation snapshot`) passed the local
  `DART_PARALLEL_JOBS=$JOBS CTEST_PARALLEL_LEVEL=$JOBS CMAKE_BUILD_PARALLEL_LEVEL=$JOBS pixi run test-all`
  gate with 6/6 top-level gates: linting, build, unit tests,
  simulation-experimental tests, Python tests, and documentation. This was a
  docs-only validation-snapshot correction on top of the earlier evidence
  commits; no PR, push, workflow, branch, or GitHub state was mutated.
...
Read-only GitHub checks on local evidence head `1bfc9103a6b`, later
read-only rechecks on local docs/evidence heads `19aed78ae04` and
`6d3224426ce`, confirmed the final review/CI gap remains external to this
branch-local pass:
...
- The `gh run list` check for full commit
  `6d3224426ce1cb3e78a782a3f1aa604197540e37` also returned `[]`.
...
After the validation-snapshot documentation correction, the same full local
validation gate was rerun on clean head `6d3224426ce`
...
- Result: passed. The full local report passed 6/6 top-level gates:
...
| Benchmark/profiling guardrails exist for the next performance wave, with feature first and correctness preserved.            | The current local broad benchmark guard is refreshed in `4b155655890`; the raw narrow-phase benchmark harness audit is refreshed in `0b425b3f7af`; the focused high-margin raw-row audit is refreshed in `184c8be739d`; the convexity and mesh benchmark harness audits are refreshed in `1f59af53d30` and `2c6a29eaf0d`; `README.md` records feature-level benchmark baselines across primitive, narrowphase, supported distance, raycast, batch, mesh-heavy, and mixed workloads. Single-CPU optimization, multi-core CPU parallelism, and stretch GPU support are documented as follow-up performance work rather than current feature-completion blockers. | Local   |
...
- The latest current-head gz-physics compatibility refresh ran on pre-record
  local head `9cc3fe99008` with `pixi run -e gazebo test-gz`. It cloned a fresh
  `gz-physics9_9.0.0` checkout, configured the gazebo DART install with
  reference collision tests and benchmarks `OFF`, built the DART plugin, passed
  65/65 gz-physics tests, and printed the expected DART plugin integration
  success line. Direct `readelf` inspection of
