...
| FCL/Bullet/ODE remain only as optional reference engines for tests and benchmarks.                                           | `DART_BUILD_COLLISION_REFERENCE_TESTS` and `DART_BUILD_COLLISION_REFERENCE_BENCHMARKS` exist in CMake/Pixi configure paths; `collision-reference` is the explicit opt-in Pixi environment; the old `DART_BUILD_COLLISION_FCL/BULLET/ODE` options now document reference-only scope; the gz environment no longer enables them; reference call sites use `createReference()` and `collision-reference-*` names.                                                           | Local   |
