...
- Current clean reference-gate validation:
  ...
  - Build API change: the current CMake/Pixi/CI surface exposes only
    ...
    comparison jobs. Per-engine FCL/Bullet/ODE collision build switches are no
    longer present in current code/config paths; FCL, Bullet, and ODE
    reference components are derived internally when either public reference
    gate is enabled.
  - Commands:
    ...
    the native compatibility package/link smoke with direct `readelf` checks
  ...
  - Result: passed. Default configure reported reference tests and reference
    ...
    The fresh gz-physics run passed 65/65 selected tests after configuring the
    DART install with reference tests and benchmarks `OFF`. The package/link
    smoke passed, and `readelf` showed both the gz DART plugin and package
    smoke executable depend on `libdart-collision-native.so` without
    `libdart-collision-reference-*`, FCL, Bullet, ODE, or libccd runtime
    dependencies. Full local `test-all` passed 6/6 top-level gates: linting,
...
- Current local collision benchmark guard refresh:
  ...
  - Result: passed. The `collision-reference` configure intentionally enabled
    reference tests and reference benchmarks as opt-in comparison gates for
    FCL, Bullet, and ODE. Checked subsets passed with no failures or skips for
...
- Current local convex/SDF distance refresh:
  - Commit: current working tree after local head `3d52e117c7f`
    (`Cover SDF compound distance order`).
  - Commands:
    `CMAKE_BUILD_DIR=build/default/cpp/Release python scripts/cmake_build.py --target test_sdf_compare --parallel 5`,
    `./build/default/cpp/Release/bin/test_sdf_compare --gtest_filter='SdfDistance.ConvexVsSdf:SdfDistance.ConvexSdfPairOrder'`,
    `ctest --test-dir build/default/cpp/Release --output-on-failure -R '^test_sdf_compare$' -j 5`,
    `CMAKE_BUILD_DIR=build/default/cpp/Release python scripts/cmake_build.py --target dart_collision_native_tests --parallel 5`,
    and
    `ctest --test-dir build/default/cpp/Release --output-on-failure -L collision-native -j 5`.
  - Result: passed. Native distance now covers `ConvexShape` against
...
- Current local mesh/SDF distance refresh:
  ...
  - Result: passed. Native distance now covers `MeshShape` against `SdfShape`
    ...
    `collision-native` CTest label passed 29/29.
- Current local MPR algorithm coverage refresh:
  - Commit: current working tree after local head `0c96e3b341b`
    (`Add mesh SDF distance support`).
  - Commands:
    `CMAKE_BUILD_DIR=build/default/cpp/Release python scripts/cmake_build.py --target test_gjk --parallel 5`,
    `./build/default/cpp/Release/bin/test_gjk --gtest_filter='Mpr.SphereSpherePenetrationDepthAnalytic'`,
...
- Current local capsule-capsule batch narrow-phase refresh:
  ...
  - Commands:
    ...
    `CMAKE_BUILD_DIR=build/collision-reference/cpp/Release python scripts/cmake_build.py --target bm_comparative_narrow_phase --parallel 5`,
    and
    `./build/collision-reference/cpp/Release/bin/bm_comparative_narrow_phase --benchmark_filter='BM_NarrowPhase_CapsuleCapsule_Native_Batch_N(1|10|100|1000)$' --benchmark_min_time=1ms --benchmark_repetitions=1 --benchmark_out=.benchmark_results/native_collision_capsule_capsule_batch.json --benchmark_out_format=json`.
  - Result: passed. The new `collideCapsulesBatch(...)` API emits one result
    per `CapsulePair`, rejects malformed result spans and null capsule
    pointers, and the focused unit target passed 16/16 tests including exact
    batch-vs-scalar comparison for 100 deterministic colliding pairs. The
    reference-enabled benchmark target rebuilt cleanly, its built-in accuracy
    verification passed for all registered raw narrow-phase pairs, and the
...
- Current local full validation after slender-capsule stability coverage:
  ...
  - Result: passed. The focused capsule-box unit target passed 17/17 tests,
    ...
    branch, or GitHub state was mutated by this local validation pass.
- Current local full validation after remaining feature-level stress coverage:
  - Commit: `944bd95f874`
    (`Add hello world native collision regression`).
  - Coverage added since the previous full validation head:
    `MeshShape.BvhLargeMesh` / `BvhDegenerateTriangles` matrix evidence,
    `MeshMesh.BvhTraversalUsesCurrentTransform`, degenerate and scale narrow
    phase batch stress rows, mixed primitive stack stability, raw stress
    stability, convex bodies on static mesh landscape, convex fragments on
    static mesh landscape, and
    `WorldTests.DefaultNativeHelloWorldBoxDoesNotTunnel`.
  - Commands:
    `CMAKE_BUILD_DIR=build/default/cpp/Release python scripts/cmake_build.py --target UNIT_simulation_World --parallel 5`,
    `./build/default/cpp/Release/bin/UNIT_simulation_World --gtest_filter=WorldTests.DefaultNativeHelloWorldBoxDoesNotTunnel`,
    `./build/default/cpp/Release/bin/UNIT_simulation_World`,
    `ctest --test-dir build/default/cpp/Release --output-on-failure -L collision-native-stability -j 5`,
    `pixi run lint`,
    `git diff --check`, and
    `DART_PARALLEL_JOBS=5 CMAKE_BUILD_PARALLEL_LEVEL=5 CTEST_PARALLEL_LEVEL=5 pixi run test-all`.
  - Result: passed. The new `hello_world`-style regression passed, the full
    `UNIT_simulation_World` executable passed 81/81, the
    ...
    passed 6/6 top-level gates: linting, build, unit tests, simulation-experimental
...
- Current local full validation after latest current-audit evidence refresh:
  ...
  - Result: passed. The full local `pixi run test-all` report passed all 6
    ...
    workflow, branch, or GitHub state was mutated by this local validation
    pass.
- Current local full validation after validation-snapshot documentation
  correction:
  - Commit: `6d3224426ce`
    (`Clarify native collision validation snapshot`).
  - Command:
    `DART_PARALLEL_JOBS=$JOBS CTEST_PARALLEL_LEVEL=$JOBS CMAKE_BUILD_PARALLEL_LEVEL=$JOBS pixi run test-all`.
  - Result: passed. The full local `pixi run test-all` report passed all 6
    top-level gates: linting, build, unit tests, simulation-experimental tests,
