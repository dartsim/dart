...
| C++ deprecation behavior               | `DART_COLLISION_DEPRECATE_LEGACY_NAMES` defaults `ON`, legacy C++ detector classes carry deprecation attributes through that gate, and legacy factory keys warn while routing to `dart`. | Implemented locally; `audit-collision-compat-facades` verifies the default-on deprecation option, factory-key warning path, and retained-name routing to native DART collision.                                                    |
...
| Runtime/package compatibility evidence | gz-physics, native package smokes, installed headers, and link inspections prove retained names are migration facades rather than external runtime backend selectors.                    | Complete locally on recorded compatibility heads `6742a21ab0f` and `dcfc994542f`; current merged validation head `f949b7cbbbe` passed full local validation. Final PR/downstream CI evidence and PR evidence transfer remain open. |
