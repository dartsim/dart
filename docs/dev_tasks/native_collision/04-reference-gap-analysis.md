...
public DART API path. The current gz-physics blocker is now narrower:
`COMMON_TEST_collisions_dartsim`, `COMMON_TEST_detachable_joint_dartsim`, and
`COMMON_TEST_joint_transmitted_wrench_features_dartsim` pass, while
`COMMON_TEST_joint_features_dartsim` still fails the `JointDetach` exact-zero
off-axis velocity checks.
...
| gz custom mesh/plane contacts      | Working tree adds native plane/mesh narrowphase dispatch, caps generated plane/mesh contacts, fixes unbounded-plane AABB transforms, and orients contact normals to public pair order. The gz `MeshAndPlane` free-fall is fixed in the focused `COMMON_TEST_collisions_dartsim` run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | gz-physics mesh-plane and joint/contact feature tests pass through the built-in detector and compatibility facades without selecting FCL, Bullet, or ODE.                                                                                                                                                                                                                                                                    | Keep mesh/plane in the focused gz subset and rerun full `test-gz` after the remaining `JointDetach` residual is fixed.                                                                                                                                                                                                   |
| gz stacked cylinder contacts       | Working tree adds parallel-cylinder cap/side contact selection and DART tests for stacked cap depth/normal. Focused gz `COMMON_TEST_detachable_joint_dartsim` now passes after the fix.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Stacked cylinder/cylinder support is stable enough for gz detachable joints and DART simulation contact workloads.                                                                                                                                                                                                                                                                                                           | Add broader simulation evidence through full gz and DART solver tests before finalizing the gate.                                                                                                                                                                                                                        |
| gz joint detach residual           | Focused gz run still fails `COMMON_TEST_joint_features_dartsim` `JointFeaturesDetachTest/0.JointDetach` on tiny off-axis velocity components after detach. Temporary contact logging showed the failing window has base-vs-ground contacts only; the upper/lower detached links are not contacting anything. Temporary velocity instrumentation then showed the upper-link angular `Y` residual follows the base link's angular `Y` support motion, and the upper-link linear `X` residual is consistent with that base rotation through the joint offset. The remaining residual therefore comes from base plate/pole support contacts against gz's plane-as-large-box fallback, not from detach-state restoration. Multiple local cylinder-cap/large-box support-contact shaping experiments are rejected: tilted and mirrored variants made the test fail earlier at the expected upper-link upward-motion check, a centered support point still failed `Z > 1e-5` and left off-axis support motion, and a weighted support centroid again killed the expected upward `Z` motion. The retained source state keeps the validated axial cap support patch and the convex fallback for tilted cap/box cases. The values match the class of gz tolerance issue already conditionally skipped for DART when box-cylinder collision is present, but this branch still needs a DART-side reduction or a maintained downstream test strategy before the north-star gate can close. | gz-physics joint feature tests pass or the remaining exact-zero expectation is handled through an accepted downstream-compatible strategy that does not weaken native collision correctness.                                                                                                                                                                                                                                 | Reduce the `JointDetach` scene to a focused DART-side base-support solver/contact test or obtain an accepted downstream test strategy for the exact-zero tolerance debt. Avoid retrying cylinder-cap support shaping unless a new reduction preserves the expected upward motion and reduces base support angular drift. |
