...
| Factory and binding alias cleanup  | Working tree routes retained factory keys `fcl`, `fcl_mesh`, `bullet`, and `ode` to `DartCollisionDetector` in native-only and reference-enabled builds. Python detector compatibility names also resolve to the built-in detector, and dartpy no longer links legacy collision component targets.                                                                   | Any backend selected through public factory or Python compatibility names uses the built-in detector, regardless of legacy key spelling. | Convert direct C++ legacy detector classes, headers, and component names into native-backed facades or explicit references.                  |
...
11. Continue legacy backend abstraction cleanup for direct legacy detector
