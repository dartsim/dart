...
| Factory alias cleanup              | Working tree routes retained factory keys `fcl`, `fcl_mesh`, `bullet`, and `ode` to `DartCollisionDetector` in native-only and reference-enabled builds.                                                                                                                                                                                                             | Any backend selected through the public factory uses the built-in detector, regardless of legacy key spelling.         | Convert direct legacy detector classes, headers, and component names into native-backed facades or explicit references.                      |
...
10. Continue legacy backend abstraction cleanup for direct legacy detector
