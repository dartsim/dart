...
- Default and wheel Pixi dependency metadata now exclude `bullet-cpp`, `fcl`,
  ...
  test-only reference target, correctness-test, and benchmark option set for
  comparison work.
...
- Dartpy exposes the clean DART 7 detector API through
  `DartCollisionDetector`; legacy detector aliases `DARTCollisionDetector`,
  `FCLCollisionDetector`, `BulletCollisionDetector`, and
  `OdeCollisionDetector` are not exposed. `dartpy` no longer links legacy
  collision component targets, including in a reference-enabled build.
...
- Source-tree top-level FCL, Bullet, and ODE detector/group, All, PascalCase,
  ...
  `tests/dart/test/reference_collision/` and are used by reference tests and
  benchmarks.
- Runtime source isolation is checked by `check-collision-runtime-isolation`,
  which is wired into `lint` and `check-lint`. It fails if runtime source paths
  include FCL, Bullet, ODE, libccd, or old collision reference backend headers,
  if non-test code includes `dart/test/reference_collision/...`, or if legacy
  engine implementation sources move back under runtime collision directories.
...
| Gap                                | Current state                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | North-star target                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Follow-up / finalization step                                                                                                                                     |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Native type cleanup                | Working tree removes the unused direct `Cone`, `HeightField`, and `PointCloud` native enum values. DART cone, pyramid, ellipsoid, heightmap, multi-sphere, voxel-grid, and point-cloud behavior remains represented through the adapter's explicit convex, mesh, compound, or non-collidable paths instead of phantom native shape tags.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Native shape taxonomy matches real supported code paths and does not imply unsupported direct semantics.                                                                                                                                                                                                                                                                                                                                                            | Keep DART adapter tests as the source of truth for approximated or intentionally non-collidable DART shape behavior.                                              |
...
| Factory and binding alias cleanup  | Working tree routes retained factory keys `fcl`, `fcl_mesh`, `bullet`, and `ode` to `DartCollisionDetector` in native-only and reference-enabled builds. Dartpy exposes `DartCollisionDetector` without legacy detector aliases and no longer links legacy collision component targets. Direct public C++ legacy detector `create()` paths return native-backed `DartCollisionDetector` facades that may preserve legacy display type strings for gz-physics compatibility, retained `collision-fcl`/`collision-bullet`/`collision-ode` package components plus installed/source-tree legacy headers are native-backed facades, and C++ tests/benchmarks use `dart/test/reference_collision/...` includes, `createReference()` APIs, and `dart-test-reference-*` targets for intentional old-engine comparisons.                         | Any backend selected through public factory, retained public C++ legacy names, retained package component names, installed compatibility headers, or source-tree top-level compatibility headers uses the built-in detector unless the call site explicitly asks for a reference-only API/target. Dartpy legacy detector names are absent, not selectors or aliases. Legacy display strings on direct C++ facades are compatibility metadata, not engine selection. | Keep downstream migration and CI evidence aligned with the source/package facade split.                                                                           |
...
11. Done for the current pushed code/evidence head: split reference tests and
    benchmarks from runtime
    backend CMake targets with CMake opt-out options, explicit
    `dart-test-reference-*` targets, native-backed retained `collision-*`
    package component facades, default/wheel Pixi dependency metadata cleanup,
    repaired py312 wheel artifact inspection, focused validation, and
    workflow-dispatch full wheel-matrix evidence on head `1e1faf6feb1`. Keep
    final PR-state packaging evidence current before completion.
...
13. Done in working tree: dartpy exposes `DartCollisionDetector`, does not
    expose legacy detector aliases, and does not link legacy collision
    component targets even in a reference-enabled build.
...
17. Done in working tree: runtime source isolation linting rejects old-engine
    ...
    implementation sources outside `tests/dart/test/reference_collision/`.
18. Done in working tree: remove unused native `ShapeType` values for direct
    cone, heightfield, and point-cloud shapes so native taxonomy reflects real
    supported shape classes and DART adapter behavior.
...
- [x] `PointCloudShape` behavior is implemented or explicitly documented as
      non-collidable.
- [x] Native `ShapeType` taxonomy only names real native shape classes; DART
      shapes that use approximation or non-collidable adapter behavior do not
      leave phantom native shape tags.
...
- [x] Dartpy exposes `DartCollisionDetector` and omits legacy detector aliases.
...
- [x] Normal pixi configure paths default reference tests and reference
      benchmarks to `OFF`.
- [x] The explicit `collision-reference` environment enables the test-only
      reference targets, correctness-test targets, and benchmark targets.
- [x] Native-only install-style core build produces `libdart.so` without FCL,
      Bullet, ODE, or libccd runtime links.
- [x] Native-only install probe installs no FCL, Bullet, ODE, libccd,
      `libdart-collision-reference-*`, or `libdart-test-reference-*`
      libraries.
...
- [x] CI wheel-matrix artifacts do not bundle or link old collision engines as
      default runtime dependencies. Workflow-dispatch Publish dartpy run
      `25887941240` passed every Ubuntu, macOS, and Windows Python
      3.12/3.13/3.14 wheel build, repair, verify, test, and upload job for
      head `1e1faf6feb1`.
...
- [x] Runtime source isolation check fails if non-reference DART source paths
      include FCL, Bullet, ODE, libccd, or old collision reference backend
      headers, if non-test code includes `dart/test/reference_collision/...`,
      or if legacy engine implementation sources move back under runtime
...
- [x] Downstream migration contract documents that retained legacy runtime
      ...
      `dart-test-reference-*` targets plus `createReference()` APIs.
...
- [x] GitHub run artifacts prove the scheduled/manual benchmark guard is green
      in CI. Workflow-dispatch CI Linux run `25887939088` passed the
      `Collision Benchmark Guard` job and uploaded
