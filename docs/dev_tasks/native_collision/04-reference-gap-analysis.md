...
benchmark/profiling checks for the public DART API path. The current
gz-physics blocker is solver-facing custom mesh/plane contact: DART-side
unbounded-plane AABB, public mesh/plane, and native plane/mesh tests pass, but
gz-physics `MeshAndPlane` still free-falls, so the next reduction must cover
the gz custom mesh shape path and resulting contact semantics.
...
| gz custom mesh/plane contacts      | Working tree adds native plane/mesh narrowphase dispatch, caps generated plane/mesh contacts, and fixes unbounded-plane AABB transforms. Focused DART tests pass, but gz-physics `COMMON_TEST_collisions_dartsim` still free-falls in `MeshAndPlane`, and joint/contact feature tests still fail.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | gz-physics mesh-plane and joint/contact feature tests pass through the built-in detector and compatibility facades without selecting FCL, Bullet, or ODE.                                                                                                                                                                                                                                                                    | Reduce gz `CustomMeshShape` plus plane simulation to a DART test, inspect generated contacts, and fix native contact generation or adapter conversion before rerunning the focused gz subset. |
...
14. Done in working tree: runtime source isolation linting rejects old-engine
    includes from non-reference DART source paths and rejects legacy
    implementation sources outside explicit `reference/` paths.
15. Continue final CI, downstream migration, GitHub benchmark guard evidence,
...
- [ ] gz-physics `MeshAndPlane` and joint/contact feature tests pass through
      the built-in detector after reducing the custom mesh/plane failure to
      DART-side coverage.
