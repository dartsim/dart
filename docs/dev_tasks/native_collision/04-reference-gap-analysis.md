...
| Factory and binding alias cleanup  | Working tree routes retained factory keys `fcl`, `fcl_mesh`, `bullet`, and `ode` to `DartCollisionDetector` in native-only and reference-enabled builds. Python detector compatibility names also resolve to the built-in detector, dartpy no longer links legacy collision component targets, direct public C++ legacy detector `create()` paths return `DartCollisionDetector`, retained `collision-fcl`/`collision-bullet`/`collision-ode` package components and installed legacy detector headers are native-backed facades, and C++ tests/benchmarks use explicit `createReference()` APIs plus `collision-reference-*` targets for intentional old-engine comparisons. | Any backend selected through public factory, Python compatibility names, retained public C++ legacy names, retained package component names, or installed compatibility headers uses the built-in detector unless the call site explicitly asks for a reference-only API/target. | Finish source-tree legacy detector headers and old-engine source placement so retained runtime surfaces are native-backed facades or explicitly named reference surfaces. |
...
| Reference harness isolation        | Working tree adds reference test/benchmark CMake options, defaults normal pixi configure paths to native-only collision, keeps explicit reference opt-in through overrides, names old-engine libraries/components as `collision-reference-*`, keeps retained `collision-*` package component names as native-backed interface facades, and has focused reference-off/on, core-link, fresh runtime-link, installed package-export, Pixi lock metadata, `collision-reference` target, and repaired py312 wheel artifact evidence.                                                                                                                                               | Reference engines are enabled only for test/benchmark targets and can be disabled without affecting runtime targets.                                                                                                                                                             | Finish CI wheel-matrix inspection; then convert remaining direct legacy detector headers/source surfaces to native-backed facades or explicit reference surfaces.         |
| Performance guardrails             | Benchmark evidence now includes public DART adapter collision, distance, raycast, dirty-world paths, and an initial native-vs-reference JSON checker for a focused narrowphase subset.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Benchmarks cover primitive, mesh-heavy, mixed, raycast, distance, batch, dirty-world, and DART adapter API paths.                                                                                                                                                                | Broaden recurring JSON checks for native core plus public `DartCollisionDetector` query paths.                                                                            |
...
12. Done in working tree: direct public C++ legacy detector `create()` paths
    return the built-in `DartCollisionDetector`.
13. Continue legacy backend abstraction cleanup for legacy detector classes,
    headers, reference-only source placement, and final runtime backend
    deletion.
...
- [x] Direct public C++ legacy detector `create()` entry points return the
      built-in `DartCollisionDetector`.
...
- [x] Installed legacy detector headers are native-backed facades and compile
      ...
      installs.
- [x] Focused native-vs-reference collision benchmark JSON guard exists for a
      narrowphase subset.
- [ ] Source-tree legacy detector classes, headers, and old-engine source
      placement are native-backed facades or explicit reference-only surfaces.
- [ ] Broad recurring benchmark guards cover native core and public
