...
| gz joint detach residual           | Focused gz run still fails `COMMON_TEST_joint_features_dartsim` `JointFeaturesDetachTest/0.JointDetach` on tiny off-axis velocity components after detach. The values match the class of gz tolerance issue already conditionally skipped for DART when box-cylinder collision is present, but this branch still needs a DART-side reduction or a maintained downstream test strategy before the north-star gate can close.                                                                                                                                                                                                                                                                                                                                                                               | gz-physics joint feature tests pass or the remaining exact-zero expectation is handled through an accepted downstream-compatible strategy that does not weaken native collision correctness.                                                                                                                                                                                                                                 | Reduce the `JointDetach` scene to DART/gz contact evidence, identify whether the source is a real native contact bug, model-level collision filtering, or downstream tolerance debt, then fix or document the accepted path. |
