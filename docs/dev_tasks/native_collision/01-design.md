...
- One public implementation: every normal factory key, package component, and
  C++ compatibility class resolves to the built-in detector, while dartpy uses
  the clean `DartCollisionDetector` API without legacy Python aliases. Explicit
  reference APIs are the only path to old engines.
...
retained factory aliases route to `DartCollisionDetector`, dartpy exposes
`DartCollisionDetector` without legacy detector aliases, and normal
package/wheel surfaces no longer carry old collision dependencies. Reference
tests and benchmarks now
