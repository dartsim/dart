...
- [x] Enforced owner-local name uniqueness for experimental world multibodies,
      rigid bodies, and multibody links/joints, with autogenerated names
      skipping collisions and cross-multibody parent links rejected at
      construction time.
...
1. Audit the remaining gap between the design docs and the public C++/dartpy
   staging surface.
2. Prefer the next narrow API slice that improves the public facade without
   exposing ECS storage, component mappers, backend handles, or unsupported
   async/solver contracts.
3. When this design task reaches a durable stopping point, promote the useful
   notes and remove this dev-task folder in that completing PR.
