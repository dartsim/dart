...
The current slice enforces owner-local name uniqueness for experimental
`World` multibodies and `MultiBody` links/joints, makes autogenerated names
skip collisions, and rejects parent links from a different `MultiBody`.
Validate it, commit it, merge the latest `origin/main`, and push the branch.
Then continue auditing the next narrow public API gap from the design docs.
