...
current owner-local name uniqueness slice was `e087118112a` (`Add experimental
multibody collection snapshots`).
...
The current follow-up slice makes experimental rigid-body autogenerated names
skip existing rigid-body names, matching the same owner-local uniqueness rule
already applied to world multibodies and multibody links/joints. Validate it,
commit it, merge the latest `origin/main`, and push the branch. Then continue
auditing the next narrow public API gap from the design docs.
