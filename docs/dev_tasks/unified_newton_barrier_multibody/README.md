...
1. Finish validating `feature/newton-barrier-runtime-wiring`, merge latest
   `main`, push it, and open one PR targeting `main`.
2. Manage that PR through review/CI without splitting the runtime slices into
   separate PRs.
...
6. Keep broader runtime stepping and GPU claims out of scope until the row-specific CPU
