...
- Latest local follow-up: the Windows-only `test_boxed_lcp_contact` target now
  disables MSVC optimization for that single test binary to avoid a repeated
  MSVC 19.44 internal compiler error while still compiling and running the AVBD
  contact regressions on Windows. This is CI portability only; it does not close
  any AVBD solver, CPU-win, GPU, or paper-number gate.
