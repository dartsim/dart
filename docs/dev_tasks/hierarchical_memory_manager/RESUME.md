...
snapshot vectors, row-counter scratch, solve scratch vectors, and point-joint
...
destruction. AVBD warm-start inventories remain default-vector follow-up work.
...
row-counter scratch, solve scratch vectors, and point-joint input vectors are
the first nested stage payloads routed through world-owned
...
AVBD warm-start inventory internals, solver option/result vectors in rigid IPC,
or an API-compatible plan for `ComputeGraph`'s exposed edge/topological-order
