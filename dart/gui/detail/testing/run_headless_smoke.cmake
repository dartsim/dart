...
file(
  READ "${DART_GUI_FILAMENT_SCREENSHOT}"
  _actual_header
  LIMIT "${_header_size}"
)
...
if(NOT _pixel_sample MATCHES "[1-9a-fA-F]")
  message(
    FATAL_ERROR
    "dartsim screenshot sample contains only zero-valued pixels"
  )
endif()
...
if(DEFINED DART_GUI_FILAMENT_PYTHON AND DEFINED DART_GUI_FILAMENT_ANALYZER)
  ...
  execute_process(
    COMMAND
      "${DART_GUI_FILAMENT_PYTHON}" "${DART_GUI_FILAMENT_ANALYZER}"
      "${DART_GUI_FILAMENT_SCREENSHOT}" --width "${_width}" --height
      "${_height}" --mode "${_analysis_mode}"
