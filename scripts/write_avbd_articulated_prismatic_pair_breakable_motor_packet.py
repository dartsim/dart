#!/usr/bin/env python3
"""Write a validated AVBD articulated prismatic breakable motor packet."""
...
SCENE_ID = "avbd_articulated_prismatic_pair_motor_breakable_joint"
BENCHMARK_NAME = "BM_AvbdArticulatedPrismaticBreakableMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
...
class AvbdArticulatedPrismaticPairBreakableMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedPrismaticPairBreakableMotorPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedPrismaticPairBreakableMotorPacketError(
            "articulated prismatic pair breakable motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedPrismaticPairBreakableMotorPacketError(
            "articulated prismatic pair breakable motor capture must not force-drag"
        )
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_prismatic_pair_breakable_motor",
        "scene": SCENE_ID,
        "target": {
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedPrismaticPairBreakableMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(
        f"Wrote AVBD articulated prismatic pair breakable motor packet: {args.output}"
    )
    return 0
