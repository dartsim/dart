#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Static Friction evidence packet."""
...
SCENE_ID = "avbd_demo2d_static_friction"
BENCHMARK_NAME = "BM_AvbdDemo2dStaticFrictionStep"
...
class AvbdDemo2dStaticFrictionPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dStaticFrictionPacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dStaticFrictionPacketError(f"capture scene must be {SCENE_ID}")
    ...
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dStaticFrictionPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dStaticFrictionPacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdDemo2dStaticFrictionPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dStaticFrictionPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dStaticFrictionPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dStaticFrictionPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dStaticFrictionPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dStaticFrictionPacketError(
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "joints") != 0:
        raise AvbdDemo2dStaticFrictionPacketError("reference timing expected 0 joints")
    if _int_field(data, "collision_shapes") != 12:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected 12 collision shapes"
        )
    if _int_field(data, "box_count") != 11:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected box_count=11"
        )
    ...
    if not math.isclose(
        _finite_field(data, "ramp_angle"), _SOURCE_RAMP_ANGLE, abs_tol=1.0e-6
    ):
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected source ramp angle"
        )
    if _finite_field(data, "min_friction") != 1.0:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected min_friction=1"
        )
    if _finite_field(data, "max_friction") != 1.0:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected max_friction=1"
        )
    if _finite_field(data, "dynamic_min_friction") != 1.0:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected dynamic_min_friction=1"
        )
    if _finite_field(data, "dynamic_max_friction") != 1.0:
        raise AvbdDemo2dStaticFrictionPacketError(
            "reference timing expected dynamic_max_friction=1"
        )
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Static Friction row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Static Friction row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        ...
        "broad static-friction persistence and contact-coupling coverage beyond this source row",
    ]
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_static_friction_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "--scene avbd_demo2d_static_friction "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dStaticFrictionStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene static_friction "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dStaticFrictionPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Static Friction packet: {args.output}")
    return 0
