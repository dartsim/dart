...
def _load_results(inputs: list[Path]) -> dict:
    ...
    for entry in inputs:
        if entry.is_dir():
            # Skip a previously merged combined.json so a benchmark is not
            # counted twice (once from its per-target file and once from the
            # merge output, whose names are already humanized).
            files.extend(
...
def _to_benches(results: dict) -> list[dict]:
    ...
    for row in results.get("benchmarks", []):
        ...
        benches.append(
            {
                # Humanize for readable chart titles, matching the published
                # dashboard's --humanize merge. "family" drives the grouping of
                # charts into sections in the local preview only.
...
<title>DART 7 World Performance Dashboard</title>
...
<body>
<header>
  <h1>DART 7 World Performance Dashboard</h1>
  <p id="meta">Benchmark history for the DART 7 World, tracked over commits. Lower is better.</p>
</header>
<main id="root"></main>
<footer>
  Local preview of <code>benchmark-action/github-action-benchmark</code> (Chart.js).
...
// One chart per benchmark (x-axis = commits oldest -> newest), grouped into the
// solver-family sections from benchmark_display_names.py.
for (const [suite, runs] of Object.entries(data.entries)) {
  ...
  // Per-commit metadata used for axis labels and rich tooltips.
  ...
  // Families in first-seen order so the sections read top-to-bottom.
  const families = [];
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--suite-name",
        default="DART 7 World Performance",
