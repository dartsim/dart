#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Fracture source-demo evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo2d-fracture-packet.json"
)
SCENE_ID = "avbd_demo2d_fracture"
BENCHMARK_NAME = "BM_AvbdDemo2dFractureStep"
...
SOURCE_REFERENCE_ROW: dict[str, Any] = {
    ...
    "source_shapes": {
        "ground": {"size": (100.0, 0.5), "density": 0.0, "friction": 0.5},
        "chain": {
            "count": 11,
            "size": (1.0, 0.5),
            "density": 1.0,
...
class AvbdDemo2dFracturePacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dFracturePacketError("capture manifest schema_version must be 1")
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dFracturePacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo2dFracturePacketError("Fracture capture must not switch scenes")
    if manifest.get("force_drag") is not None:
        raise AvbdDemo2dFracturePacketError("Fracture capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dFracturePacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dFracturePacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo2dFracturePacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo2dFracturePacketError(f"{frames_dir}: no PNG frames found")

    width, height = _png_dimensions(screenshot)
    if _png_dimensions(frame_paths[0]) != (width, height):
        raise AvbdDemo2dFracturePacketError(
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dFracturePacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dFracturePacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dFracturePacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo2dFracturePacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo2dFracturePacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
    iteration_rows = [
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_bodies", 29.0)
            _require_counter(row, "rigid_body_joints", 10.0)
            _require_counter(row, "breakable_joints", 10.0)
            _require_counter(row, "joint_connected_collision_pairs", 10.0)
            _require_counter(row, "collision_shapes", 29.0)
            _require_counter(row, "source_scene_index", 18.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dFracturePacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dFracturePacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo2dFracturePacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "steps") < 1:
        raise AvbdDemo2dFracturePacketError("reference timing steps must be positive")
    if _int_field(data, "warmup_steps") < 0:
        raise AvbdDemo2dFracturePacketError(
            "reference timing warmup_steps must be non-negative"
        )
    if _finite_field(data, "break_force") != 500.0:
        raise AvbdDemo2dFracturePacketError("reference timing expected break_force=500")
    ...
    if _int_field(data, "broken_joints_after_timing") < 0:
        raise AvbdDemo2dFracturePacketError(
            "reference timing broken_joints_after_timing must be non-negative"
        )
    ...
    return {
        ...
        "invariants": {
            ...
            for key in (
                ...
                "joints",
                "breakable_joints",
                "joint_connected_collision_pairs",
                "collision_shapes",
                "chain_links",
                "falling_blocks",
                "support_blocks",
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Fracture row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Fracture row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        "remaining avbd-demo2d rope, spring, and source rows",
        "GPU AVBD row parity and same-hardware benchmark packets",
        "paper/site/video scene visual and performance packets",
        "broad breakable constraint and fracture coverage beyond this source row",
    ]
    if not comparison["dart_faster_than_reference"]:
        remaining_gates.insert(
            0,
            "DART CPU performance for this Fracture row must beat the native reference before a CPU win is claimed",
        )

    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_fracture_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo2d_fracture "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dFractureStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene fracture "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dFracturePacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Fracture packet: {args.output}")
    return 0
