#!/usr/bin/env python3
"""Write a validated AVBD same-multibody fixed breakable-joint packet."""
...
SCENE_ID = "avbd_articulated_fixed_pair_breakable_joint"
BENCHMARK_NAME = "BM_AvbdArticulatedBreakableJointStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_BREAK_FORCE_N = 1.0e-18
DEMO_RESET_BREAK_FORCE_N = 1.0e12
DEMO_PULL_FORCE_N = (0.0, 5.0, 0.0)
DEMO_FORCE_POINT_M = (0.55, 0.0, 0.0)
DEMO_CAPTURED_POSITION_M = (0.68, 0.0, 0.0)
DEMO_CAPTURED_YAW_RAD = 0.22
BENCHMARK_BREAK_FORCE_N = 1.0e12
BENCHMARK_GRAVITY_M_PER_S2 = (0.0, -9.81, 0.0)


class AvbdArticulatedFixedPairBreakableJointPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedFixedPairBreakableJointPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedFixedPairBreakableJointPacketError(
            "articulated fixed-pair breakable joint capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedFixedPairBreakableJointPacketError(
            "articulated fixed-pair breakable joint capture must not force-drag"
        )
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_fixed_pair_breakable_joint",
        "scene": SCENE_ID,
        "target": {
            "broad_breakable_constraint_complete": False,
            "row_family": (
                "public same-multibody articulated fixed point-joint break/reset"
            ),
            "scope": (
                "single public same-multibody articulated fixed point-joint "
        ...
        "visual_scene_invariants": {
            ...
            "breakable_joints": 1,
            "joint_anchor_scope": "same_multibody_pair",
            "captured_position_m": list(DEMO_CAPTURED_POSITION_M),
            "captured_yaw_rad": DEMO_CAPTURED_YAW_RAD,
            "pull_force_n": list(DEMO_PULL_FORCE_N),
            "force_point_m": list(DEMO_FORCE_POINT_M),
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedFixedPairBreakableJointPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD articulated fixed-pair breakable joint packet: {args.output}")
    return 0
