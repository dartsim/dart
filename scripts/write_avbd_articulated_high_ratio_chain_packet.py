...
SCENE_ID = "avbd_articulated_high_ratio_chain"
BENCHMARK_NAME = "BM_AvbdArticulatedHighRatioChainStep"
LINKS = 5
LIGHT_LINK_MASS = 1.0
TIP_LINK_MASS = 200.0
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedHighRatioChainPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedHighRatioChainPacketError(
            "high-ratio chain capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedHighRatioChainPacketError(
            "high-ratio chain capture must not force-drag"
        )
