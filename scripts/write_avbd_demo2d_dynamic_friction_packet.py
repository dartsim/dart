#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Dynamic Friction evidence packet."""
...
BENCHMARK_NAME = "BM_AvbdDemo2dDynamicFrictionStep"
...
SOURCE_REFERENCE_ROW: dict[str, Any] = {
    ...
    "source_shapes": {
        "ground": {"size": (100.0, 1.0), "density": 0.0, "friction": 0.5},
        "boxes": {
            "count": 11,
            "size": (1.0, 0.5),
            "density": 1.0,
...
class AvbdDemo2dDynamicFrictionPacketError(RuntimeError):
    pass
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Google Benchmark JSON containing BM_AvbdDemo2dDynamicFrictionStep.",
    )
    parser.add_argument(
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise AvbdDemo2dDynamicFrictionPacketError(
...
def _png_dimensions(path: Path) -> tuple[int, int]:
    ...
    if (
        ...
        raise AvbdDemo2dDynamicFrictionPacketError(f"{path}: expected a PNG image")
    width, height = struct.unpack(">II", header[16:24])
    if width < 1 or height < 1:
        raise AvbdDemo2dDynamicFrictionPacketError(f"{path}: invalid PNG dimensions")
    return width, height


def _artifact_path(manifest_dir: Path, value: object, key: str) -> Path:
    if not isinstance(value, str) or not value:
        raise AvbdDemo2dDynamicFrictionPacketError(f"capture manifest missing {key}")
    path = Path(value)
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dDynamicFrictionPacketError(f"capture scene must be {SCENE_ID}")
    ...
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dDynamicFrictionPacketError("capture manifest missing artifacts")
    ...
    if not frame_paths:
        raise AvbdDemo2dDynamicFrictionPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dDynamicFrictionPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dDynamicFrictionPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dDynamicFrictionPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dDynamicFrictionPacketError(
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "joints") != 0:
        raise AvbdDemo2dDynamicFrictionPacketError("reference timing expected 0 joints")
    ...
    if _finite_field(data, "initial_speed") != 10.0:
        raise AvbdDemo2dDynamicFrictionPacketError(
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Dynamic Friction row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Dynamic Friction row"
        ),
    }
...
def make_packet(
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_dynamic_friction_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dDynamicFrictionStep$ "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dDynamicFrictionPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Dynamic Friction packet: {args.output}")
    return 0
