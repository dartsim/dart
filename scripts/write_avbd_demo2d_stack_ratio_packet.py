#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Stack Ratio evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/" "avbd-demo2d-stack-ratio-packet.json"
)
SCENE_ID = "avbd_demo2d_stack_ratio"
BENCHMARK_NAME = "BM_AvbdDemo2dStackRatioStep"
...
class AvbdDemo2dStackRatioPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dStackRatioPacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dStackRatioPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo2dStackRatioPacketError(
            "Stack Ratio capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdDemo2dStackRatioPacketError("Stack Ratio capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dStackRatioPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dStackRatioPacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdDemo2dStackRatioPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dStackRatioPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dStackRatioPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dStackRatioPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo2dStackRatioPacketError("benchmark JSON missing benchmarks list")
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_bodies", 7.0)
            _require_counter(row, "rigid_body_joints", 0.0)
            _require_counter(row, "collision_shapes", 7.0)
            _require_counter(row, "source_scene_index", 12.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dStackRatioPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dStackRatioPacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo2dStackRatioPacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "rigid_bodies") != 7:
        raise AvbdDemo2dStackRatioPacketError(
            "reference timing expected 7 rigid bodies"
        )
    if _int_field(data, "dynamic_bodies") != 6:
        raise AvbdDemo2dStackRatioPacketError(
            "reference timing expected 6 dynamic bodies"
        )
    if _int_field(data, "static_bodies") != 1:
        raise AvbdDemo2dStackRatioPacketError("reference timing expected 1 static body")
    if _int_field(data, "joints") != 0:
        raise AvbdDemo2dStackRatioPacketError("reference timing expected 0 joints")
    if _int_field(data, "collision_shapes") != 7:
        raise AvbdDemo2dStackRatioPacketError(
            "reference timing expected 7 collision shapes"
        )
    if _int_field(data, "stack_ratio_box_count") != 6:
        raise AvbdDemo2dStackRatioPacketError(
            "reference timing expected stack_ratio_box_count=6"
        )
    if _int_field(data, "steps") < 1:
        raise AvbdDemo2dStackRatioPacketError("reference timing steps must be positive")
    ...
    if _finite_field(data, "max_box_size") != 32.0:
        raise AvbdDemo2dStackRatioPacketError(
            "reference timing expected max_box_size=32"
        )
    ...
    return {
        ...
        "invariants": {
            ...
            "max_box_size": data["max_box_size"],
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Stack Ratio row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Stack Ratio row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        "remaining avbd-demo2d card tower, spring, spring ratio, rod, soft-body, joint-grid, and net source rows",
        "GPU AVBD row parity and same-hardware benchmark packets",
        "paper/site/video scene visual and performance packets",
        "broad size-ratio stacking and high-ratio stability coverage beyond this source row",
    ]
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_stack_ratio_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo2d_stack_ratio "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dStackRatioStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene stack_ratio "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dStackRatioPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Stack Ratio packet: {args.output}")
    return 0
