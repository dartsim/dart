#!/usr/bin/env python3
"""Write a validated AVBD articulated compliant-joint evidence packet."""
...
SCENE_ID = "avbd_articulated_compliant_joints"
BENCHMARK_NAME = "BM_AvbdArticulatedCompliantJointStep"
BENCHMARK_ARGS = (1, 4, 16)
EXACT_PARENT_COMMIT = "0b0154573b842c142d31ef9feef4567c5677c4fd"
BEHAVIOR_TEST = (
    "VariationalIntegration."
    "AvbdCompliantPublicArticulatedJointMasksResistOnlyConstrainedMotion"
)
TIME_STEP = 0.002
START_STIFFNESS = 10.0
...
RESOLVED_SOLVER_IDENTITY = make_resolved_solver_identity(
    ...
    recorded_from=("articulated compliant-joint benchmark runtime identity counters"),
    multibody_integration_family="variational",
)


class AvbdArticulatedCompliantJointsPacketError(RuntimeError):
    pass
...
def _validate_capture(
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedCompliantJointsPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedCompliantJointsPacketError(
            "compliant-joint capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedCompliantJointsPacketError(
            "compliant-joint capture must not force-drag"
        )
...
def _validate_image_verdict(
    ...
    if verdict.get("pass") is not True:
        raise AvbdArticulatedCompliantJointsPacketError("image verdict must pass")
    checks = verdict.get("checks")
    if not isinstance(checks, dict):
        raise AvbdArticulatedCompliantJointsPacketError("image verdict missing checks")
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        _require_counter(row, "spherical_joints", float(arg))
        _require_counter(row, "revolute_joints", float(arg))
        _require_counter(row, "prismatic_joints", float(arg))
        _require_counter(row, "compliant_joints", float(3 * arg))
        representative[arg].append(row)

    missing = [arg for arg in BENCHMARK_ARGS if not representative[arg]]
    if missing:
        raise AvbdArticulatedCompliantJointsPacketError(
            "benchmark JSON missing compliant-joint scale rows: "
    ...
    for arg in BENCHMARK_ARGS:
        ...
        try:
            ...
            raise AvbdArticulatedCompliantJointsPacketError(str(exc)) from exc
        ...
        scale_data.append(
            {
                ...
                "compliant_joints": compliant_joints,
                "cpu_time_per_step_ns": cpu_time,
                "real_time_per_step_ns": real_time,
                "cpu_time_per_joint_ns": cpu_time / compliant_joints,
    ...
    return {
        ...
        "invariants": {
            ...
            "joint_types": ["spherical", "revolute", "prismatic"],
            "time_step": TIME_STEP,
...
def _validate_exact_parent_mutation(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(assertions, list) or len(assertions) != 5:
        raise AvbdArticulatedCompliantJointsPacketError(
            "exact-parent mutation must record five constrained-row failures"
        )
    ...
    for assertion in assertions:
        ...
        upper_bound = assertion.get("upper_bound")
        ...
        if (
            ...
            or float(actual) <= float(upper_bound)
        ):
            raise AvbdArticulatedCompliantJointsPacketError(
                "exact-parent mutation assertions must record finite "
                "actual values above their upper bounds"
            )
...
def make_packet(
    ...
    return {
        ...
        "packet": "avbd_articulated_compliant_joints",
        "scene": SCENE_ID,
        "target": {
            "contract_rows": [
            ...
            "scope": (
                "passive finite-stiffness articulated spherical, revolute, "
                "and prismatic masked rows on CPU"
            ),
        ...
        "correctness": {
            ...
            "cpp_multi_dof_inverse_test": (
                "VariationalIntegration."
                "ArticulatedInverseMassMultiDofMatchesDenseSolve"
            ),
            "python_scene_test": (
                "test_avbd_articulated_compliant_joints_demo_"
                "preserves_free_coordinates"
            ),
            "allocation_tests": [
        ...
        "reproduction": {
            "capture_command": (
                ...
                "BUILD_TYPE=Release python scripts/capture_py_demo.py "
                "--scene avbd_articulated_compliant_joints --frames 120 "
                "--width 960 --height 540 --view three-quarter "
                "--camera-distance 8 --camera-target 0,0,1.2 "
                "--output-dir <capture-dir>"
            ),
            "image_verdict_command": (
                "pixi run image-verdict -- <capture-dir>/"
                "avbd_articulated_compliant_joints.png "
                "--out <capture-dir>/image_verdict.json "
                "--meta scene=avbd_articulated_compliant_joints "
                "--meta view=three-quarter"
            ),
            "benchmark_command": (
                "taskset -c 18 "
                "build/default/cpp/Release/bin/bm_avbd_rigid_fixed_joint "
                "--benchmark_filter='BM_AvbdArticulatedCompliantJointStep/"
        ...
        "remaining_gates": [
            "finite articulated one-DOF velocity-motor coupling",
            "finite-row break-force accounting and fracture lifecycle",
            "same-multibody finite articulated endpoint pairs",
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedCompliantJointsPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD articulated compliant-joints packet: {args.output}")
    return 0
