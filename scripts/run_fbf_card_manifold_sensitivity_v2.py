#!/usr/bin/env python3
"""Run the frozen Native card-house manifold-sensitivity protocol v2."""
...
PROTOCOL = (
    ...
    "CARD_HOUSE_MANIFOLD_SENSITIVITY_V2.md"
)
...
SCHEMA_VERSION = "dart.fbf_card_manifold_sensitivity_v2/v1"
...
def _mode_summary(
    ...
    if returncode == 0:
        ...
        if failures:
            ...
            raise EvidenceError(
                f"{mode} return one does not match a frozen v2 exit class"
            )
    accepted_cap_counts = [
...
def _comparison(summaries: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "process_exit_class": {
            mode: summaries[mode]["process_exit_class"] for mode in MODES
        },
        "deltas": {
