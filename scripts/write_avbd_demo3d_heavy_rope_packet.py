#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo3d Heavy Rope evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo3d-heavy-rope-packet.json"
)
SCENE_ID = "avbd_demo3d_heavy_rope"
BENCHMARK_NAME = "BM_AvbdDemo3dHeavyRopeStep"
...
class AvbdDemo3dHeavyRopePacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo3dHeavyRopePacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo3dHeavyRopePacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo3dHeavyRopePacketError(
            "Heavy Rope capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdDemo3dHeavyRopePacketError("Heavy Rope capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo3dHeavyRopePacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo3dHeavyRopePacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo3dHeavyRopePacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo3dHeavyRopePacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo3dHeavyRopePacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dHeavyRopePacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dHeavyRopePacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo3dHeavyRopePacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo3dHeavyRopePacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
    iteration_rows = [
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dHeavyRopePacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dHeavyRopePacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo3dHeavyRopePacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "steps") < 1:
        raise AvbdDemo3dHeavyRopePacketError("reference timing steps must be positive")
    if _int_field(data, "warmup_steps") < 0:
        raise AvbdDemo3dHeavyRopePacketError(
            "reference timing warmup_steps must be non-negative"
        )
    if _finite_field(data, "heavy_block_size") != 5.0:
        raise AvbdDemo3dHeavyRopePacketError(
            "reference timing expected heavy_block_size=5.0"
        )
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo3d Heavy Rope row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo3d Heavy Rope row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        ...
        "broad high-ratio rope and point-joint stress coverage beyond this source row",
    ]
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo3d_heavy_rope_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo3d_heavy_rope "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo3dHeavyRopeStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo3d-source-dir> --scene heavy_rope "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo3dHeavyRopePacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo3d Heavy Rope packet: {args.output}")
    return 0
