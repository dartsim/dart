#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo3d Breakable source-demo evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo3d-breakable-packet.json"
)
SCENE_ID = "avbd_demo3d_breakable"
BENCHMARK_NAME = "BM_AvbdDemo3dBreakableStep"
...
SOURCE_REFERENCE_ROW: dict[str, Any] = {
    ...
    "source_shapes": {
        ...
        "chain": {"size": (1.0, 1.0, 0.5), "density": 1.0, "friction": 0.5},
        "support": {"size": (1.0, 1.0, 5.0), "density": 0.0, "friction": 0.5},
        "falling_block": {
            "size": (2.0, 1.0, 1.0),
            "density": 1.0,
            "friction": 0.5,
...
class AvbdDemo3dBreakablePacketError(RuntimeError):
    pass
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Google Benchmark JSON containing BM_AvbdDemo3dBreakableStep.",
    )
    parser.add_argument(
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise AvbdDemo3dBreakablePacketError(f"{path}: JSON root must be an object")
    return data
...
def _png_dimensions(path: Path) -> tuple[int, int]:
    ...
    if (
        ...
        raise AvbdDemo3dBreakablePacketError(f"{path}: expected a PNG image")
    width, height = struct.unpack(">II", header[16:24])
    if width < 1 or height < 1:
        raise AvbdDemo3dBreakablePacketError(f"{path}: invalid PNG dimensions")
    return width, height


def _artifact_path(manifest_dir: Path, value: object, key: str) -> Path:
    if not isinstance(value, str) or not value:
        raise AvbdDemo3dBreakablePacketError(f"capture manifest missing {key}")
    path = Path(value)
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo3dBreakablePacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo3dBreakablePacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo3dBreakablePacketError("Breakable capture must not switch scenes")
    if manifest.get("force_drag") is not None:
        raise AvbdDemo3dBreakablePacketError("Breakable capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo3dBreakablePacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo3dBreakablePacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo3dBreakablePacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo3dBreakablePacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo3dBreakablePacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dBreakablePacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dBreakablePacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo3dBreakablePacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo3dBreakablePacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dBreakablePacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dBreakablePacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo3dBreakablePacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "rigid_bodies") != 19:
        raise AvbdDemo3dBreakablePacketError(
            "reference timing expected 19 rigid bodies"
        )
    if _int_field(data, "joints") != 10:
        raise AvbdDemo3dBreakablePacketError("reference timing expected 10 joints")
    if _int_field(data, "breakable_joints") != 10:
        raise AvbdDemo3dBreakablePacketError(
            "reference timing expected 10 breakable joints"
        )
    if _int_field(data, "steps") < 1:
        raise AvbdDemo3dBreakablePacketError("reference timing steps must be positive")
    ...
    invariants: dict[str, Any] = {
        ...
        "joints": data["joints"],
        "breakable_joints": data["breakable_joints"],
        "final_time": data["final_time"],
    }
    if "broken_joints_after_timing" in data:
        invariants["broken_joints_after_timing"] = data["broken_joints_after_timing"]
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo3d Breakable row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo3d Breakable row"
        ),
    }
...
def make_packet(
    ...
    if not comparison["dart_faster_than_reference"]:
        remaining_gates.insert(
            0,
            "DART CPU performance for this Breakable row must beat the native reference before a CPU win is claimed",
        )

    return {
        "schema_version": 1,
        "packet": "avbd_demo3d_breakable_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo3d_breakable "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo3dBreakableStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo3d-source-dir> --scene breakable "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo3dBreakablePacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo3d Breakable packet: {args.output}")
    return 0
