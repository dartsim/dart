#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo3d Static Friction evidence packet."""
...
SCENE_ID = "avbd_demo3d_static_friction"
BENCHMARK_NAME = "BM_AvbdDemo3dStaticFrictionStep"
...
class AvbdDemo3dStaticFrictionPacketError(RuntimeError):
    pass
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Google Benchmark JSON containing BM_AvbdDemo3dStaticFrictionStep.",
    )
    parser.add_argument(
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise AvbdDemo3dStaticFrictionPacketError(
...
def _png_dimensions(path: Path) -> tuple[int, int]:
    ...
    if (
        ...
        raise AvbdDemo3dStaticFrictionPacketError(f"{path}: expected a PNG image")
    width, height = struct.unpack(">II", header[16:24])
    if width < 1 or height < 1:
        raise AvbdDemo3dStaticFrictionPacketError(f"{path}: invalid PNG dimensions")
    return width, height


def _artifact_path(manifest_dir: Path, value: object, key: str) -> Path:
    if not isinstance(value, str) or not value:
        raise AvbdDemo3dStaticFrictionPacketError(f"capture manifest missing {key}")
    path = Path(value)
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo3dStaticFrictionPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo3dStaticFrictionPacketError(
            "Static Friction capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdDemo3dStaticFrictionPacketError(
            "Static Friction capture must not force-drag"
        )

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo3dStaticFrictionPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo3dStaticFrictionPacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdDemo3dStaticFrictionPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo3dStaticFrictionPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dStaticFrictionPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dStaticFrictionPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_bodies", 13.0)
            _require_counter(row, "rigid_body_joints", 0.0)
            _require_counter(row, "collision_shapes", 13.0)
            _require_counter(row, "source_scene_index", 3.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dStaticFrictionPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dStaticFrictionPacketError(
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "rigid_bodies") != 13:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected 13 rigid bodies"
        )
    if _int_field(data, "dynamic_bodies") != 11:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected 11 dynamic bodies"
        )
    if _int_field(data, "static_bodies") != 2:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected 2 static bodies"
        )
    if _int_field(data, "joints") != 0:
        raise AvbdDemo3dStaticFrictionPacketError("reference timing expected 0 joints")
    if _int_field(data, "collision_shapes") != 13:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected 13 collision shapes"
        )
    ...
    if _finite_field(data, "min_friction") != 0.25:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected min_friction=0.25"
        )
    if _finite_field(data, "max_friction") != 1.0:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected max_friction=1"
        )
    if _finite_field(data, "dynamic_min_friction") != 0.25:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected dynamic_min_friction=0.25"
        )
    if _finite_field(data, "dynamic_max_friction") != 0.5:
        raise AvbdDemo3dStaticFrictionPacketError(
            "reference timing expected dynamic_max_friction=0.5"
        )
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo3d Static Friction row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo3d Static Friction row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        ...
        "broad static friction persistence and stacking coverage",
    ]
    if not comparison["dart_faster_than_reference"]:
        remaining_gates.insert(
            0,
            "DART CPU performance for this Static Friction row must beat the native reference before a CPU win is claimed",
        )

    return {
        "schema_version": 1,
        "packet": "avbd_demo3d_static_friction_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo3d_static_friction "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo3dStaticFrictionStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo3d-source-dir> --scene static_friction "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo3dStaticFrictionPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo3d Static Friction packet: {args.output}")
    return 0
