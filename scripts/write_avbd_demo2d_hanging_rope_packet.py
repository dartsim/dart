#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Hanging Rope source-demo evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/" "avbd-demo2d-hanging-rope-packet.json"
)
SCENE_ID = "avbd_demo2d_hanging_rope"
BENCHMARK_NAME = "BM_AvbdDemo2dHangingRopeStep"
...
SOURCE_REFERENCE_ROW: dict[str, Any] = {
    ...
    "dimension": 2,
    "scene_index": 8,
    "scene_name": "Hanging Rope",
    "scene_count": 19,
    "scene_builder": "sceneHangingRope",
    "solver_defaults": {
        "time_step": 1.0 / 60.0,
        "gravity_axis": "y",
        "gravity": -10.0,
...
class AvbdDemo2dHangingRopePacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dHangingRopePacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dHangingRopePacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo2dHangingRopePacketError(
            "Hanging Rope capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdDemo2dHangingRopePacketError(
            "Hanging Rope capture must not force-drag"
        )

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dHangingRopePacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dHangingRopePacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdDemo2dHangingRopePacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dHangingRopePacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dHangingRopePacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dHangingRopePacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo2dHangingRopePacketError("benchmark JSON missing benchmarks list")
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_bodies", 50.0)
            _require_counter(row, "rigid_body_joints", 49.0)
            _require_counter(row, "linear_point_joints", 49.0)
            _require_counter(row, "collision_shapes", 50.0)
            _require_counter(row, "source_scene_index", 8.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dHangingRopePacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dHangingRopePacketError(f"reference timing has non-finite {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _finite_field(data, "heavy_block_size") != 10.0:
        raise AvbdDemo2dHangingRopePacketError(
            "reference timing expected heavy_block_size=10"
        )
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Hanging Rope row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Hanging Rope row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        "remaining avbd-demo2d rope, spring, fracture, and source rows",
    ...
    if not comparison["dart_faster_than_reference"]:
        remaining_gates.insert(
            0,
            "DART CPU performance for this Hanging Rope row must beat the native reference before a CPU win is claimed",
        )

    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_hanging_rope_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo2d_hanging_rope "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dHangingRopeStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene hanging_rope "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dHangingRopePacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Hanging Rope packet: {args.output}")
    return 0
