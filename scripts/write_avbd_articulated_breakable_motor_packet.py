#!/usr/bin/env python3
"""Write a validated AVBD articulated breakable motor evidence packet."""
...
SCENE_ID = "avbd_articulated_motor_breakable_joint"
BENCHMARK_NAME = "BM_AvbdArticulatedBreakableMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_TARGET_SPEED_RAD_PER_S = 0.8
...
class AvbdArticulatedBreakableMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedBreakableMotorPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedBreakableMotorPacketError(
            "articulated breakable motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedBreakableMotorPacketError(
            "articulated breakable motor capture must not force-drag"
        )
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_breakable_motor",
        "scene": SCENE_ID,
        "target": {
            "row_family": "public articulated revolute velocity motor break/reset",
            "scope": (
                "single same-multibody public articulated revolute motor "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedBreakableMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD articulated breakable motor packet: {args.output}")
    return 0
