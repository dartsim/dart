#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo3d Stack evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo3d-stack-packet.json"
)
SCENE_ID = "avbd_demo3d_stack"
BENCHMARK_NAME = "BM_AvbdDemo3dStackStep"
...
class AvbdDemo3dStackPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo3dStackPacketError("capture manifest schema_version must be 1")
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo3dStackPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo3dStackPacketError("Stack capture must not switch scenes")
    if manifest.get("force_drag") is not None:
        raise AvbdDemo3dStackPacketError("Stack capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo3dStackPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo3dStackPacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo3dStackPacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo3dStackPacketError(f"{frames_dir}: no PNG frames found")

    width, height = _png_dimensions(screenshot)
    if _png_dimensions(frame_paths[0]) != (width, height):
        raise AvbdDemo3dStackPacketError(
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo3dStackPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dStackPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dStackPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo3dStackPacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo3dStackPacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
    iteration_rows = [
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_bodies", 11.0)
            _require_counter(row, "rigid_body_joints", 0.0)
            _require_counter(row, "collision_shapes", 11.0)
            _require_counter(row, "source_scene_index", 9.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo3dStackPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo3dStackPacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo3dStackPacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "rigid_bodies") != 11:
        raise AvbdDemo3dStackPacketError("reference timing expected 11 rigid bodies")
    if _int_field(data, "dynamic_bodies") != 10:
        raise AvbdDemo3dStackPacketError("reference timing expected 10 dynamic bodies")
    if _int_field(data, "static_bodies") != 1:
        raise AvbdDemo3dStackPacketError("reference timing expected 1 static body")
    if _int_field(data, "joints") != 0:
        raise AvbdDemo3dStackPacketError("reference timing expected 0 joints")
    if _int_field(data, "collision_shapes") != 11:
        raise AvbdDemo3dStackPacketError(
            "reference timing expected 11 collision shapes"
        )
    if _int_field(data, "stack_box_count") != 10:
        raise AvbdDemo3dStackPacketError("reference timing expected stack_box_count=10")
    if _int_field(data, "steps") < 1:
        raise AvbdDemo3dStackPacketError("reference timing steps must be positive")
    if _int_field(data, "warmup_steps") < 0:
        raise AvbdDemo3dStackPacketError(
            "reference timing warmup_steps must be non-negative"
        )
    if _finite_field(data, "box_z_spacing") != 1.5:
        raise AvbdDemo3dStackPacketError("reference timing expected box_z_spacing=1.5")
    if _finite_field(data, "min_friction") != 0.5:
        raise AvbdDemo3dStackPacketError("reference timing expected min_friction=0.5")
    if _finite_field(data, "max_friction") != 0.5:
        raise AvbdDemo3dStackPacketError("reference timing expected max_friction=0.5")
    ...
    return {
        ...
        "invariants": {
            ...
            "box_z_spacing": data["box_z_spacing"],
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo3d Stack row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo3d Stack row"
        ),
    }
...
def make_packet(
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo3d_stack_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo3d_stack "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo3dStackStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo3d-source-dir> --scene stack "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo3dStackPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo3d Stack packet: {args.output}")
    return 0
