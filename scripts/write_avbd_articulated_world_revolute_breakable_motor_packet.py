#!/usr/bin/env python3
"""Write a validated AVBD world-revolute breakable motor packet."""
...
SCENE_ID = "avbd_articulated_world_revolute_motor_breakable_joint"
BENCHMARK_NAME = "BM_AvbdArticulatedWorldRevoluteBreakableMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_TARGET_SPEED_RAD_PER_S = 0.75
DEMO_MAX_TORQUE_NM = 900.0
DEMO_BREAK_FORCE_N = 1.0e-18
DEMO_RESET_BREAK_FORCE_N = 1.0e12
DEMO_PULL_FORCE_N = (0.0, 5.0, 0.0)
BENCHMARK_TARGET_SPEED_RAD_PER_S = 0.5
BENCHMARK_MAX_TORQUE_NM = 800.0
BENCHMARK_BREAK_FORCE_N = 1.0e12


class AvbdArticulatedWorldRevoluteBreakableMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedWorldRevoluteBreakableMotorPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedWorldRevoluteBreakableMotorPacketError(
            "articulated world-revolute breakable motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedWorldRevoluteBreakableMotorPacketError(
            "articulated world-revolute breakable motor capture must not force-drag"
        )
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "invariants": {
            ...
            "time_step": TIME_STEP,
            "target_speed_rad_per_s": BENCHMARK_TARGET_SPEED_RAD_PER_S,
            "max_torque_nm": BENCHMARK_MAX_TORQUE_NM,
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_world_revolute_breakable_motor",
        "scene": SCENE_ID,
        "target": {
            "row_family": (
                "public world-anchored articulated revolute velocity motor "
                "break/reset"
            ),
            "scope": (
        ...
        "visual_scene_invariants": {
            ...
            "target_speed_rad_per_s": DEMO_TARGET_SPEED_RAD_PER_S,
            "max_torque_nm": DEMO_MAX_TORQUE_NM,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "--scene avbd_articulated_world_revolute_motor_breakable_joint "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedWorldRevoluteBreakableMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(
        f"Wrote AVBD articulated world-revolute breakable motor packet: {args.output}"
    )
    return 0
