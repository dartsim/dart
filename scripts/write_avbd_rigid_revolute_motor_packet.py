#!/usr/bin/env python3
"""Write a validated AVBD rigid revolute motor evidence packet."""
...
BENCHMARK_NAME = "BM_AvbdRigidRevoluteMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_TARGET_SPEED_RAD_PER_S = 1.2
DEMO_MAX_TORQUE_NM = 800.0
BENCHMARK_TARGET_SPEED_RAD_PER_S = 0.75
BENCHMARK_MAX_TORQUE_NM = 600.0


class AvbdRigidRevoluteMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdRigidRevoluteMotorPacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdRigidRevoluteMotorPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdRigidRevoluteMotorPacketError(
            "rigid revolute motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdRigidRevoluteMotorPacketError(
            "rigid revolute motor capture must not force-drag"
        )

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdRigidRevoluteMotorPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdRigidRevoluteMotorPacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdRigidRevoluteMotorPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdRigidRevoluteMotorPacketError("capture manifest missing ui_ready")
...
def _matches_benchmark_arg(row: dict[str, Any]) -> bool:
    ...
    return identity == f"{BENCHMARK_NAME}/{BENCHMARK_ARG}"


def _finite_counter(row: dict[str, Any], key: str) -> float:
    value = row.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdRigidRevoluteMotorPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdRigidRevoluteMotorPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_rigid_revolute_motor",
        "scene": SCENE_ID,
        "target": {
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_rigid_revolute_motor "
                "--frames 8 --width 640 --height 360 --output-dir <capture-dir>"
            ),
            "benchmark_command": (
                "build/default/cpp/Release/bin/bm_avbd_rigid_fixed_joint "
                "--benchmark_filter=BM_AvbdRigidRevoluteMotorStep/1$ "
        ...
        "remaining_gates": [
            ...
            "articulated motor visual/benchmark packets",
...
def main(argv: list[str]) -> int:
    ...
    except AvbdRigidRevoluteMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD rigid revolute motor packet: {args.output}")
    return 0
