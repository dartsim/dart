#!/usr/bin/env python3
"""Write a validated AVBD articulated breakable fixed-joint packet."""
...
class AvbdArticulatedBreakableJointPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedBreakableJointPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedBreakableJointPacketError(
            "articulated breakable joint capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedBreakableJointPacketError(
            "articulated breakable joint capture must not force-drag"
        )
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_breakable_joint",
        "scene": SCENE_ID,
        "target": {
        ...
        "reproduction": {
            "capture_command": (
                ...
                "--scene avbd_articulated_breakable_joint "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedBreakableJointPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD articulated breakable joint packet: {args.output}")
    return 0
