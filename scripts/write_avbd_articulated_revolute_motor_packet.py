#!/usr/bin/env python3
"""Write a validated AVBD articulated revolute motor evidence packet."""
...
SCENE_ID = "avbd_articulated_revolute_motor"
BENCHMARK_NAME = "BM_AvbdArticulatedRevoluteMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_TARGET_SPEED_RAD_PER_S = 1.0
...
class AvbdArticulatedRevoluteMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdArticulatedRevoluteMotorPacketError(
    ...
    if manifest.get("switch_scene") is not None:
        raise AvbdArticulatedRevoluteMotorPacketError(
            "articulated revolute motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdArticulatedRevoluteMotorPacketError(
            "articulated revolute motor capture must not force-drag"
        )
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_articulated_revolute_motor",
        "scene": SCENE_ID,
        "target": {
...
def main(argv: list[str]) -> int:
    ...
    except AvbdArticulatedRevoluteMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD articulated revolute motor packet: {args.output}")
    return 0
