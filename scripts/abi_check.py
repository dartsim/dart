...
def select_baseline_tag(major):
    ...
    result = subprocess.run(
        cmd, cwd=ROOT, check=True, text=True, stdout=subprocess.PIPE
    )
...
def find_library(build_dir, lib_name):
    ...
    raise FileNotFoundError(f"Unable to find {lib_name}.so under {build_dir}")
...
def main():
    ...
    if not from_ref or not baseline_commit:
        msg = (
            f"No from tag found for major {current_major}. Set --from or DART_ABI_FROM."
        )
    ...
    if args.skip_missing_targets:
        ...
        targets = [
            ...
            if target in from_targets and target in to_targets
        ]
        libs = targets
        if not targets:
