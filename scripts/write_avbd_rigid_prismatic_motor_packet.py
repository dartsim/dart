#!/usr/bin/env python3
"""Write a validated AVBD rigid prismatic motor evidence packet."""
...
BENCHMARK_NAME = "BM_AvbdRigidPrismaticMotorStep"
BENCHMARK_ARG = 1
TIME_STEP = 0.005
DEMO_TARGET_SPEED_M_PER_S = 0.8
DEMO_MAX_FORCE_N = 800.0
BENCHMARK_TARGET_SPEED_M_PER_S = 0.6
BENCHMARK_MAX_FORCE_N = 700.0


class AvbdRigidPrismaticMotorPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdRigidPrismaticMotorPacketError(
            "capture manifest schema_version must be 1"
        )
    if manifest.get("scene") != SCENE_ID:
        raise AvbdRigidPrismaticMotorPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdRigidPrismaticMotorPacketError(
            "rigid prismatic motor capture must not switch scenes"
        )
    if manifest.get("force_drag") is not None:
        raise AvbdRigidPrismaticMotorPacketError(
            "rigid prismatic motor capture must not force-drag"
        )

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdRigidPrismaticMotorPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdRigidPrismaticMotorPacketError(f"{screenshot}: screenshot not found")
    ...
    if not frame_paths:
        raise AvbdRigidPrismaticMotorPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdRigidPrismaticMotorPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdRigidPrismaticMotorPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdRigidPrismaticMotorPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def make_packet(capture_manifest: Path, benchmark_json: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "packet": "avbd_rigid_prismatic_motor",
        "scene": SCENE_ID,
        "target": {
            "row_family": "public free-rigid-body prismatic velocity motor",
            "scope": "single public free-rigid prismatic motor visual and benchmark",
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_rigid_prismatic_motor "
                "--frames 8 --width 640 --height 360 --output-dir <capture-dir>"
            ),
            "benchmark_command": (
                "build/default/cpp/Release/bin/bm_avbd_rigid_fixed_joint "
                "--benchmark_filter=BM_AvbdRigidPrismaticMotorStep/1$ "
        ...
        "remaining_gates": [
            ...
            "motor fracture packet coverage",
...
def main(argv: list[str]) -> int:
    ...
    except AvbdRigidPrismaticMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD rigid prismatic motor packet: {args.output}")
    return 0
