...
- every packet in `evidence/` against the `dart.citation_claim_evidence/v1`
  schema (missing target commit, scene digest, requested/resolved method,
  command, ensemble, disposition, claim boundary, or review record fails);
- metric groups that must be measured-with-method or explicitly typed
  `unsupported` with a reason -- never silently absent, null, or NaN;
