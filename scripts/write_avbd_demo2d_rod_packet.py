#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Rod source-demo evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo2d-rod-packet.json"
)
SCENE_ID = "avbd_demo2d_rod"
BENCHMARK_NAME = "BM_AvbdDemo2dRodStep"
...
class AvbdDemo2dRodPacketError(RuntimeError):
    pass
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dRodPacketError("capture manifest schema_version must be 1")
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dRodPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo2dRodPacketError("Rod capture must not switch scenes")
    if manifest.get("force_drag") is not None:
        raise AvbdDemo2dRodPacketError("Rod capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dRodPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dRodPacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo2dRodPacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo2dRodPacketError(f"{frames_dir}: no PNG frames found")

    width, height = _png_dimensions(screenshot)
    if _png_dimensions(frame_paths[0]) != (width, height):
        raise AvbdDemo2dRodPacketError("first frame dimensions do not match screenshot")
    if _png_dimensions(frame_paths[-1]) != (width, height):
        raise AvbdDemo2dRodPacketError("last frame dimensions do not match screenshot")
    ui_ready = manifest.get("ui_ready")
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dRodPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dRodPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dRodPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _require_counter(row: dict[str, Any], key: str, expected: float) -> None:
    ...
    if value != expected:
        raise AvbdDemo2dRodPacketError(
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo2dRodPacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo2dRodPacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
    iteration_rows = [
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "fixed_joints", 19.0)
            _require_counter(row, "collision_shapes", 20.0)
            _require_counter(row, "source_scene_index", 13.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dRodPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dRodPacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo2dRodPacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "steps") < 1:
        raise AvbdDemo2dRodPacketError("reference timing steps must be positive")
    ...
    if _finite_field(data, "min_friction") != 0.5:
        raise AvbdDemo2dRodPacketError("reference timing expected min_friction=0.5")
    if _finite_field(data, "max_friction") != 0.5:
        raise AvbdDemo2dRodPacketError("reference timing expected max_friction=0.5")
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Rod row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Rod row"
        ),
    }
...
def make_packet(
    ...
    remaining_gates = [
        "remaining avbd-demo2d card tower, spring, spring ratio, soft-body, joint-grid, and net source rows",
        "GPU AVBD row parity and same-hardware benchmark packets",
        "paper/site/video scene visual and performance packets",
        "broad rod/strand and high-ratio articulated stability coverage beyond this source row",
    ]
    if not comparison["dart_faster_than_reference"]:
        remaining_gates.insert(
            0,
            "DART CPU performance for this Rod row must beat the native reference before a CPU win is claimed",
        )

    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_rod_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo2d_rod "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dRodStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene rod "
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dRodPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Rod packet: {args.output}")
    return 0
