#!/usr/bin/env python3
"""Write a validated AVBD avbd-demo2d Motor source-demo evidence packet."""
...
DEFAULT_OUTPUT = Path(
    "docs/plans/104-vertex-block-descent-solver/avbd-demo2d-motor-packet.json"
)
SCENE_ID = "avbd_demo2d_motor"
BENCHMARK_NAME = "BM_AvbdDemo2dMotorStep"
...
class AvbdDemo2dMotorPacketError(RuntimeError):
    pass
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Google Benchmark JSON containing BM_AvbdDemo2dMotorStep.",
    )
    parser.add_argument(
        "--reference-timing-json",
        type=Path,
        required=True,
        help="JSON from scripts/run_avbd_demo2d_reference_timing.py.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise AvbdDemo2dMotorPacketError(f"{path}: JSON root must be an object")
    return data
...
def _png_dimensions(path: Path) -> tuple[int, int]:
    ...
    if (
        ...
        raise AvbdDemo2dMotorPacketError(f"{path}: expected a PNG image")
    width, height = struct.unpack(">II", header[16:24])
    if width < 1 or height < 1:
        raise AvbdDemo2dMotorPacketError(f"{path}: invalid PNG dimensions")
    return width, height


def _artifact_path(manifest_dir: Path, value: object, key: str) -> Path:
    if not isinstance(value, str) or not value:
        raise AvbdDemo2dMotorPacketError(f"capture manifest missing {key}")
    path = Path(value)
...
def _validate_capture(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != 1:
        raise AvbdDemo2dMotorPacketError("capture manifest schema_version must be 1")
    if manifest.get("scene") != SCENE_ID:
        raise AvbdDemo2dMotorPacketError(f"capture scene must be {SCENE_ID}")
    if manifest.get("switch_scene") is not None:
        raise AvbdDemo2dMotorPacketError("motor capture must not switch scenes")
    if manifest.get("force_drag") is not None:
        raise AvbdDemo2dMotorPacketError("motor capture must not force-drag")

    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, dict):
        raise AvbdDemo2dMotorPacketError("capture manifest missing artifacts")
    ...
    if not screenshot.is_file():
        raise AvbdDemo2dMotorPacketError(f"{screenshot}: screenshot not found")
    if not frames_dir.is_dir():
        raise AvbdDemo2dMotorPacketError(f"{frames_dir}: frame directory not found")

    frame_paths = sorted(frames_dir.glob("frame_*.png"))
    if not frame_paths:
        raise AvbdDemo2dMotorPacketError(f"{frames_dir}: no PNG frames found")
    ...
    if (first_width, first_height) != (width, height):
        raise AvbdDemo2dMotorPacketError(
    ...
    if not isinstance(ui_ready, dict):
        raise AvbdDemo2dMotorPacketError("capture manifest missing ui_ready")
...
def _finite_counter(row: dict[str, Any], key: str) -> float:
    ...
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dMotorPacketError(f"{BENCHMARK_NAME}: missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dMotorPacketError(f"{BENCHMARK_NAME}: non-finite {key}")
    return value
...
def _validate_benchmark(benchmark_json: Path) -> dict[str, Any]:
    ...
    if not isinstance(rows, list):
        raise AvbdDemo2dMotorPacketError("benchmark JSON missing benchmarks list")
    ...
    if not matching_rows:
        raise AvbdDemo2dMotorPacketError(f"benchmark JSON missing {BENCHMARK_NAME}")
    ...
    for row in matching_rows:
        ...
        if run_type == "iteration" or aggregate_name in ("mean", "median"):
            ...
            _require_counter(row, "rigid_body_joints", 1.0)
            _require_counter(row, "motors", 1.0)
            _require_counter(row, "collision_shapes", 0.0)
            _require_counter(row, "source_scene_index", 17.0)
...
def _finite_field(data: dict[str, Any], key: str) -> float:
    value = data.get(key)
    if not isinstance(value, (int, float)) or isinstance(value, bool):
        raise AvbdDemo2dMotorPacketError(f"reference timing missing {key}")
    value = float(value)
    if not math.isfinite(value):
        raise AvbdDemo2dMotorPacketError(f"reference timing has non-finite {key}")
    return value
...
def _int_field(data: dict[str, Any], key: str) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise AvbdDemo2dMotorPacketError(f"reference timing missing integer {key}")
    return value
...
def _validate_reference_timing(reference_json: Path) -> dict[str, Any]:
    ...
    if _int_field(data, "rigid_bodies") != 2:
        raise AvbdDemo2dMotorPacketError("reference timing expected two rigid bodies")
    if _int_field(data, "joints") != 1:
        raise AvbdDemo2dMotorPacketError("reference timing expected one joint")
    if _int_field(data, "motors") != 1:
        raise AvbdDemo2dMotorPacketError("reference timing expected one motor")
    if _int_field(data, "steps") < 1:
        raise AvbdDemo2dMotorPacketError("reference timing steps must be positive")
...
def _comparison(
    ...
    return {
        ...
        "performance_claim": (
            "DART beats the native avbd-demo2d Motor row"
            if ratio < 1.0
            else "DART does not yet beat the native avbd-demo2d Motor row"
        ),
    }
...
def make_packet(
    ...
    return {
        "schema_version": 1,
        "packet": "avbd_demo2d_motor_source_demo",
        "scene": SCENE_ID,
        ...
        "reproduction": {
            "capture_command": (
                ...
                "python scripts/capture_py_demo.py --scene avbd_demo2d_motor "
            ...
            "dart_benchmark_command": (
                ...
                "--benchmark_filter=BM_AvbdDemo2dMotorStep$ "
            ...
            "reference_timing_command": (
                ...
                "--source-dir <avbd-demo2d-source-dir> --scene motor "
                "--output <reference-timing-json>"
            ),
        },
        "remaining_gates": [
            "DART CPU performance for this Motor row must beat the native reference before a CPU win is claimed",
...
def main(argv: list[str]) -> int:
    ...
    except AvbdDemo2dMotorPacketError as exc:
        raise SystemExit(str(exc)) from exc
    write_packet(args.output, packet)
    print(f"Wrote AVBD demo2d Motor packet: {args.output}")
    return 0
