...
bool LCPSolver::SolveTemp(const Eigen::MatrixXd& _A, const Eigen::VectorXd& _b, Eigen::VectorXd& _x, int _numContacts, double _mu, int _numDir, bool _bUseODESolver)
{
    if (!_bUseODESolver)
    {
        int err = Lemke(_A, _b, _x);
    ...
    else
    {
        ...
        transferSolFromODEFormulationTemp(xODE, _x, _numDir, _numContacts);
...
void LCPSolver::transferToODEFormulationTemp(const Eigen::MatrixXd& _A, const Eigen::VectorXd& _b, Eigen::MatrixXd& _AOut, Eigen::VectorXd& _bOut, int _numDir, int _numContacts)
{
    int numOtherConstrs = _A.rows() - _numContacts * 3;
    int n = _numContacts * 3 + numOtherConstrs;
    Eigen::MatrixXd AIntermediate = Eigen::MatrixXd::Zero(n, _A.cols());
    _AOut = Eigen::MatrixXd::Zero(n, n);
    _bOut = Eigen::VectorXd::Zero(n);
    ...
    for (int i = 0; i < _numContacts; ++i)
    {
        ...
        AIntermediate.row(_numContacts + i * 2 + 0) = _A.row(_numContacts + i * 2 + 0);
        AIntermediate.row(_numContacts + i * 2 + 1) = _A.row(_numContacts + i * 2 + offset);
        _bOut[_numContacts + i * 2 + 0] = _b[_numContacts + i * 2 + 0];
        _bOut[_numContacts + i * 2 + 1] = _b[_numContacts + i * 2 + offset];
    }
    for (int i = 0; i < numOtherConstrs; i++) {
        AIntermediate.row(_numContacts * 3 + i) = _A.row(_numContacts * 3 + i);
        _bOut[_numContacts * 3 + i] = _b[_numContacts * 3 + i];
    }
    for (int i = 0; i < _numContacts; ++i)
    {
        _AOut.col(i) = AIntermediate.col(i);
        _AOut.col(_numContacts + i * 2 + 0) = AIntermediate.col(_numContacts + i * 2 + 0);
        _AOut.col(_numContacts + i * 2 + 1) = AIntermediate.col(_numContacts + i * 2 + offset);
    }
    for (int i = 0; i < numOtherConstrs; i++)
        _AOut.col(_numContacts * 3 + i) = AIntermediate.col(_numContacts * 3 + i);
...
void LCPSolver::transferSolFromODEFormulationTemp(const Eigen::VectorXd& _x, Eigen::VectorXd& _xOut, int _numDir, int _numContacts)
{
    int numOtherConstrs = _x.size() - _numContacts * 3;
    _xOut = Eigen::VectorXd::Zero(_numContacts * 3 + numOtherConstrs);
    ...
    for (int i = 0; i < _numContacts; ++i)
    {
        _xOut[_numContacts + i * 2 + 0] = _x[_numContacts + i * 2 + 0];
        _xOut[_numContacts + i * 2 + offset] = _x[_numContacts + i * 2 + 1];
    }
    for (int i = 0; i < numOtherConstrs; i++)
        _xOut[_numContacts * 3 + i] = _x[_numContacts * 3 + i];
}
