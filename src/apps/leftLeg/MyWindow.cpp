  ...
  static bool inverse = false;
	switch(key) {
		...
		case '1': {  // Set first value -- Assume I have 6 DOF
			...
			pose(0) = pose(0) + (inverse ? -dDOF : dDOF);
		...
		case '2': {  // Set first value -- Assume I have 6 DOF
			...
			pose(1) = pose(1) + (inverse ? -dDOF : dDOF);
			mSkels[0]->setPose(pose);
		...
		case '3': {  // Set first value -- Assume I have 6 DOF
			Eigen::VectorXd pose;
			mSkels[0]->getPose(pose);
			// Increase DOF value (DOF 2)
			pose(2) = pose(2) + (inverse ? -dDOF : dDOF);
			mSkels[0]->setPose(pose);
		...
		case '4': {  // Set first value -- Assume I have 6 DOF
			Eigen::VectorXd pose;
			mSkels[0]->getPose(pose);
		...
		case '5': {  // Set first value -- Assume I have 6 DOF
			Eigen::VectorXd pose;
			mSkels[0]->getPose(pose);
		...
		case '6': {  // Set first value -- Assume I have 6 DOF
			Eigen::VectorXd pose;
			mSkels[0]->getPose(pose);
