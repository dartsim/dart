    ...
    takeScreenshot();
		...
		for(int i = 0; i < numIter; i++) {
			...
			if(moveCondition1 || moveCondition2 || moveCondition3 || moveCondition4) {
				...
				for(size_t dof_idx = 6; dof_idx < 12; dof_idx++) {
					dofs(dof_idx) = rightDofs(dof_idx - 6);
					dofs(dof_idx - 6) = rightDofs(dof_idx - 6);
				}
...
//				printf("changing..\n");
			...
			else mController->computeTorques(mDofs[1], mDofVels[1]);
