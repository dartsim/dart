...
int main(int argc, char* argv[]) {
	...
	// **********
	//-- Create Right Leg skeleton
	dynamics::SkeletonDynamics RightLegSkel;
	...
	//-- *****  BodyNode 1: Right Hip Yaw (LHY) *****
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LHY");
	...
	vmin = 0;
	vmax = PI;
	add_DOF(&RightLegSkel, joint, val, vmin, vmax, GOLEM_YAW);
	...
	// Add node to Skel
	RightLegSkel.addNode(node);

	//-- ***** BodyNode 2: Right Hip Roll (LHR)  whose parent is: LHY*****
	parent_node = (dynamics::BodyNodeDynamics*) RightLegSkel.getNode("LHY");
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LHR");
	...
	vmin = 0;
	vmax = PI;
	add_DOF(&RightLegSkel, joint, val, vmin, vmax, GOLEM_ROLL);
	...
	//-- ***** BodyNode 3: Right Hip Pitch (LHP) whose parent is: LHR*****
	parent_node = (dynamics::BodyNodeDynamics*) RightLegSkel.getNode("LHR");
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LHP");
	...
	parent_node = (dynamics::BodyNodeDynamics*) RightLegSkel.getNode("LHP");
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LKP");
	...
	//-- ***** BodyNode 5: Right Ankle Pitch (LAP) whose parent is: LKP*****
	parent_node = (dynamics::BodyNodeDynamics*) RightLegSkel.getNode("LKP");
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LAP");
...
//  -0.000045000000000  -0.024755000000000   0.299942000000000
	...
	//-- ***** BodyNode 6: Right Ankle Roll (LAR) whose parent is: LAP*****
	parent_node = (dynamics::BodyNodeDynamics*) RightLegSkel.getNode("LAP");
	node = (dynamics::BodyNodeDynamics*) RightLegSkel.createBodyNode("LAR");
	...
	RightLegSkel.initSkel();

	// Verify that our skeleton has something inside :)
	printf("Our RightLeg has %d nodes \n", RightLegSkel.getNumNodes());
	printf("Our RightLeg has %d joints \n", RightLegSkel.getNumJoints());
	printf("Our RightLeg has %d DOFs \n", RightLegSkel.getNumDofs());

	MyWindow window(&RightLegSkel, NULL);
