...
void MyWindow::initDyn()
{
    ...
    int nDof = mModel->getNumDofs();
    mDofs.resize(nDof);
    mDofVels.resize(nDof);
    ...
    mModel->setKd(MatrixXd::Identity(nDof, nDof)); 
}
...
VectorXd MyWindow::evalDeriv() {
    ...
    VectorXd qddot = (mModel->getMassMatrix() + mModel->getKd() * mTimeStep).fullPivHouseholderQr().solve(-mModel->getCombinedVector() + mModel->getExternalForces() + mModel->getInternalForces()); // the control force is scaled by mass matrix 
    // SPD
    //    VectorXd qddot = (mModel->getMassMatrix() + mModel->getKd() * mTimeStep).fullPivHouseholderQr().solve(-mModel->getCombinedVector() + mModel->getExternalForces() - mController->getKp() * (mDofs + mDofVels * mTimeStep - mController->getDesiredDofs()) - mController->getKd() * mDofVels); // the control
