...
void MyWindow::initDyn() {
	...
	// Initialize the positions and velocities for the dofs of the skeletons
	...
	// Get the position of the skeleton in the beginnning of the motion
	...
	for (unsigned int i = 0; i < mSkels.size(); i++) {
		...
		// Compute the dynamics (i.e. mass matrix and etc.) here because the computation of control
		// force at first iteration will need them
...
VectorXd MyWindow::evalDeriv() {
	...
	for (unsigned int i = 0; i < mSkels.size(); i++) {
		...
		VectorXd qddot = mSkels[i]->getInvMassMatrix()
				* (-mSkels[i]->getCombinedVector() + mSkels[i]->getExternalForces()
...
void MyWindow::displayTimer(int _val) {
	...
	if(mPlay) {
		...
		for (int i = 0; i < numIter; i++) {
			//            tSim.startTimer();
			static_cast <BodyNodeDynamics*>(mSkels[0]->getNode("fixedHand_indexDIP"))->addExtForce(
					Vector3d(0.02, 0.0, 0), mForce);
